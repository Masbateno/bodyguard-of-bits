...
class ContainerSecuritySnapshot:
    ...
    def from_system(cls) -> "ContainerSecuritySnapshot":
        ...
        # a non-zero cost, deferred pending a deliberate decision rather than a
        # piecemeal one. Kept, not removed: the value costs one dict lookup on a
        # string already read, and discarding it would only mean re-deriving it
        # later.
