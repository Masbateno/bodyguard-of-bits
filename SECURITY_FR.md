...
Si BOB est packagé pour une distro qui supporte le MAC (AppArmor sur Ubuntu/Debian, SELinux sur RHEL/Fedora) :
  ...
  - Le profil DOIT autoriser la lecture sur `/etc/`, `/proc/`, `/sys/`, `/var/log/` et l'exécution de `/usr/sbin/ufw`, `/usr/bin/ss`, `/usr/sbin/iptables`, `/usr/bin/systemctl`, `/usr/bin/journalctl`, `/usr/bin/openssl`, `/usr/bin/smartctl`, `/usr/bin/fwupdmgr`, `/usr/bin/aa-status`, `/usr/bin/apt-cache`, `/usr/bin/dpkg`.
