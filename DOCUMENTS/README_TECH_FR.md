...
![Release](https://img.shields.io/badge/version-v0.15.5-brightgreen)
...
- **`--explain KEY`** — explication structurée par constat (POURQUOI / COMMENT CORRIGER / référence CIS) ; 186 clés sur 49 préfixes ; 70 clés avec sections par profil ; TUI interactif ; sans droit root ; `--explain list` liste toutes les clés
...
À partir de v0.11.x, le set de clés `--explain` contient **186 clés** réparties sur **49 préfixes**. La convention de nommage canonique est appliquée par `tests/test_explain_naming_convention.py` :
