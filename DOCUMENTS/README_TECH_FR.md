...
- **`--explain KEY`** — explication structurée par constat (POURQUOI / COMMENT CORRIGER / référence CIS) ; 181 clés sur 46 préfixes ; 70 clés avec sections par profil ; TUI interactif ; sans droit root ; `--explain list` liste toutes les clés
...
À partir de v0.11.x, le set de clés `--explain` contient **181 clés** réparties sur **46 préfixes**. La convention de nommage canonique est appliquée par `tests/test_explain_naming_convention.py` :
