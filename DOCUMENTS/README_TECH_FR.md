...
- **Cartographie CIS inline** — chaque finding affiche son code CIS `[CIS:X.Y.Z]` dans la boîte de synthèse ; référence complète en mode `--verbose` ; 198 entrées (109 CIS formels, 82 best-practice, 7 Docker)
...
À partir de v0.11.x, le set de clés `--explain` contient **193 clés** réparties sur **50 préfixes**. La convention de nommage canonique est appliquée par `tests/test_explain_naming_convention.py` :
