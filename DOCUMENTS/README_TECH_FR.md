...
- **Cartographie CIS inline** — chaque finding affiche son code CIS `[CIS:X.Y.Z]` dans la boîte de synthèse ; référence complète en mode `--verbose` ; 196 entrées (109 CIS formels, 80 best-practice, 7 Docker)
...
- **`--explain KEY`** — explication structurée par constat (POURQUOI / COMMENT CORRIGER / référence CIS) ; 191 clés sur 50 préfixes ; 106 d'entre elles rendent une section par profil — 71 avec une prose écrite pour lui, les autres avec une note dérivée du fichier de profil — et 85 s'appliquent identiquement à tous les profils ; TUI interactif ; sans droit root ; `--explain list` liste toutes les clés
...
║                           — Bodyguard Of Bits —                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  BOB v0.18.0  │  Auditeur de durcissement Linux                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
À partir de v0.11.x, le set de clés `--explain` contient **191 clés** réparties sur **50 préfixes**. La convention de nommage canonique est appliquée par `tests/test_explain_naming_convention.py` :
