...
- **Cartographie CIS inline** — chaque finding affiche son code CIS `[CIS:X.Y.Z]` dans la boîte de synthèse ; référence complète en mode `--verbose` ; 192 entrées (108 CIS formels, 77 best-practice, 7 Docker)
...
║                           — Bodyguard Of Bits —                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  BOB v0.16.4  │  Auditeur de durcissement Linux                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
| `duration_seconds` | float \| null | Durée de l'audit en secondes, arrondie à la milliseconde. Mesurée depuis le démarrage des checks — pas depuis l'entrée du processus, donc hors analyse des arguments et vérification root — avec une horloge monotone, si bien qu'un ajustement d'horloge en cours d'exécution ne peut pas la rendre négative. `null` quand l'exécution ne s'est pas chronométrée. Nouveau en v0.16.4, additif dans v3. |
