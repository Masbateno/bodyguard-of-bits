...
| Version | Tests | Notes |
|---------|-------|-------|
| v0.19.0 | 10455 | **Remediation that applies on modern Include/drop-in configs — field-tested on a real Pi Zero W (Samba 4.22).** Validated against the service's own parser (`testparm`, `sshd -T`) + round-trip. **samba**: the SMB1/signing/map fixes appended without deleting; samba is last-wins so `testparm` kept the bad value, and BOB's first-wins parser then read the finding as *resolved* — a fix that lied. Now delete-then-insert. **ssh**: `PermitRootLogin`/`PasswordAuthentication`/`MaxAuthTries` edited the main file, but OpenSSH reads `sshd_config.d/*.conf` first (`50-cloud-init.conf` had `PasswordAuthentication yes`); fix now writes a first-winning `00-bob-hardening.conf`. |
