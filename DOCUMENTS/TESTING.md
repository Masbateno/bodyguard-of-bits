...
| Version | Tests | Notes |
|---------|-------|-------|
| v0.19.0 | 10470 | **Remediation that applies on modern Include/drop-in configs — field-tested on a real Pi Zero W (Samba 4.22); `updates` made cross-distro on real VMs.** Validated against the service's own parser (`testparm`, `sshd -T`) + round-trip. **samba**: the SMB1/signing/map fixes appended without deleting; samba is last-wins so `testparm` kept the bad value, and BOB's first-wins parser then read the finding as *resolved* — a fix that lied. Now delete-then-insert. **ssh**: `PermitRootLogin`/`PasswordAuthentication`/`MaxAuthTries` edited the main file, but OpenSSH reads `sshd_config.d/*.conf` first (`50-cloud-init.conf` had `PasswordAuthentication yes`); fix now writes a first-winning `00-bob-hardening.conf`. **updates**: was apt-only (blind on 4/5 families — a Fedora host with 93 pending security updates read "no apt"); now detects dnf/zypper/pacman/apk and reads each through its own resolver, with a per-manager upgrade command and the −2 security deduction (pacman/apk have no security channel → regular INFO). Fixtures pinned to real VM output. |
