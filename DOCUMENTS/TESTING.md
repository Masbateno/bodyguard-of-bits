...
- **Unit test history** (per-version table, plus detailed sections for the early releases) — every release lists the tests added, removed, or corrected, with the platform and test count at the time. This is the audit trail of how the suite grew from v0.1.0 (4200 tests) to the current release (over 10,900). Contract-preserving releases (like the v0.6.0 `ssh.py` / `cron.py` splits and the UFW_AUDIT_SHARE sunset) add zero test delta; the deep-audit and real-hardware field-test campaigns account for most of the growth.
...
| Version | Tests | Notes |
|---------|-------|-------|
| 0.21.3 | 11119 | **Patch — BOB reads the distribution's real config paths, not just Debian's: `/usr/etc`, doas, and SELinux precedence.** Four distro-mechanism gaps from the v0.21.2 field campaign (openSUSE Leap 16, fresh Alpine 3.24), each changing output only on the affected layout. **(1)** SSH audit blind on the `/usr/etc` layout (MEDIUM-HIGH) — openSUSE ships `sshd_config` under `/usr/etc`, so BOB parsed `/etc` only, never followed the drop-in `Include`, and reported OpenSSH defaults, masking a real `PermitRootLogin yes`; now falls back to `/usr/etc/ssh/sshd_config` (guard `test_v0213_sshd_config_usr_etc_fallback.py`, mutation `ssh/usr-etc-sshd-config-not-read`). **(2)** doas now audited (MEDIUM) — `permit nopass` in `/etc/doas.conf` (Alpine/OpenBSD passwordless root, the `NOPASSWD:ALL` equivalent) was invisible; now WARN −2 with `--explain file_perms.doas_nopass_all` (guard `test_v0213_doas_audit.py`, mutation `file_perms/doas-nopass-not-flagged`). **(3)** permissive SELinux no longer misdiagnosed as AppArmor-off (MEDIUM) — the AppArmor-off branch deferred to SELinux when it is the installed MAC (guard `test_v0213_mac_policy_selinux_precedence.py`, mutation `mac_policy/apparmor-off-steals-selinux-permissive`). **(4)** login.defs + sudoers `/usr/etc` fallback (LOW) (guard `test_v0213_usr_etc_login_defs_sudoers.py`, mutations `password_policy/login-defs-usr-etc-not-read`, `file_perms/sudoers-usr-etc-vendor-not-read`). Explain 205→206, CIS refs 205→206, locale 2595→2606. Scoring changes only on affected layouts. **Three lower-severity field findings also fixed**: firewalld-stopped banner now "installed (inactive)" (guard `test_v0213_firewalld_banner_presence.py`, mutation `sysinfo/firewalld-stopped-reads-not-installed`); doas/bbsuid recognised as known SUID (guard `test_v0213_suid_doas_bbsuid.py`, mutation `suid/doas-flagged-unexpected`); SSH restart hints OpenRC-aware (guard `test_v0213_openrc_fix_wording.py`, mutation `locale/ssh-restart-hint-systemd-only`). **11053 → 11119**. |
| 0.21.2 | 11053 | **Patch — a FIFO at `/etc/ssh/sshd_config` no longer hangs the audit.** No scoring/schema/verdict change. The ssh snapshot probed the config with a raw `open("rb")` (to tell absent/unreadable/readable apart); `open` on a FIFO blocks forever, so `mkfifo /etc/ssh/sshd_config` hung the whole audit — samba FIFO class (v0.20.1) at a purpose-built open-probe the `read_text_capped` guard never reached. Now `stat`s first (never blocks) and fails closed on a non-regular file (unreadable, never parsed, no fallback to sshd defaults). Guard `test_v0212_sshd_config_no_hang.py` (SIGALRM watchdog raising a `BaseException` — a `TimeoutError` is an `OSError` the probe's `except OSError` would swallow), mutation `ssh/fifo-sshd-config-blocks-open`, killed. Found by an exhaustive Debian stress test (38 CLI cmds, hostile inputs × 22 config paths, detection-polarity) otherwise clean. **11048 → 11053**. |
| 0.21.1 | 11048 | **Patch — documentation accuracy, five backfilled `--explain` entries, and anti-drift guards; no scoring / JSON-schema / CSV-order / check-behaviour change.** **Five explains backfilled** (`hardening.icmp_broadcast_enabled`, `kernel_modules.kernels_obsolete`/`kernels_reboot_pending`/`kernels_update_available`, `mac_policy.apparmor_complain_profiles`) → explain 200→205 keys / 56 prefixes, `cis_refs` 204→205; the apparmor-complain one met in the field on real Debian trixie. **Documentation-accuracy pass**: SNAPSHOT *Numbers at a glance* frozen at v0.14.1 (domains 7→6, `_PREFIX_TO_DOMAIN` 36→57, filterable 38→47, LoC/version rows — value-second `| Metric | N |` layout no guard saw); README_DEV frozen ~v0.13.x (tree/table missing 9 modules — `raspberry_pi` + the 8 v0.21.0 checks — `schema_version` 1→3, `docker`→`container`, stale line/test/locale counts); CSV contract 12→16 cols; CLI table adds `--test-email`; CIS breakdown + ~30 LoC figures; last "7 domains" stragglers. **`report_markdown`** ufw version de-`v`'d. **Four new guards + a fixed relic test**, each mutation-tested: `test_v0211_live_count_claims.py` (value-second table + per-category CIS breakdown + locale total + check-module count; mutations `docs/glance-table-value-drifts`, `docs/module-count-stale-again`), `regen_loc.py` extended (hotspot table + tree LoC), and `test_get_public_ip_offline_skips_urllib` fixed (patched a non-existent `sysinfo.urllib` `raising=False` seam → real `urllib.request.urlopen`; mutation `sysinfo/offline-still-calls-urlopen`). Relic-test audit otherwise clean (0 dead skips, all mutations kill). **Field-tested on 8 real machines** (Debian/Ubuntu/Fedora/openSUSE/Alpine/Kali/Mint/Pi — desktop+server, systemd+OpenRC, SELinux+AppArmor, firewalld+nft, SSD+mechanical), 0 bug, results differentiated by context. Run time 9 s – 2 min 14 s (disk/CPU/services, not architecture). **10950 → 11048**. |
| 0.21.0 | 10950 | **Minor — six new check sections, and the SemVer "v" prefix dropped everywhere.** **SemVer**: version display (banner, `--version`, help header, JSON, webhook, report stamp, shields badge) and git tags now use the bare `0.21.0` — no `v`; the publish workflow accepts both tagless `[0-9]+.[0-9]+.[0-9]+` and legacy `v*`; formalized in `CONVENTIONS.md §8`. Guard `test_v0210_semver_no_v_prefix.py`, mutation `versioning/help-header-readds-v-prefix`, killed. **Six new checks**, each `Snapshot.from_system()` + pure `check_*`, profile-aware, non-dogmatic (unknown/unreadable ≠ clean; no auto-edit of PAM/fstab): **GRUB** — world/group-readable `grub.cfg` WARN (escalated on a readable `password_pbkdf2` hash), no boot password INFO (CIS 1.4.1); **CUPS** — non-loopback `Listen` WARN (`cupsctl --no-remote-any`), Browsing INFO; **mount hardening** — `nodev`/`nosuid` missing on a separate `/tmp`,`/var/tmp`,`/dev/shm` WARN, `noexec`-only missing INFO, non-separate INFO no penalty (CIS separate-partition trap avoided); **core dumps** (INFO) — `core_pattern` + systemd `Storage=` + `hard core 0`; **kexec/lockdown** (INFO) — `kexec_load_disabled` + `/sys/kernel/security/lockdown`; **PAM lockout** — no `pam_faillock`/`pam_tally2` in the auth stack WARN −1 on server, INFO on desktop/workstation (CIS 5.3.2). Explain set 197→198 keys / 53→54 prefixes, cis_refs 202→204. **disk encryption** (`disk_encryption`) — root filesystem on LUKS/dm-crypt via `/proc/mounts` + dm-stack walk (LVM-on-LUKS recognised); WARN on desktop/workstation plain root, INFO on server, N/A container, unknown when unresolved; best practice, `_MANUAL_BY_DESIGN` (FDE is destructive/one-way). **polkit** (`polkit`, access-control) — a non-root-writable `.rules`/`.pkla` (or `rules.d` dir) is a privesc path (polkit evaluates rules as root) → WARN + `chown/chmod` fix; legacy `.pkla` `ResultAny=yes` → INFO; JS rules deliberately not parsed for YES (noise). Guards `test_v0210_{grub_bootloader,cups_service,mount_hardening,core_dumps,kexec_lockdown,faillock,disk_encryption,polkit}.py`, eight mutations (`grub/loose-perms-not-flagged`, `cups/exposed-listen-not-flagged`, `mount_hardening/loose-options-not-flagged`, `core_dumps/storage-none-not-recognised-disabled`, `kexec_lockdown/locked-read-as-not-locked`, `faillock/server-lockout-gap-not-flagged`, `disk_encryption/plain-root-read-as-unknown`, `polkit/group-writable-rule-not-flagged`), all killed. Waves 1–3 of the v0.21.x check-growth arc. |
| v0.20.5 | 10762 | **Patch — two summary-accuracy fixes found on a real Kali desktop.** **SSH glance line**: `compute_exposure` built its one-line SSH verdict from bad-severity keys only, so on a LAN/desktop host (where `PasswordAuthentication yes` is INFO, not WARN) it fell through to "key-only, root login disabled" while password auth was enabled — the glance contradicting its own detailed finding (measured on real Kali). Now checks `all_keys` (includes INFO) → truthful "root login restricted, password auth on (OK for this context)"; genuine key-only keeps its line, "disabled" reworded to "restricted" (prohibit-password permits key root login). **Empty-group NOPASSWD**: Kali's `%kali-trusted ALL=(ALL:ALL) NOPASSWD: ALL` on an empty group read like a live grant; now resolves membership (secondary + primary GID) and flags it "latent — no members yet", keeping the WARN + deduction; an unresolvable group is never claimed empty. **Rolling-distro updates glance**: "security updates: up to date" showed green while 1335 updates pended — Kali (like Arch/Alpine) has no `-security` channel, so 0 classify as security and the glance read secure. `UpdatesSnapshot.security_channel_present` (False for pacman/apk, and apt when `apt-cache policy` has no `-security` suite; default True) now drives an `updates.no_security_channel` INFO and a "⚠ no security channel to classify" glance; a clean channel still reads "up to date" (Debian/Ubuntu path unchanged). Display-only, INFO. Rest of the Kali pass clean (AppArmor 0-profiles via kernel securityfs, desktop auto-detect, 1335 updates counted). Guard `test_v0205_kali_findings.py`, mutations `exposure/ssh-summary-ignores-info-password-auth` + `file_perms/nopasswd-empty-group-not-flagged` + `updates/no-security-channel-not-flagged`, killed. |
| v0.20.4 | 10735 | **Patch — the microcode check names the right package on openSUSE/zypper.** The microcode-currency check maps a vendor (`microcode-amd` / `microcode-intel`) to the package that ships it, per package manager; the zypper column was missing, so on a real openSUSE Leap 16 (AMD) `microcode_name_known` was False and the finding read "microcode package unknown / no package manager available" — misleading, since `detect_install_manager` returned `zypper` and package queries worked; the real gate was the absent name, which capped the domain at an upper bound of 8. The maps now carry `"zypper": "ucode-amd"` / `"zypper": "ucode-intel"`, so the rpm-backed probe finds the installed package and the check reports "✔ microcode installed", score 8 firm. A cross-distro sweep confirmed zypper was the only gap (apt `amd64-microcode`, dnf `amd-ucode-firmware` field-confirmed on AMD; apk/pacman standard). Found on a first real openSUSE Leap 16 pass (firewalld + SELinux enforcing + zypper, Python 3.13). Guard `test_v0204_opensuse_microcode.py`, mutation `microcode/opensuse-name-unmapped`, killed. **firewalld visibility**: the credited "zone allows …" line was built from `--list-services` + `--list-ports` only, so a port opened by an `accept` rich rule, a forward-port, or a zone's sources was invisible; `FirewalldStatus` now reads `--list-rich-rules` / `--list-forward-ports` / `--list-sources` and `allows_summary()` folds accept rich-rule ports (`5432/tcp (rich)`) and forward-ports into the line, deny rules never shown as allowances. Display-only (checks use firewalld as a boolean, no verdict moves). Field-validated on real Fedora 44 (5432/tcp rich rule + forward-port shown). Guard `test_v0204_firewalld_rich_rules.py` (14 tests), mutation `firewalld/rich-rule-port-omitted-from-allows`, killed. |
| v0.20.3 | 10709 | **Minor — Podman container hardening, snap-packaged service state, and desktop-profile auto-detection; three scope gaps closed, each field-validated on real hardware.** **Podman**: container hardening was Docker-only — a `podman run --privileged` container was invisible (measured on real Fedora 44). `DockerAuditSnapshot` now scans docker *or* podman via one shared inspect parser (Docker-compatible JSON), records the `runtime`, names it in the remediation cmd, accepts both sockets, and skips the Docker-only userns section for podman (`test_v0203_podman_container_audit.py`, mutation `podman/container-audit-is-docker-only`). Field-validated: privileged podman container flagged (−1). **Snap services**: `_detect_state` iterated deb unit names and reported a snap service stopped; measured on real Ubuntu 26.04 where the Nextcloud snap served 200 while BOB said "not running". Now resolves `detection.snap` via `snap services <pkg>` (`test_v0203_snap_service_state.py`, mutation `snap/service-state-ignores-snap-units`); field-validated: Nextcloud reads active, compare shows the inactive→active flip. **Desktop autodetect**: fallback profile was a hardcoded `server`, over-strict on graphical hosts; now `desktop` when a display-manager is active, explicit/saved profile always winning (`test_v0203_desktop_profile_autodetect.py`, mutation `profile/active-display-manager-not-read-as-desktop`). Field-validated on Fedora + Mint (desktop) and Ubuntu Server — the last **caught an over-detection** (`graphical.target` alone, no active DM, was read as desktop) that the field test found and the guard was corrected to reject. containerd/CRI remains a documented follow-up. 3 new mutations, all killed. |
| v0.20.2 | 10676 | **Minor — firewalld recognition (RPM/SUSE family), a banner naming every distro's firewall front-end + init manager, and seven blind spots from an exhaustive CLI pass on a real Fedora 44.** BOB was 100 % UFW-centric and declared a firewalld-protected Fedora "UFW not installed / wide open"; a shared `FirewalldStatus` (`firewall-cmd`) is now threaded into the firewall, iptables/nftables, firewall-stack, ipv6, services and ports checks — firewalld credited by zone, ACCEPT base policy explained, IPv6 read as firewalld-filtered, per-port advice reframed, its own nft table no longer "parallel to UFW". Risk ÉLEVÉ→MOYEN, score 6→7; credit conditional (firewalld stopped → HIGH; zone port reflected live) — both polarities field-validated, plus a `--fix --apply` sysctl round-trip on RPM. Banner now shows firewalld + Init (systemd/OpenRC/…). Blind spots: **cockpit socket-activation false negative** (static+inactive `cockpit.service`, active `cockpit.socket` on 9090 read as "not running" — v0.20.1 ssh-socket fix generalised to the inactive-service case, `test_v0202_socket_activated_static.py`); `--min-level` leaked directly-printed INFO/OK meta (now filtered at the print primitive); `--watch` dumped risk-context each cycle (now screen-silent under quiet) and block-buffered when piped (now line-buffered); `--output-dir` mkdir message localised; `--lang <unsupported>` now warns. `--explain` firewalld OK keys left by design. Install/uninstall/reinstall verified. Guards: `test_v0202_firewalld_recognized.py`, `test_v0202_blind_spots.py`, `test_v0202_socket_activated_static.py`; 11 new mutations, all killed. |
| v0.20.1 | 10603 | **Patch — six fixes harvested by stress-testing v0.20.0 on real hardware** (a Raspberry Pi, an exhaustive local hostile-`/etc` pass, and a real Debian 13 machine run under true root over ~25 angles including a physical reboot). **samba FIFO hang**: `_read_smb_conf` handed the path to `configparser.read` (`open().read()`, no guard), so a `mkfifo /etc/samba/smb.conf` made `--check samba` block forever — the exact hang `read_text_capped` was written to stop in v0.14.1; now routed through the capped reader (`test_v0201_samba_fifo_hang`, mutation `robustness/samba-conf-read-bypasses-fifo-guard`, directory as deterministic killer). **3 ANSI-injection holes** in the direct-render display closed — interface name, disk mountpoint/device, and `Deduction.reason` now pass the same sanitiser as `Finding.message` (`test_v0201_display_injection`, `injection/*` mutations). **Score span** now widens for a partially-blinded active domain, not only a wholly-blinded one (mutation `span/partial-blinding-not-widened`). **Fix-that-lies**: the world-writable systemd-timer `chmod` covered only the first five scripts; now every one (mutation `fix-lies/timer-chmod-caps-at-five`, round-tripped on real systemd). **False negative**: on a slow disk `apt-get -s dist-upgrade` (47 s, 468 pending) exceeded the 30 s timeout and its empty result was read as zero, hiding 322 pending security updates as "inconsistent state"; now runs via `run_result` with a 90 s timeout and falls back to `apt list --upgradable` (`test_v0201_apt_dist_upgrade_timeout`, mutation `updates/dist-upgrade-timeout-read-as-zero`). **Slow-disk budget**: the same 468-package upgrade took ~25 min via `--fix --apply`, past the 900 s package-apply cap, stopping it half-way; `_TIMEOUT_PACKAGE` raised to 3600 s (floor guard 300→1800, mutation `fixes/package-budget-back-to-thirty-seconds`). **False positive**: on Ubuntu 24.04+ ssh is socket-activated (ssh.service disabled, ssh.socket enabled) and BOB warned "active but will not restart automatically" — the service-state detector now reads an *enabled* trigger as restart-on-boot (`test_v0201_active_socket_service_restarts`, mutation `services/socket-activated-active-service-warns`). **UI**: `--manage-logs` hid the last report behind the footer — the report list is the one screen with two header rows and `body_h` reserved only one, so the scroll window was one taller than the rows drawn (render-driving guard `test_v0201_manage_logs_viewport`, mutation `tui/log-list-off-by-one-header`). **Flake**: the timeout grandchild-death check re-samples liveness before concluding. |
| v0.20.0 | 10539 | **BREAKING — per-domain scores realigned 1:1 with the six display groups.** The score domains were seven (`ssh`, `samba`, `file_perms`, `updates`, `hardening`, `disk`, `firewall`) and did not match the six on-screen groups: "Disk Health 10/10" showed while disk findings scrolled under HEALTH & RESILIENCE, and the detection tools had a group but no score domain. Domains are now exactly `firewall_network`/`exposure_services`/`access_control`/`system_hardening`/`health_resilience`/`detection`. New guard `test_v0200_domain_group_alignment` parses the runner and fails if a section is displayed under one group but scored under another; mutation `domain-alignment/detection-scored-as-hardening` proves it. BREAKING for JSON `domain_scores` consumers. **Also — two more fixes-that-lie closed** (ufw-IPv6 and umask): the `sed` matched an over-literal pattern, a no-op on `IPV6 = no`/`IPV6=NO`/`umask<tab>002`; now matches what detection matches, with a forge→fix→re-detect round-trip guard (`test_v0200_config_fix_matches_detection`) + 3 mutations. **UX**: the banner shows a distro logo (emoji) beside the OS name; `test_v0200_distro_logo` pins the mapping and the single-Wide-code-point invariant that keeps the box aligned (+1 mutation). |
| v0.19.0 | 10486 | **Remediation that applies on modern Include/drop-in configs — field-tested on a real Pi Zero W (Samba 4.22); `updates` made cross-distro on real VMs; trust-boundary symlink checks fail closed under Python 3.14 denial.** Validated against the service's own parser (`testparm`, `sshd -T`) + round-trip. **samba**: the SMB1/signing/map fixes appended without deleting; samba is last-wins so `testparm` kept the bad value, and BOB's first-wins parser then read the finding as *resolved* — a fix that lied. Now delete-then-insert. **ssh**: `PermitRootLogin`/`PasswordAuthentication`/`MaxAuthTries` edited the main file, but OpenSSH reads `sshd_config.d/*.conf` first (`50-cloud-init.conf` had `PasswordAuthentication yes`); fix now writes a first-winning `00-bob-hardening.conf`. **updates**: was apt-only (blind on 4/5 families — a Fedora host with 93 pending security updates read "no apt"); now detects dnf/zypper/pacman/apk and reads each through its own resolver, with a per-manager upgrade command and the −2 security deduction (pacman/apk have no security channel → regular INFO). Fixtures pinned to real VM output. **robustness**: `_is_safe_config_path`/`_is_safe_user_path` used `Path.is_symlink()`, which on 3.14 answers False to a denial → a symlink BOB can't lstat read as "safe"; now `strict_is_symlink` + fail-closed (undeterminable → unsafe), with a mode-000 denial polarity test. |
| v0.18.3 | 10406 | **`--manage-cron` reflows to width; pathlib-3.14 denial audit batches 1–4.** B1 `/etc/sudoers.d`, `/etc/ssh`, `/etc/cron.d`, `/var/log/journal`; B2 backup (denied config → "installed", not a crash); B3 `/etc/letsencrypt/live` + `/etc/ssl/private` (`glob()` swallows the read denial → `iterdir()`); B4 AIDE/Tripwire DB dir (a denied dir was a false WARN + −1 pt → `file_integrity.db_unknown`). A refused dir reads as not-established, and no longer *lowers* the score. |
...
| v0.17.1 | 9528 | **A real Fedora VM found what no container could**: a service BOB believed stopped removed its own port from the audit. Apache active, enabled, listening on `*:80` — no warning anywhere. The registry declared its unit as `apache2` (Fedora ships `httpd`), and the runner credited its port as "already handled by services" regardless of the verdict, so the ports section skipped it. Unit names measured with `dnf repoquery -l`; the port is credited now only when the service was judged active. **A real Debian 13 then found two more, both in `--fix --apply`**: the 30-second timeout killed `sudo` and nothing under it, so `apt-get upgrade` ran on orphaned while BOB reported "0 of 1 fix(es) applied" and exited, leaving packages unpacked but not configured — fixes run in their own session now, package transactions get minutes, and a command BOB stopped waiting on is *outcome unknown*, never *apply it yourself*. And the fix could not have applied a kernel update at all: detection uses `apt-get -s dist-upgrade` while the fix ran plain `upgrade`, which keeps new packages back, exits 0, and printed `✔ Applied` in six seconds with nothing installed. A fifth: the SSH staleness caveat fired on four milliseconds — `systemctl` answers in whole seconds, `st_mtime` does not, so an edit-then-reload done right printed "modified at 15:25, after … at 15:25" and muted every SSH finding below. Six real machines in all — Fedora, Debian 13, Kali, openSUSE Leap, Arch, Alpine — produced thirteen fix commits. openSUSE printed `User : unknown` where the kernel identifies the process, and `UFW : vN/A` beside two neighbours saying `not installed`. Arch ships no `hostname` binary (`Host : N/A`) and calls the unit `sshd`, which made every SSH fix command inert on Arch, openSUSE, Fedora and RHEL. Alpine, with no systemd, was told its running sshd was stopped — the undetermined state existed but was unreachable without systemctl. None of them is a v0.17.0 regression. |
| v0.17.0 | 9240 | **On the whole RPM family, BOB answered yes to every package it was asked about.** `rpm -q nosuchpackage` prints its error on stdout and exits 1; the query table counted any output as proof of installation, so a Fedora audit reported on services that were not installed (137 findings → 60; 76 of the 77 that went were `services.*` rows) and called microcode OK having checked nothing. Remediation commands now follow the host's manager, from names *measured in containers* — `sudo dnf install auditd` installs nothing. Two defects surfaced that a green suite on this Debian host could not see: `cmd=None` raised inside a check and fault isolation rendered three sections "unavailable", and the v0.16.4 non-interactive-flag guard went inert when its literals moved into a table. **BOB also read Debian's files to decide**: `/etc/pam.d/common-password` is Debian's name for the PAM password stack, so the password check deducted a point on Fedora, RHEL, openSUSE and Arch having read nothing — Fedora's `system-auth` carries `pam_pwquality.so` on line 13. Alpine, which has no PAM, now gets "could not establish" rather than a deduction. And a third hiding place for a Debian command was found: translated prose, where a message said apt while the same finding's cmd said dnf. **BOB had also never asked what machine it was on**: with no UEFI it announced "Legacy BIOS detected", which a Raspberry Pi does not have either. A Raspberry Pi section reports the Imager's `userconf.txt` — a password hash on a FAT partition that cannot protect it — the first-boot ssh marker, and the historical `pi` account when it can log in. No default-password test: `crypt` is gone from the standard library in Python 3.13. Field tested on emulated aarch64: the Bookworm `/boot/firmware` branch the x86 simulation could not reach, the yescrypt hash, and the full `--fix --apply` round trip. Running the whole suite there found five failures — four were the container running as root, and the fifth was real and not about ARM: **`setrlimit(RLIMIT_AS)` returns success and applies nothing under qemu-user**, so the plugin sandbox claimed a 256 MiB cap it never got. The limit is read back now; the adversarial-plugin test probes whether it took effect and skips with that reason rather than accusing the sandbox. With that done the whole suite exits 0 on aarch64 as an ordinary user. **The CI matrix then caught what none of that could**: Python 3.14 defaults `multiprocessing` away from `fork`, so a `Process(target=…)` whose target is a nested function raises `PicklingError` — two probes written for this release did exactly that, passed on 3.10 through 3.13, and failed on 3.14. BOB's sandbox was never affected (it asks for `spawn` explicitly and targets a module-level function); the probes now do the same, and a guard sweeps the whole suite for the shape. An **arm64 CI job** now runs on every PR (`ci/arm64_smoke.sh`, driven from `integration.yml` with `docker/setup-qemu-action`): it refuses to pass unless `uname -m` really answers `aarch64` — a green job that never ran on ARM is worse than no job — then asserts the audit exits cleanly with no sentinels or tracebacks, that microcode degrades, that absent UEFI is not asserted to be a BIOS, and that the Raspberry Pi section fires on a simulated board. Both negative controls were verified locally: forcing `--platform linux/amd64` fails on the arch guard, and re-introducing `Legacy BIOS detected` fails on the claim guard. The full suite is deliberately not run there — roughly fifteen times slower under emulation, and the architecture-sensitive part is asserted directly. **The cron wizard also promised delivery it had never established.** *"sendmail available — notifications will be delivered"* was decided by `shutil.which("sendmail")`, which a Postfix installed and never configured satisfies while dropping every message — and the case where the promise fails is the case nobody sees. `bob --test-email` sends a real message through the same transport the cron script uses and reports what sendmail answered — **field tested end to end on the maintainer's host**: sendmail accepted, and the message arrived with its subject and body intact after crossing a real Postfix, a relay and a provider's spam filter, which is the difference between accepted and delivered that this command exists to draw; a job with no address, whose only channel was that email, now hears that it will notify nobody. Nine mutations, two of which exist because a guard was inert: the flush-ordering test asserted only that stdout was non-empty, which the interpreter guarantees at exit either way, and an AST guard passed while the screen it guarded raised `TypeError` on sight — found by a pty run of the real wizard, and replaced by one that renders it. |
| v0.16.4 | 8951 | **"This guard bites" was a claim in a commit message, and nothing re-checked it.** The 26 mutations behind v0.16.3's guards now live in `tests/mutations.py` and `scripts/mutate.py` replays them in CI, with the bench stating its own controls on every run: the guards must pass unmutated, and a change that cannot matter must survive. |
| v0.16.3 | 8572 | **A hint line, a profile note and a reference manual — three claims about BOB, none checked against BOB.** Eight passes, opening with five wizards and five key contracts (2026-09-07). Each interactive screen invented its own bindings and spelled its hints out beside the dispatch; the line was translated in `--explain` only, so a French operator read French content under English key hints — the class v0.15.3 closed for `--help`. One floor everywhere now (`↑↓`/`jk`, `PgUp`/`PgDn`, `g`/`G`, `⏎`), each screen declaring its actions once with the footer and the dispatch both derived from that. `q` exits from a first page only; six nested screens lost theirs. A language switch (`l`) on every landing page, because these wizards run under `sudo` and sudo resets `$LANG`. Six guards, each mutation-tested; two were inert when first written. |
| v0.16.2 | 8328 | **A ceiling claims a direction, and blindness does not always have one (2026-09-04).** v0.16.0's ceiling holds while the blinded check's domain is still scored; when the whole domain leaves the average the *denominator* changes and the score moves unpredictably — masking `/etc/passwd` drops `file_perms` (10/10) and takes the score from 7 **down** to 6, so `≤ 6` claimed a ceiling below the truth while the domain read `no action needed`. Three states now, with the span (`between 5 and 7`) derived from the [0,10] domain range rather than guessed. The `--target` gate moved from `score_is_upper_bound` to `score_is_uncertain`, or making the bound honest would have opened it; its guard, pinned to a source literal for three releases running, is behavioural. `--target` also gained `target`/`target_met` in JSON after leaving no machine trace at all. The cron script stopped exporting `PYTHONPATH` unconditionally — a wizard run from a git checkout had been sending uncommitted edits to the nightly audit. Four smaller ones from the option sweep: silent `--webhook` under `--offline`, `--ignore` accepting a key that matches nothing, a false reason on the `--quiet --fix --apply` refusal, and a domain cap announced as a score cap. |
| v0.16.1 | 8282 | **v0.16.0 closed one half of its own class, and the other half was live (2026-09-04).** Found by an exhaustive local sweep of every option and combination (~700 invocations). A domain whose input goes unreadable leaves the score *average* entirely, so masking `/etc/passwd` takes `file_perms` (10/10) out and the score from 7 down to **6** with the deductions byte-identical — read against a sighted baseline as `Score degraded by 1 point(s)`, a regression on a host where only BOB's eyesight changed. Three defects, one root: `user_accounts.no_passwd` / `no_shadow` sat outside `VISIBILITY_KEYS` because the set was enumerated **by name**; `load_baseline` never restored `unverified`, so **the entire `--diff` visibility protection was dead outside the unit tests**, which never round-trip through the file; and no branch existed for a score that *falls* on a blinder run. Also `--target` printed `✔ target reached` while the same run exited 4, and the exit-code table — a documented stable public API — still said "score < N" in README, README_FR, SNAPSHOT and the man page. `--install-cron` gained three closed screens (profile / language / network) after a scheduled audit was found running as root with root's profile; the interactive wizard itself had crashed since **v0.7.0**, 40 released versions, because a pipe is not a TTY and every test drove the plain-text path. Curses selection recoloured orange — it shared the banner's cyan on all four screens. |
| v0.16.0 | 8152 | **The score went up when BOB could see less (2026-09-04).** Masking `/etc/ssh/sshd_config` removes four deductions and moves the score from 7 to **8**, with `degraded_sections` empty by contract. The number now renders as the ceiling it is (`≤ 8/10`), the risk level reads as a best case, the trend arrow is withheld while bounded, and a Visibility line counts the sections not fully read. `logs.brute_found` renamed to `blocked_repeat_public` with three back-compat surfaces, each its own test. One truth, six renderers — the archived report said `7/10` where the terminal said `≤ 7/10`; a guard now requires every sink to declare itself, and it immediately found `--target` satisfiable by a ceiling. `bob -q -d` had written a 440-line report with no summary block. `--format=html` crashed with the whole suite green. |
| v0.15.5 | 8089 | **Five hunting angles on one question: is what BOB says verifiable? (2026-09-03).** Non-breaking — under privilege the audit is byte-identical before and after. The thread: an absence of answer collapsed into an assertion, seven instances, all found with the same instrument — the blind audit in three variants (tool removed, tool present but refusing, file masked by `mount --bind /etc/shadow`). Yield measured across the campaign: 7 → 2 → 1 → 1 → 1 → 0 defects per angle, with rising documented rejection rates. |
| v0.15.4 | 7958 | **The backlog, emptied in one pass (2026-09-02).** 🔴 **BREAKING on a patch number, deliberately** — see the versioning note in the changelog. Seven backlog items and the three teeth v0.14.0 had deferred: container `privileged` −3 / `CAP_SYS_ADMIN` −2 / seccomp-off −1, field-tested against four real podman containers (a default container still scores zero); a non-loopback orphan socket −1; world-readable cloud user-data −2; and nftables scoring parity, which turned out to be a **false** deduction — `iifname` unread, penalising a correctly hardened host. IMDS-without-IMDSv2 **dropped, not deferred**: enforcement cannot be read host-side, so no bench would have unblocked it. Also `config_key` retired, `--help` release titles fixed, samba's `writeable`/`directory` synonyms, ollama's systemd port, and four permanent guards (no parsed option without a reader, no orphan public function, no unexplainable WARN, no unguarded system binary in a test). |
| v0.15.3 | 7743 | **Four defects aged twelve to fourteen versions (2026-09-02).** `-r/--reconfigure` was parsed and dropped since v0.1.0 — advertised in `--help` and twice in the man page while nothing read it. `--help` returned byte-identical English under `--french`, on the one screen that advertises `--french`; 83 keys per locale now. `--diff` printed **nothing at all** since v0.3.0: `_silent_mode` muted it and only `--breakdown` re-enabled output. And a single `Feb 29` log line took out the whole UFW section, the year rollback sitting outside the guard that caught the construction. |
| v0.15.2 | 7560 | **"I could not look" stops sounding like "nothing to see" (2026-08-31).** An unreadable file answered like an empty one across the audit path, and `dpkg-query` alone had made every service read as NOT INSTALLED on four of the five claimed distributions. Closed at the root: `run_result` carries success, `path_exists` degrades instead of raising, and `package_installed` walks dpkg/rpm/pacman/apk. |
| v0.15.1 | 7288 | **A second hunt, to test the first one's result (2026-08-31).** New angles only: cross-checking overlapping checks against each other, which no module-by-module audit can see. Carries the documentation pass v0.15.0 deferred. |
...
**Test host:** Linux Mint 22.3 — `so6minttest`
