...
| Version | Tests | Notes |
|---------|-------|-------|
| v0.17.0 | 9016 | **The cron wizard promised delivery it had never established.** *"sendmail available — notifications will be delivered"* was decided by `shutil.which("sendmail")`, which a Postfix installed and never configured satisfies while dropping every message — and the case where the promise fails is the case nobody sees. `bob --test-email` sends a real message through the same transport the cron script uses and reports what sendmail answered; a job with no address, whose only channel was that email, now hears that it will notify nobody. Nine mutations, two of which exist because a guard was inert: the flush-ordering test asserted only that stdout was non-empty, which the interpreter guarantees at exit either way, and an AST guard passed while the screen it guarded raised `TypeError` on sight — found by a pty run of the real wizard, and replaced by one that renders it. |
