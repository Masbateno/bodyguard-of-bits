...
| `explain.py` | `--explain KEY` — `normalize_key()`, `run_explain()`, 191-key canonical list in 50 prefixes, profile variants (71 keys × 3 profiles), CIS reference lookup via `cis_refs.py` |
...
├── explain.py           # run_explain(), normalize_key(), EXPLAIN_KEYS — 191 keys in 50 prefixes
...
The file contains exactly 2399 keys organised into sections (verified with `bob/locales/en.json` vs `fr.json` strict-parity test). Translate all values while keeping `{variable}` placeholders intact.
