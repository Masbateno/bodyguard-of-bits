...
| `explain.py` | `--explain KEY` — `normalize_key()`, `run_explain()`, 184-key canonical list in 47 prefixes, profile variants (70 keys × 3 profiles), CIS reference lookup via `cis_refs.py` |
...
├── explain.py           # run_explain(), normalize_key(), EXPLAIN_KEYS — 184 keys in 47 prefixes
...
The file contains exactly 2200 keys organised into sections (verified with `bob/locales/en.json` vs `fr.json` strict-parity test). Translate all values while keeping `{variable}` placeholders intact.
