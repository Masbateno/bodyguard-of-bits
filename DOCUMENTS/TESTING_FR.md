...
| Version | Tests | Notes |
|---------|-------|-------|
| v0.17.1 | 9270 | **Une VM Fedora réelle a trouvé ce qu'aucun conteneur ne pouvait** : un service que BOB croyait arrêté retirait son propre port de l'audit. Apache actif, activé, en écoute sur `*:80` — aucun avertissement. Le registre déclarait son unité `apache2` (Fedora livre `httpd`), et le runner créditait son port comme « déjà traité par services » quel que soit le verdict, si bien que la section ports le sautait. Noms d'unités mesurés avec `dnf repoquery -l` ; le port n'est crédité que si le service a été jugé actif. Aucun des deux défauts n'est une régression de la v0.17.0. |
