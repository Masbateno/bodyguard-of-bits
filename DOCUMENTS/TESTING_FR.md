...
- **Historique des tests unitaires** (table par version, plus sections détaillées pour les premières releases) — chaque release liste les tests ajoutés, supprimés ou corrigés, avec la plateforme et le compteur de tests de l'époque. C'est la trace d'audit de la croissance de la suite, de v0.1.0 (4200 tests) à la release courante (plus de 10 900). Les releases contract-preserving (comme les splits `ssh.py` / `cron.py` de v0.6.0 et le sunset UFW_AUDIT_SHARE) ajoutent zéro delta test ; les campagnes deep-audit et field-test sur vrai matériel expliquent l'essentiel de la croissance.
...
| Version | Tests | Notes |
|---------|-------|-------|
| 0.21.3 | 11119 | **Correctif — BOB lit les vrais chemins de config de la distribution, pas seulement ceux de Debian : `/usr/etc`, doas, précédence SELinux.** Quatre angles morts de mécanisme distro issus de la campagne terrain v0.21.2 (openSUSE Leap 16, Alpine 3.24 fraîche), chacun ne changeant la sortie que sur le layout concerné. **(1)** audit SSH aveugle sur `/usr/etc` (MEDIUM-HIGH) — openSUSE place `sshd_config` sous `/usr/etc`, BOB ne lisait que `/etc`, ne suivait jamais l'`Include` des drop-ins, rapportait les défauts OpenSSH, masquant un vrai `PermitRootLogin yes` ; retombe désormais sur `/usr/etc/ssh/sshd_config` (garde `test_v0213_sshd_config_usr_etc_fallback.py`, mutation `ssh/usr-etc-sshd-config-not-read`). **(2)** doas audité (MEDIUM) — `permit nopass` dans `/etc/doas.conf` (root sans mot de passe Alpine/OpenBSD, équivalent `NOPASSWD:ALL`) était invisible ; désormais WARN −2 avec `--explain file_perms.doas_nopass_all` (garde `test_v0213_doas_audit.py`, mutation `file_perms/doas-nopass-not-flagged`). **(3)** SELinux permissive plus mal diagnostiqué en AppArmor-off (MEDIUM) — la branche AppArmor-off cède à SELinux quand il est le MAC installé (garde `test_v0213_mac_policy_selinux_precedence.py`, mutation `mac_policy/apparmor-off-steals-selinux-permissive`). **(4)** fallback `/usr/etc` login.defs + sudoers (LOW) (garde `test_v0213_usr_etc_login_defs_sudoers.py`, mutations `password_policy/login-defs-usr-etc-not-read`, `file_perms/sudoers-usr-etc-vendor-not-read`). Explain 205→206, refs CIS 205→206, locale 2595→2606. Le score ne change que sur les layouts concernés. **Trois findings de moindre sévérité aussi corrigés** : banner firewalld stoppé désormais « installed (inactive) » (garde `test_v0213_firewalld_banner_presence.py`, mutation `sysinfo/firewalld-stopped-reads-not-installed`) ; doas/bbsuid reconnus SUID standard (garde `test_v0213_suid_doas_bbsuid.py`, mutation `suid/doas-flagged-unexpected`) ; conseils restart SSH OpenRC-aware (garde `test_v0213_openrc_fix_wording.py`, mutation `locale/ssh-restart-hint-systemd-only`). **11053 → 11119**. |
| 0.21.2 | 11053 | **Correctif — un FIFO sur `/etc/ssh/sshd_config` ne fait plus hanger l'audit.** Aucun changement de score/schéma/verdict. Le snapshot ssh sondait la config par un `open("rb")` brut (distinguer absent/illisible/lisible) ; `open` sur un FIFO bloque pour toujours, donc `mkfifo /etc/ssh/sshd_config` faisait hanger tout l'audit — classe FIFO samba (v0.20.1) à un probe-open dédié que la garde `read_text_capped` n'atteignait pas. Fait désormais un `stat` d'abord (ne bloque jamais) et échoue fermé sur un non-régulier (illisible, jamais parsé, pas de fallback aux défauts sshd). Garde `test_v0212_sshd_config_no_hang.py` (watchdog SIGALRM levant une `BaseException` — un `TimeoutError` est un `OSError` que le `except OSError` du probe avalerait), mutation `ssh/fifo-sshd-config-blocks-open`, tuée. Trouvé par un stress test Debian exhaustif (38 cmds CLI, entrées hostiles × 22 chemins, polarité détection) par ailleurs propre. **11048 → 11053**. |
| 0.21.1 | 11048 | **Correctif — justesse documentation, cinq entrées `--explain` complétées, gardes anti-dérive ; aucun changement de score / schéma JSON / ordre CSV / comportement des checks.** **Cinq explains complétés** (`hardening.icmp_broadcast_enabled`, `kernel_modules.kernels_obsolete`/`kernels_reboot_pending`/`kernels_update_available`, `mac_policy.apparmor_complain_profiles`) → explain 200→205 clés / 56 préfixes, `cis_refs` 204→205 ; celui d'apparmor-complain rencontré sur le terrain sur un vrai Debian trixie. **Passe de justesse doc** : SNAPSHOT *Numbers at a glance* gelée à v0.14.1 (domaines 7→6, `_PREFIX_TO_DOMAIN` 36→57, filtrables 38→47, lignes LoC/version — layout `| Métrique | N |` qu'aucune garde ne voyait) ; README_DEV gelé ~v0.13.x (arbre/table sans 9 modules — `raspberry_pi` + les 8 checks v0.21.0 — `schema_version` 1→3, `docker`→`container`, comptes lignes/tests/locale périmés) ; contrat CSV 12→16 col ; table CLI + `--test-email` ; breakdown CIS + ~30 LoC ; derniers « 7 domaines ». **`report_markdown`** version ufw dé-`v`. **Quatre nouvelles gardes + un test relique corrigé**, chacune mutation-testée : `test_v0211_live_count_claims.py` (table nombre-en-second + breakdown CIS par catégorie + total locale + compte modules ; mutations `docs/glance-table-value-drifts`, `docs/module-count-stale-again`), `regen_loc.py` étendu (table hotspot + LoC arbre), et `test_get_public_ip_offline_skips_urllib` corrigé (patchait un seam `sysinfo.urllib` `raising=False` inexistant → vrai `urllib.request.urlopen` ; mutation `sysinfo/offline-still-calls-urlopen`). Audit tests reliques sinon propre (0 skip mort, toutes les mutations tuent). **Field-testé sur 8 machines réelles** (Debian/Ubuntu/Fedora/openSUSE/Alpine/Kali/Mint/Pi — desktop+serveur, systemd+OpenRC, SELinux+AppArmor, firewalld+nft, SSD+mécanique), 0 bug, résultats différenciés selon le contexte. Runtime 9 s – 2 min 14 s (disque/CPU/services, pas l'architecture). **10950 → 11048**. |
| 0.21.0 | 10950 | **Mineure — six nouvelles sections de vérification, et le préfixe « v » de SemVer abandonné partout.** **SemVer** : l'affichage de version (bandeau, `--version`, en-tête d'aide, JSON, webhook, tampon de rapport, badge shields) et les tags git utilisent désormais `0.21.0` nu — sans `v` ; le workflow de publication accepte le sans-`v` `[0-9]+.[0-9]+.[0-9]+` et l'ancien `v*` ; formalisé dans `CONVENTIONS.md §8`. Garde `test_v0210_semver_no_v_prefix.py`, mutation `versioning/help-header-readds-v-prefix`, tuée. **Six nouveaux checks**, chacun `Snapshot.from_system()` + `check_*` pur, sensible au profil, non-dogmatique (inconnu/illisible ≠ propre ; pas d'auto-édition de PAM/fstab) : **GRUB** — `grub.cfg` lisible monde/groupe WARN (escaladé sur un hash `password_pbkdf2` lisible), pas de mot de passe de boot INFO (CIS 1.4.1) ; **CUPS** — `Listen` non-loopback WARN (`cupsctl --no-remote-any`), Browsing INFO ; **durcissement montages** — `nodev`/`nosuid` absents sur un `/tmp`,`/var/tmp`,`/dev/shm` séparé WARN, `noexec`-seul absent INFO, non-séparé INFO sans pénalité (piège CIS partition séparée évité) ; **core dumps** (INFO) — `core_pattern` + `Storage=` systemd + `hard core 0` ; **kexec/lockdown** (INFO) — `kexec_load_disabled` + `/sys/kernel/security/lockdown` ; **verrouillage PAM** — pas de `pam_faillock`/`pam_tally2` dans la pile auth WARN −1 sur serveur, INFO sur desktop/workstation (CIS 5.3.2). Ensemble explain 197→198 clés / 53→54 préfixes, cis_refs 202→204. **chiffrement du disque** (`disk_encryption`) — racine sur LUKS/dm-crypt via `/proc/mounts` + parcours de la pile dm (LVM-sur-LUKS reconnu) ; WARN sur desktop/workstation à racine nue, INFO sur serveur, N/A conteneur, inconnu si non résolu ; bonne pratique, `_MANUAL_BY_DESIGN` (le FDE est destructif/à sens unique). **polkit** (`polkit`, contrôle-d'accès) — un `.rules`/`.pkla` (ou répertoire `rules.d`) modifiable par non-root est un chemin de privesc (polkit évalue les règles en tant que root) → WARN + correctif `chown/chmod` ; `.pkla` legacy `ResultAny=yes` → INFO ; le JS des règles délibérément non parsé pour YES (bruit). Gardes `test_v0210_{grub_bootloader,cups_service,mount_hardening,core_dumps,kexec_lockdown,faillock,disk_encryption,polkit}.py`, huit mutations (`grub/loose-perms-not-flagged`, `cups/exposed-listen-not-flagged`, `mount_hardening/loose-options-not-flagged`, `core_dumps/storage-none-not-recognised-disabled`, `kexec_lockdown/locked-read-as-not-locked`, `faillock/server-lockout-gap-not-flagged`, `disk_encryption/plain-root-read-as-unknown`, `polkit/group-writable-rule-not-flagged`), toutes tuées. Vagues 1–3 de l'arc de croissance des checks v0.21.x. |
| v0.20.5 | 10762 | **Correctif — deux corrections de justesse trouvées sur une Kali réelle.** **Ligne ssh du résumé** : `compute_exposure` bâtissait son verdict ssh sur les clés de sévérité bad seulement, si bien que sur un hôte LAN/desktop (où `PasswordAuthentication yes` est INFO, pas WARN) il retombait sur « clé uniquement, connexion root désactivée » alors que password auth était actif — le résumé contredisant son propre constat détaillé (mesuré sur Kali réelle). Consulte désormais `all_keys` (inclut INFO) → « connexion root restreinte, auth par mot de passe active (OK pour ce contexte) » ; le vrai clé-uniquement garde sa ligne, « désactivée » → « restreinte » (prohibit-password autorise le root par clé). **NOPASSWD groupe vide** : le `%kali-trusted ALL=(ALL:ALL) NOPASSWD: ALL` de Kali sur groupe vide ressemblait à un grant live ; résout maintenant l'appartenance (secondaires + GID primaire) et le signale « latent — aucun membre », en gardant le WARN + déduction ; un groupe non résolvable n'est jamais déclaré vide. **Glance updates rolling** : « mises à jour sécurité : à jour » en vert alors que 1335 updates en attente — Kali (comme Arch/Alpine) n'a pas de canal `-security`, donc 0 classé sécurité et le glance lisait sécurisé. `UpdatesSnapshot.security_channel_present` (False pour pacman/apk, et apt quand `apt-cache policy` sans suite `-security` ; défaut True) pilote maintenant un INFO `updates.no_security_channel` et un glance « ⚠ pas de canal sécurité pour classer » ; un canal propre lit toujours « à jour » (chemin Debian/Ubuntu inchangé). Affichage seulement, INFO. Reste de la passe propre (AppArmor 0-profil via securityfs kernel, desktop auto-détecté, 1335 updates comptés). Guard `test_v0205_kali_findings.py`, mutations `exposure/ssh-summary-ignores-info-password-auth` + `file_perms/nopasswd-empty-group-not-flagged` + `updates/no-security-channel-not-flagged`, tuées. |
| v0.20.4 | 10735 | **Correctif — le check microcode nomme le bon paquet sur openSUSE/zypper.** Le check d'actualité du microcode mappe un vendeur (`microcode-amd` / `microcode-intel`) au paquet qui le fournit, par gestionnaire de paquets ; la colonne zypper manquait, donc sur une vraie openSUSE Leap 16 (AMD) `microcode_name_known` valait False et le constat se lisait « paquet microcode inconnu / aucun gestionnaire de paquets disponible » — trompeur, car `detect_install_manager` renvoyait `zypper` et les requêtes de paquets marchaient ; le vrai verrou était le nom absent, qui plafonnait le domaine à une borne haute de 8. Les maps portent maintenant `"zypper": "ucode-amd"` / `"zypper": "ucode-intel"`, si bien que la sonde adossée à rpm trouve le paquet installé et le check rapporte « ✔ microcode installé », score 8 ferme. Un balayage cross-distro a confirmé que zypper était le seul trou (apt `amd64-microcode`, dnf `amd-ucode-firmware` field-confirmés sur AMD ; apk/pacman standards). Trouvé lors d'une première passe openSUSE Leap 16 réelle (firewalld + SELinux enforcing + zypper, Python 3.13). Guard `test_v0204_opensuse_microcode.py`, mutation `microcode/opensuse-name-unmapped`, tuée. **Visibilité firewalld** : la ligne créditée « la zone autorise … » était bâtie sur `--list-services` + `--list-ports` seulement, si bien qu'un port ouvert par une rich rule `accept`, un forward-port ou les sources d'une zone était invisible ; `FirewalldStatus` lit désormais `--list-rich-rules` / `--list-forward-ports` / `--list-sources` et `allows_summary()` intègre les ports de rich rule `accept` (`5432/tcp (rich)`) et forward-ports à la ligne, les règles de déni jamais montrées comme autorisations. Affichage seulement (les checks utilisent firewalld comme booléen, aucun verdict ne bouge). Field-validé sur Fedora 44 réelle (rich rule 5432/tcp + forward-port affichés). Guard `test_v0204_firewalld_rich_rules.py` (14 tests), mutation `firewalld/rich-rule-port-omitted-from-allows`, tuée. |
| v0.20.3 | 10709 | **Mineure — durcissement conteneurs Podman, état des services packagés en snap, et auto-détection du profil desktop ; trois trous de scope fermés, chacun field-validé sur matériel réel.** **Podman** : le durcissement conteneurs était Docker-only — un conteneur `podman run --privileged` était invisible (mesuré sur Fedora 44 réelle). `DockerAuditSnapshot` scanne désormais docker *ou* podman via un parser `inspect` partagé (JSON compatible Docker), mémorise le `runtime`, le nomme dans la cmd de remédiation, accepte les deux sockets, et saute la section userns (Docker-only) pour podman (`test_v0203_podman_container_audit.py`, mutation `podman/container-audit-is-docker-only`). Field-validé : conteneur podman privilégié flaggé (−1). **Services snap** : `_detect_state` itérait les noms d'unités deb et disait un service snap arrêté ; mesuré sur Ubuntu 26.04 réelle où le snap Nextcloud servait en 200 pendant que BOB disait « ne tourne pas ». Résout maintenant `detection.snap` via `snap services <pkg>` (`test_v0203_snap_service_state.py`, mutation `snap/service-state-ignores-snap-units`) ; field-validé : Nextcloud lu actif, le compare montre le flip inactif→actif. **Auto-détection desktop** : le profil de repli était un `server` en dur, trop strict sur les hôtes graphiques ; désormais `desktop` quand un display-manager est actif, un profil explicite/sauvé gagnant toujours (`test_v0203_desktop_profile_autodetect.py`, mutation `profile/active-display-manager-not-read-as-desktop`). Field-validé sur Fedora + Mint (desktop) et Ubuntu Server — ce dernier a **attrapé une sur-détection** (`graphical.target` seul, sans DM actif, lu comme desktop) que le field-test a trouvée et le guard a été corrigé pour la rejeter. containerd/CRI reste une suite documentée. 3 nouvelles mutations, toutes tuées. |
| v0.20.2 | 10676 | **Mineure — reconnaissance de firewalld (famille RPM/SUSE), un banner nommant le front-end pare-feu + le gestionnaire d'init de chaque distro, et sept angles morts d'une passe CLI exhaustive sur une vraie Fedora 44.** BOB était 100 % UFW-centrique et déclarait une Fedora protégée par firewalld « UFW pas installé / grand-ouvert » ; un `FirewalldStatus` partagé (`firewall-cmd`) est câblé dans les checks firewall, iptables/nftables, firewall-stack, ipv6, services et ports — firewalld crédité par zone, politique de base ACCEPT expliquée, IPv6 lu comme filtré par firewalld, conseil par port reformulé, sa propre table nft plus « en parallèle d'UFW ». Risque ÉLEVÉ→MOYEN, score 6→7 ; crédit conditionnel (firewalld arrêté → ÉLEVÉ ; port de zone reflété en direct) — deux polarités field-validées, plus un roundtrip `--fix --apply` sysctl sur RPM. Banner affiche firewalld + Init (systemd/OpenRC/…). Angles morts : **faux négatif cockpit socket-activé** (`cockpit.service` static+inactive, `cockpit.socket` active sur 9090 lu « ne tourne pas » — fix ssh-socket v0.20.1 généralisé au cas service-inactif, `test_v0202_socket_activated_static.py`) ; `--min-level` laissait fuir les INFO/OK méta imprimées en direct (filtrées à la primitive) ; `--watch` déversait les blocs contexte-de-risque à chaque cycle (silencieux à l'écran sous quiet) et bloc-bufferisait une fois pipé (line-buffered) ; message `--output-dir` localisé ; `--lang <inconnue>` avertit désormais. Clés OK firewalld de `--explain` laissées par design. Install/désinstall/réinstall vérifiés. Guards : `test_v0202_firewalld_recognized.py`, `test_v0202_blind_spots.py`, `test_v0202_socket_activated_static.py` ; 11 nouvelles mutations, toutes tuées. |
| v0.20.1 | 10603 | **Correctif — six fixes récoltés en stress-testant v0.20.0 sur du matériel réel** (un Raspberry Pi, une passe locale exhaustive à `/etc` hostile, et une vraie machine Debian 13 en vrai root sur ~25 angles dont un reboot physique). **blocage FIFO samba** : `_read_smb_conf` passait le chemin à `configparser.read` (`open().read()`, sans garde), donc un `mkfifo /etc/samba/smb.conf` faisait pendre `--check samba` indéfiniment — le blocage exact que `read_text_capped` devait empêcher en v0.14.1 ; désormais routé par le lecteur borné (`test_v0201_samba_fifo_hang`, mutation `robustness/samba-conf-read-bypasses-fifo-guard`, répertoire comme tueur déterministe). **3 failles d'injection ANSI** dans l'affichage à rendu direct fermées — nom d'interface, point de montage/périphérique disque et `Deduction.reason` passent le même assainisseur que `Finding.message` (`test_v0201_display_injection`, mutations `injection/*`). **Span de score** s'élargit maintenant pour un domaine actif partiellement aveuglé, pas seulement entièrement (mutation `span/partial-blinding-not-widened`). **Fix-qui-ment** : le `chmod` des scripts de timer systemd inscriptibles par tous ne couvrait que les cinq premiers ; désormais chacun (mutation `fix-lies/timer-chmod-caps-at-five`, roundtrip sur vrai systemd). **Faux négatif** : sur disque lent `apt-get -s dist-upgrade` (47 s, 468 en attente) dépassait le timeout de 30 s et son résultat vide était lu comme zéro, masquant 322 updates de sécurité en « état incohérent » ; passe désormais par `run_result` avec timeout 90 s + fallback `apt list --upgradable` (`test_v0201_apt_dist_upgrade_timeout`, mutation `updates/dist-upgrade-timeout-read-as-zero`). **Budget disque lent** : le même upgrade de 468 paquets a pris ~25 min via `--fix --apply`, au-delà du cap de 900 s, le stoppant à mi-chemin ; `_TIMEOUT_PACKAGE` porté à 3600 s (garde plancher 300→1800, mutation `fixes/package-budget-back-to-thirty-seconds`). **Faux positif** : sur Ubuntu 24.04+ ssh est socket-activé (ssh.service désactivé, ssh.socket activé) et BOB avertissait « actif mais ne redémarrera pas automatiquement » — le détecteur d'état lit désormais un trigger *activé* comme redémarrage-au-boot (`test_v0201_active_socket_service_restarts`, mutation `services/socket-activated-active-service-warns`). **UI** : `--manage-logs` cachait la dernière entrée derrière le pied de page — la liste des rapports est le seul écran à deux lignes d'en-tête et `body_h` n'en réservait qu'une, donc la fenêtre de scroll dépassait d'une ligne les lignes dessinées (garde pilotant le rendu `test_v0201_manage_logs_viewport`, mutation `tui/log-list-off-by-one-header`). **Flake** : le contrôle de mort du petit-fils ré-échantillonne la vivacité avant de conclure. |
| v0.20.0 | 10539 | **BREAKING — scores par domaine réalignés 1:1 sur les six groupes affichés.** Les domaines de score étaient sept (`ssh`, `samba`, `file_perms`, `updates`, `hardening`, `disk`, `firewall`) et ne coïncidaient pas avec les six groupes affichés : « Santé des disques 10/10 » s'affichait alors que les constats disque défilaient sous SANTÉ & RÉSILIENCE, et les outils de détection avaient un groupe mais aucun domaine de score. Les domaines sont maintenant exactement `firewall_network`/`exposure_services`/`access_control`/`system_hardening`/`health_resilience`/`detection`. Nouvelle garde `test_v0200_domain_group_alignment` : lit le runner et échoue si une section est affichée sous un groupe mais scorée sous un autre ; mutation `domain-alignment/detection-scored-as-hardening` le prouve. BREAKING pour les consommateurs JSON de `domain_scores`. **Aussi — deux fixes-qui-mentent de plus fermés** (ufw-IPv6 et umask) : le `sed` matchait un motif trop littéral, no-op sur `IPV6 = no`/`IPV6=NO`/`umask<tab>002` ; matche désormais ce que la détection matche, avec une garde round-trip forge→fix→re-détecte (`test_v0200_config_fix_matches_detection`) + 3 mutations. **UX** : le banner affiche un logo de distro (emoji) à côté du nom de l'OS ; `test_v0200_distro_logo` épingle le mapping et l'invariant mono-code-point-Wide qui garde le cadre aligné (+1 mutation). |
| v0.19.0 | 10486 | **Une remédiation qui s'applique sur les configs modernes Include/drop-in — field-testée sur un vrai Pi Zero W (Samba 4.22) ; `updates` rendu cross-distro sur des VMs réelles ; les vérifs symlink de frontière de confiance échouent fermé sous refus Python 3.14.** Validée contre le parseur natif du service (`testparm`, `sshd -T`) + round-trip. **samba** : les fixes SMB1/signature/map ajoutaient sans supprimer ; samba est dernière-gagne, `testparm` gardait la mauvaise valeur, et le parseur première-gagne de BOB lisait alors le constat *résolu* — un fix qui ment. Désormais supprime-puis-insère. **ssh** : `PermitRootLogin`/`PasswordAuthentication`/`MaxAuthTries` éditaient le main, mais OpenSSH lit `sshd_config.d/*.conf` en premier (`50-cloud-init.conf` avait `PasswordAuthentication yes`) ; le fix écrit désormais un `00-bob-hardening.conf` gagnant. **updates** : était apt-only (aveugle sur 4/5 familles — un hôte Fedora avec 93 MàJ de sécurité en attente lisait « no apt ») ; détecte désormais dnf/zypper/pacman/apk et lit chacun via son propre résolveur, avec une commande de mise à jour par gestionnaire et la déduction −2 sécurité (pacman/apk sans canal sécurité → INFO régulier). Fixtures épinglées sur la sortie VM réelle. **robustesse** : `_is_safe_config_path`/`_is_safe_user_path` utilisaient `Path.is_symlink()`, qui sous 3.14 répond False à un refus → un symlink que BOB ne peut lstat se lisait « sûr » ; désormais `strict_is_symlink` + fail-closed (indéterminable → non sûr), avec un test de polarité sur refus mode-000. |
| v0.18.3 | 10406 | **`--manage-cron` se recompose à la largeur ; audit pathlib-3.14 lots 1–4.** L1 `/etc/sudoers.d`, `/etc/ssh`, `/etc/cron.d`, `/var/log/journal` ; L2 backup (config refusée → « installé », pas un crash) ; L3 `/etc/letsencrypt/live` + `/etc/ssl/private` (`glob()` avale le refus de lecture → `iterdir()`) ; L4 base AIDE/Tripwire (un répertoire refusé était un faux WARN à −1 pt → `file_integrity.db_unknown`). Un répertoire refusé se lit non-établi, et ne *baisse* plus le score. |
| v0.18.2 | 10321 | **`--install-completion` affichait des clés de locale entre crochets** — sa branche dans `__main__` sautait `i18n.init()` ; trouvé en validant BOB sur le Pi Zero W. Guide Pi étoffé (venv, invocation). |
| v0.18.1 | 10314 | **La détection de force brute SSH était aveugle sous OpenSSH ≥ 9.8** — trouvé sur un vrai Raspberry Pi Zero W. |
| v0.18.0 | 9959 | **BOB lit OpenRC.** Tout service d'un hôte sans systemd revenait UNKNOWN — honnête et inutile, puisque Alpine, Gentoo et Devuan tournent sous OpenRC. Le contrat est mesuré sur Alpine Linux 3.22 : `rc-service status` sort en 0 démarré, 3 arrêté et 1 pour un service inconnu, tandis que l'existence est tranchée par `/etc/init.d/<nom>` — une sonde qui n'a pas pu tourner n'est donc jamais lue comme un démon arrêté, l'erreur que la v0.17.1 a dû défaire quatre fois. `CommandResult` porte désormais le code de sortie, car `ok` confondait « sorti en 3 » et « n'a pas pu démarrer ». Les commandes de remédiation suivent le système d'init et non son orthographe : `rc-service sshd restart`, et un `enable` en deux verbes. Vérifié en exécutant la commande résolue sur les deux types d'hôte. **BREAKING** — UNKNOWN devient un vrai état sur les hôtes OpenRC : verdicts, scores et références y bougent ; rien ne change sous systemd. |
| v0.17.1 | 9528 | **Une VM Fedora réelle a trouvé ce qu'aucun conteneur ne pouvait** : un service que BOB croyait arrêté retirait son propre port de l'audit. Apache actif, activé, en écoute sur `*:80` — aucun avertissement. Le registre déclarait son unité `apache2` (Fedora livre `httpd`), et le runner créditait son port comme « déjà traité par services » quel que soit le verdict, si bien que la section ports le sautait. Noms d'unités mesurés avec `dnf repoquery -l` ; le port n'est crédité que si le service a été jugé actif. **Une Debian 13 réelle en a ensuite trouvé deux autres, tous deux dans `--fix --apply`** : le délai de 30 s tuait `sudo` et rien en dessous, si bien qu'`apt-get upgrade` continuait orphelin pendant que BOB annonçait « 0 correction sur 1 appliquée » et sortait, laissant des paquets dépaquetés mais non configurés — les corrections tournent désormais dans leur propre session, les transactions de paquets reçoivent des minutes, et une commande que BOB a cessé d'attendre est un *résultat inconnu*, jamais un *à faire vous-même*. Et la correction n'aurait pas pu appliquer une mise à jour noyau : la détection utilise `apt-get -s dist-upgrade` là où la correction lançait `upgrade` simple, qui garde en arrière les nouveaux paquets, sort en 0, et affichait `✔ Appliqué` en six secondes sans rien installer. Un cinquième : la réserve de fraîcheur SSH se déclenchait sur quatre millisecondes — `systemctl` répond à la seconde entière, `st_mtime` non, si bien qu'une modification suivie d'un rechargement, faite correctement, affichait « modifié à 15:25, après … à 15:25 » et muselait tous les constats SSH en dessous. Six machines réelles au total — Fedora, Debian 13, Kali, openSUSE Leap, Arch, Alpine — ont produit treize commits de correction. openSUSE affichait `User : unknown` là où le noyau identifie le processus, et `UFW : vN/A` à côté de deux voisins disant `not installed`. Arch ne livre pas de binaire `hostname` (`Host : N/A`) et nomme l'unité `sshd`, ce qui rendait toutes les commandes de correction SSH inertes sur Arch, openSUSE, Fedora et RHEL. Alpine, sans systemd, s'entendait dire que son sshd en marche était arrêté — l'état indéterminé existait mais était inatteignable sans systemctl. Aucun n'est une régression de la v0.17.0. |
| v0.17.0 | 9240 | **Sur toute la famille RPM, BOB répondait oui à tout paquet soumis.** `rpm -q paquet-inexistant` écrit son erreur sur stdout et sort en 1 ; la table comptait toute sortie comme preuve d'installation, si bien qu'un audit Fedora rendait compte de services non installés (137 findings → 60 ; 76 des 77 disparus étaient des lignes `services.*`) et déclarait le microcode OK sans rien vérifier. Les commandes de remédiation suivent désormais le gestionnaire de l'hôte, depuis des noms *mesurés en conteneur* — `sudo dnf install auditd` n'installe rien. Deux défauts qu'une suite verte sur cet hôte Debian ne pouvait pas voir : `cmd=None` levait dans un check et l'isolation de fautes rendait trois sections « unavailable », et la garde v0.16.4 sur le drapeau non interactif est devenue inerte quand ses littéraux ont emménagé dans une table. **BOB lisait aussi les fichiers de Debian pour décider** : `/etc/pam.d/common-password` est le nom Debian de la pile PAM « password », le check déduisait donc un point sur Fedora, RHEL, openSUSE et Arch sans avoir rien lu — le `system-auth` de Fedora porte `pam_pwquality.so` en ligne 13. Alpine, sans PAM, reçoit désormais « non établi » plutôt qu'une déduction. Et une troisième cachette pour une commande Debian a été trouvée : la prose traduite, où un message disait apt pendant que le cmd du même finding disait dnf. **BOB n'avait pas non plus demandé sur quelle machine il tournait** : sans UEFI il annonçait « Legacy BIOS detected », qu'un Raspberry Pi n'a pas davantage. Une section Raspberry Pi rapporte le `userconf.txt` de l'Imager — un hash de mot de passe sur une partition FAT incapable de le protéger — le marqueur ssh de premier démarrage, et le compte `pi` historique quand il peut se connecter. Pas de test du mot de passe par défaut : `crypt` a disparu de la bibliothèque standard en Python 3.13. Testé sur aarch64 émulé : la branche `/boot/firmware` de Bookworm que la simulation x86 ne pouvait pas atteindre, le hash yescrypt, et le round trip complet `--fix --apply`. Passer toute la suite là-bas a produit cinq échecs — quatre venaient du conteneur en root, le cinquième était réel et ne concernait pas ARM : **`setrlimit(RLIMIT_AS)` retourne un succès sans rien appliquer sous qemu-user**, le bac à sable revendiquait donc un plafond de 256 Mio qu'il n'avait jamais obtenu. La limite est désormais relue ; le test des plugins adverses sonde si elle a pris effet et passe en skip avec cette raison au lieu d'accuser le bac à sable. Cela fait, la suite complète sort en 0 sur aarch64 en utilisateur ordinaire. **La matrice CI a ensuite attrapé ce que rien de tout cela ne pouvait voir** : Python 3.14 change le défaut de `multiprocessing` et abandonne `fork`, si bien qu'un `Process(target=…)` visant une fonction imbriquée lève `PicklingError` — deux sondes écrites pour cette version faisaient exactement cela, passaient de 3.10 à 3.13 et échouaient en 3.14. Le bac à sable de BOB n'a jamais été concerné (il demande `spawn` explicitement et vise une fonction de module) ; les sondes font désormais pareil, et une garde balaie toute la suite à la recherche de cette forme. Un **job CI arm64** tourne désormais sur chaque PR (`ci/arm64_smoke.sh`, piloté depuis `integration.yml` via `docker/setup-qemu-action`) : il refuse de passer si `uname -m` ne répond pas réellement `aarch64` — un job vert qui n'a jamais tourné sur ARM est pire que pas de job — puis vérifie que l'audit sort proprement sans sentinelle ni traceback, que le microcode dégrade, qu'un UEFI absent n'est pas affirmé être un BIOS, et que la section Raspberry Pi se déclenche sur une carte simulée. Les deux contrôles négatifs ont été vérifiés localement : forcer `--platform linux/amd64` échoue sur la garde d'architecture, et réintroduire `Legacy BIOS detected` échoue sur la garde d'affirmation. La suite complète n'y est délibérément pas lancée — environ quinze fois plus lente sous émulation, et la partie sensible à l'architecture est vérifiée directement. **Le wizard cron promettait par ailleurs une remise qu'il n'avait jamais établie.** *« sendmail disponible — les notifications seront envoyées »* était décidé par `shutil.which("sendmail")`, auquel un Postfix installé et jamais configuré satisfait tout en jetant chaque message — et le cas où la promesse échoue est celui que personne ne voit. `bob --test-email` envoie un vrai message par le transport même du script cron et rapporte ce que sendmail a répondu — **éprouvé de bout en bout sur la machine du mainteneur** : sendmail a accepté, et le message est arrivé avec son sujet et son corps intacts après avoir traversé un vrai Postfix, un relais et le filtre anti-spam d'un fournisseur, ce qui est précisément la différence entre accepté et délivré que cette commande existe pour tracer ; une tâche sans adresse, dont le seul canal était cet email, s'entend désormais dire qu'elle ne préviendra personne. Neuf mutations, dont deux existent parce qu'une garde était inerte : le test d'ordre du flush n'affirmait que la non-vacuité de stdout, ce que l'interpréteur garantit à la sortie dans les deux cas, et une garde d'AST passait pendant que l'écran gardé levait `TypeError` à vue — trouvée par une exécution du vrai wizard derrière un pty, et remplacée par une garde qui le rend. |
| v0.16.4 | 8951 | **« Cette garde mord » était une affirmation dans un message de commit, que rien ne revérifiait.** Les 26 mutations derrière les gardes de la v0.16.3 vivent désormais dans `tests/mutations.py` et `scripts/mutate.py` les rejoue en CI, le banc énonçant ses propres contrôles à chaque exécution : les gardes doivent passer non mutées, et un changement qui ne peut rien changer doit survivre. |
| v0.16.3 | 8572 | **Une ligne d'aide, une note de profil, un manuel de référence — trois affirmations sur BOB, aucune vérifiée contre BOB.** Huit passes, ouvertes par cinq assistants et cinq contrats de touches (07-09-2026). Chaque écran interactif inventait ses liaisons et écrivait ses indications à la main ; la ligne n'était traduite que dans `--explain`, si bien qu'un opérateur français lisait du contenu français sous des indications anglaises — la classe fermée par la v0.15.3 pour `--help`. Un socle unique désormais (`↑↓`/`jk`, `PgUp`/`PgDn`, `g`/`G`, `⏎`), chaque écran déclarant ses actions une fois, pied de page et répartiteur en dérivant. `q` ne quitte que depuis une première page ; six écrans imbriqués ont perdu le leur. Une bascule de langue (`l`) sur chaque page de garde, parce que ces assistants tournent sous `sudo` qui réinitialise `$LANG`. Six gardes, testées par mutation ; deux étaient inertes à la première écriture. |
| v0.16.2 | 8328 | **Un plafond affirme une direction, et l'aveuglement n'en a pas toujours une (04-09-2026).** Le plafond de la v0.16.0 tient tant que le domaine du check aveuglé est encore noté ; quand le domaine entier quitte la moyenne, c'est le *dénominateur* qui change et le score bouge imprévisiblement — masquer `/etc/passwd` retire `file_perms` (10/10) et fait passer le score de 7 à **6**, si bien que `≤ 6` affirmait un plafond sous la vérité pendant que le domaine affichait `no action needed`. Trois états désormais, avec l'intervalle (`entre 5 et 7`) déduit de l'échelle [0,10] des domaines plutôt que supposé. Le portail `--target` est passé de `score_is_upper_bound` à `score_is_uncertain`, sans quoi rendre le bornage honnête l'aurait ouvert ; sa garde, épinglée sur un littéral trois versions durant, est devenue comportementale. `--target` gagne aussi `target`/`target_met` en JSON après n'avoir laissé aucune trace machine. Le script cron a cessé d'exporter `PYTHONPATH` sans condition — un assistant lancé depuis un dépôt git envoyait des modifications non commitées à l'audit nocturne. Quatre plus petits du balayage d'options : `--webhook` muet sous `--offline`, `--ignore` acceptant une clé qui ne correspond à rien, une raison fausse au refus de `--quiet --fix --apply`, et un plafond de domaine annoncé comme un plafond de score. |
| v0.16.1 | 8282 | **La v0.16.0 n'avait fermé qu'une moitié de sa propre classe, et l'autre était vivante (04-09-2026).** Trouvé par un balayage local exhaustif de toutes les options et combinaisons (~700 invocations). Un domaine dont l'entrée devient illisible quitte entièrement la *moyenne* du score : masquer `/etc/passwd` retire `file_perms` (10/10) et fait passer le score de 7 à **6** avec des déductions identiques au point près — lu face à une baseline voyante comme `Score dégradé de 1 point(s)`, une régression sur un hôte où seule la vue de BOB a changé. Trois défauts, une racine : `user_accounts.no_passwd` / `no_shadow` étaient hors de `VISIBILITY_KEYS` parce que l'ensemble avait été énuméré **par nom** ; `load_baseline` ne restaurait jamais `unverified`, donc **toute la protection de visibilité de `--diff` était morte hors des tests unitaires**, qui ne passent jamais par le fichier ; et aucune branche n'existait pour un score qui *baisse* sur une exécution plus aveugle. Par ailleurs `--target` imprimait `✔ objectif atteint` alors que la même exécution sortait en 4, et la table des codes de sortie — API publique stable documentée — disait encore « score < N » dans README, README_FR, SNAPSHOT et la page de manuel. `--install-cron` gagne trois écrans fermés (profil / langue / réseau) après qu'un audit programmé s'est révélé tourner en root avec le profil de root ; l'assistant interactif lui-même plantait depuis la **v0.7.0**, 40 versions publiées, parce qu'un tuyau n'est pas un terminal et que tous les tests pilotaient le chemin texte. Sélection curses passée en orange — elle partageait le cyan du bandeau sur les quatre écrans. |
| v0.16.0 | 8152 | **Le score montait quand BOB voyait moins (04-09-2026).** Masquer `/etc/ssh/sshd_config` retire quatre déductions et fait passer le score de 7 à **8**, `degraded_sections` restant vide par contrat. Le nombre est désormais rendu comme le plafond qu'il est (`≤ 8/10`), le niveau de risque se lit comme un meilleur cas, la flèche de tendance est retenue tant qu'il est borné, et une ligne Visibilité compte les sections non entièrement lues. `logs.brute_found` renommée `blocked_repeat_public`, avec trois surfaces de compatibilité, chacune son test. Une vérité, six rendus — le rapport archivé disait `7/10` là où le terminal disait `≤ 7/10` ; une garde impose désormais à toute sortie de se déclarer, et elle a immédiatement trouvé `--target` satisfiable par un plafond. `bob -q -d` écrivait un rapport de 440 lignes sans bloc de synthèse. `--format=html` plantait avec toute la suite verte. |
| v0.15.5 | 8089 | **Cinq angles de chasse sur une seule question : ce que BOB affirme est-il vérifiable ? (03-09-2026).** Non cassant — sous privilège l'audit est identique octet pour octet avant et après. Le fil : une absence de réponse collapsée en affirmation, sept instances, toutes trouvées avec le même instrument — l'audit aveugle en trois variantes (outil retiré, outil présent mais refusant, fichier masqué par `mount --bind /etc/shadow`). Rendement mesuré sur la campagne : 7 → 2 → 1 → 1 → 1 → 0 défauts par angle, avec des taux de rejet documentés croissants. |
| v0.15.4 | 7958 | **Le backlog, vidé d'un seul coup (02-09-2026).** 🔴 **BREAKING sur un numéro de patch, délibérément** — voir la note de versionnage du changelog. Sept items de backlog et les trois dents que la v0.14.0 avait reportées : conteneur `privileged` −3 / `CAP_SYS_ADMIN` −2 / seccomp désactivé −1, field-testées contre quatre conteneurs podman réels (un conteneur par défaut ne perd toujours rien) ; socket orpheline non-loopback −1 ; user-data cloud lisible par tous −2 ; et la parité de scoring nftables, qui s'est révélée être une **fausse** déduction — `iifname` non lu, pénalisant un hôte correctement durci. IMDS-sans-IMDSv2 **retiré, non reporté** : l'application ne se lit pas depuis l'hôte, aucun banc ne l'aurait débloqué. Aussi : `config_key` retiré, titres des releases corrigés, synonymes samba `writeable`/`directory`, port systemd d'ollama, et quatre gardes permanentes (aucune option analysée sans lecteur, aucune fonction publique orpheline, aucun WARN inexplicable, aucun binaire système non gardé dans un test). |
| v0.15.3 | 7743 | **Quatre défauts vieux de douze à quatorze versions (02-09-2026).** `-r/--reconfigure` était analysé puis jeté depuis la v0.1.0 — annoncé dans `--help` et deux fois dans le manuel alors que rien ne le lisait. `--help` rendait un anglais identique à l'octet sous `--french`, sur le seul écran qui annonce `--french` ; 83 clés par locale désormais. `--diff` n'affichait **rien du tout** depuis la v0.3.0 : `_silent_mode` le rendait muet et seul `--breakdown` réactivait la sortie. Et une seule ligne de journal « Feb 29 » emportait toute la section UFW, le repli d'année se trouvant hors du garde qui protégeait la construction. |
| v0.15.2 | 7560 | **« Je n'ai pas pu regarder » ne sonne plus comme « rien à voir » (31-08-2026).** Un fichier illisible répondait comme un fichier vide sur tout le chemin d'audit, et `dpkg-query` seul faisait lire chaque service comme NON INSTALLÉ sur quatre des cinq distributions annoncées. Fermé à la racine : `run_result` porte le succès, `path_exists` dégrade au lieu de lever, et `package_installed` parcourt dpkg/rpm/pacman/apk. |
| v0.15.1 | 7288 | **Une seconde chasse, pour éprouver le résultat de la première (31-08-2026).** Angles neufs uniquement : croisement des contrôles qui se recouvrent, ce qu'aucun audit module par module ne peut voir. Porte la passe documentaire reportée de v0.15.0. |
| v0.15.0 | 7220 | **Exactitude des verdicts (31-08-2026).** Tests différentiels contre l'analyseur qui fait autorité (`sshd -T`, `ssh -G`, `cvtsudoers -f json`) au lieu d'attentes écrites à la main. Une classe de défaut domine : les motifs écrits sur la forme *propre* d'une ligne cessaient de correspondre dès qu'un opérateur ajoutait un commentaire final — une règle UFW `allow from anywhere` commentée n'était pas détectée, et `NOPASSWD: ALL # temporaire` était rétrogradé de WARN −2 à un INFO sans score. Chaque correctif est testé par mutation : le défaut est réinjecté et la garde doit échouer. |
...
**Hôte de test :** Linux Mint 22.3 — `so6minttest`
