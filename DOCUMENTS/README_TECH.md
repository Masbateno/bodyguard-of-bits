...
- **`--explain KEY`** — structured per-finding explanation (WHY IT IS A RISK / HOW TO FIX / CIS reference); 189 explainable keys across 50 prefixes; 103 of them render a section per profile — 71 with prose written for it, the rest with a note derived from the profile file — and 86 apply equally to every profile; interactive TUI; no root required; `--explain list` shows all keys
