...
- **`--explain KEY`** — structured per-finding explanation (WHY IT IS A RISK / HOW TO FIX / CIS reference); 186 explainable keys across 49 prefixes; 70 keys show profile-specific sections; interactive TUI; no root required; `--explain list` shows all keys
...
As of v0.11.x, the `--explain` key set contains **186 keys** across **49 prefixes**. The canonical naming convention is enforced by `tests/test_explain_naming_convention.py`:
