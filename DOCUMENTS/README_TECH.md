...
![Release](https://img.shields.io/badge/version-v0.17.1-brightgreen)
...
- **CIS compliance mapping inline** — each finding shows its CIS code `[CIS:X.Y.Z]` in the summary box; full reference text in `--verbose` mode; 194 entries (108 formal CIS, 79 best-practice, 7 Docker)
...
- **`--explain KEY`** — structured per-finding explanation (WHY IT IS A RISK / HOW TO FIX / CIS reference); 189 explainable keys across 50 prefixes; 105 of them render a section per profile — 71 with prose written for it, the rest with a note derived from the profile file — and 84 apply equally to every profile; interactive TUI; no root required; `--explain list` shows all keys
...
║                           — Bodyguard Of Bits —                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  BOB v0.17.1  │  Linux hardening auditor                                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
As of v0.11.x, the `--explain` key set contains **189 keys** across **50 prefixes**. The canonical naming convention is enforced by `tests/test_explain_naming_convention.py`:
