...
- **CIS compliance mapping inline** — each finding shows its CIS code `[CIS:X.Y.Z]` in the summary box; full reference text in `--verbose` mode; 196 entries (109 formal CIS, 80 best-practice, 7 Docker)
...
- **Services panorama** — compact table of all 38 known services after the audit: SERVICE / STATUS / PORT(S) / UFW; non-installed services shown dimmed. States come from systemd, or — since v0.18.0 — from OpenRC on hosts without it (Alpine, Gentoo, Devuan), where `rc-service status` exit codes are read and existence is settled by `/etc/init.d/<name>`, so a probe that could not run is never read as a stopped daemon. A service **enabled but not running** is reported and scored like its mirror, *running but not enabled*
...
- **SUID/SGID audit** — root-owned SUID and SGID binaries outside the expected set (whitelistable in `config.conf`) → WARN −1 pt. Since v0.18.0 each one is also **attributed to its package** (`dpkg -S`, `rpm -qf`, `pacman -Qo`, `apk info --who-owns`): a root-owned SUID binary that **no package ships** is its own finding → ALERT −2 pts — the shape a left-behind privilege escalation takes. A query that answers about itself rather than the file (a locked database, a missing manager) produces no finding, so it never becomes an accusation
...
- **`--explain KEY`** — structured per-finding explanation (WHY IT IS A RISK / HOW TO FIX / CIS reference); 191 explainable keys across 50 prefixes; 106 of them render a section per profile — 71 with prose written for it, the rest with a note derived from the profile file — and 85 apply equally to every profile; interactive TUI; no root required; `--explain list` shows all keys
...
║                           — Bodyguard Of Bits —                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  BOB v0.18.0  │  Linux hardening auditor                                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
| `duration_seconds` | float \| null | How long the audit took, in seconds, rounded to milliseconds. Measured from the moment the checks start — not from process entry, so it excludes argument parsing and the root check — with a monotonic clock, so a clock adjustment mid-run cannot make it negative. `null` when the run did not time itself. New in v0.17.1, additive within v3. |
...
As of v0.11.x, the `--explain` key set contains **191 keys** across **50 prefixes**. The canonical naming convention is enforced by `tests/test_explain_naming_convention.py`:
