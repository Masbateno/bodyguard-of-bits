...
| `explain.py` | `--explain KEY` — `normalize_key()`, `run_explain()`, 194 clés canoniques dans 50 préfixes, variantes par profil (71 clés × 3 profils), lookup référence CIS via `cis_refs.py` |
...
├── explain.py           # run_explain(), normalize_key(), EXPLAIN_KEYS — 194 clés dans 50 préfixes
...
Si le service peut écouter sur un port configurable, nommer son fichier de configuration — BOB l'analyse dès qu'il est déclaré :
...
Le fichier contient exactement 2429 clés organisées en sections (vérifié par le test de stricte parité `bob/locales/en.json` vs `fr.json`). Traduire toutes les valeurs en conservant les placeholders `{variable}` intacts.
...
Déplacées dans **[CONVENTIONS_FR.md](CONVENTIONS_FR.md)** en v0.16.4, où
elles rejoignent la charte graphique — ce que veut dire une couleur, ce que
veut dire un symbole — qui n'avait jamais été écrite. Un document répond
« comment fait-on ici » ; celui-ci reste sur l'architecture.
