...
| `explain.py` | `--explain KEY` — `normalize_key()`, `run_explain()`, 186 clés canoniques dans 49 préfixes, variantes par profil (70 clés × 3 profils), lookup référence CIS via `cis_refs.py` |
...
├── explain.py           # run_explain(), normalize_key(), EXPLAIN_KEYS — 186 clés dans 49 préfixes
...
Le fichier contient exactement 2212 clés organisées en sections (vérifié par le test de stricte parité `bob/locales/en.json` vs `fr.json`). Traduire toutes les valeurs en conservant les placeholders `{variable}` intacts.
