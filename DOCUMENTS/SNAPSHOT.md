...
│                 8747 unit tests · 19 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
├── tests/                     ← 227 test files, ~5757 functions, 8747 collected (v0.16.3)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~8747-test suite running with no mocks.
...
| Tests | 227 test files, ~5757 functions, **8747 collected** (v0.16.3) | `pytest --collect-only -q` |
