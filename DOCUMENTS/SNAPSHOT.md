...
│                 9894 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
DATA (read-only at runtime, shipped in the package)
    ...
    bob/data/cis_refs.json        ← 196 CIS references
...
EXTERNAL CONTRACTS (frozen, do not break without major bump)
    ...
    EXPLAIN_KEYS                    ← 191 keys / 50 prefixes; alias map emptied v0.9.0 D-3, first live alias v0.10.1 (ssh.x11)
...
│   │   ├── cis_refs.json      ← 196 CIS references
...
│   ├── explain.py             ← --explain TUI, EXPLAIN_KEYS (191 keys, 50 prefixes), v0.8.0 backfill of 51 WARN/ALERT gaps + v0.10.1 `ssh.x11.forwarding.client`; `EXPLAIN_KEY_ALIASES` dict emptied (v0.9.0 D-3 — drift fix at source) then first live entry v0.10.1 (ssh.x11 D-4 Rank 1 split migration path)
...
├── tests/                     ← 261 test files, ~6150 functions, 9894 collected (v0.18.0)
...
| 1017 | `bob/explain.py` | EXPLAIN_KEYS (191 keys / 50 prefixes after v0.8.0 backfill + v0.10.1 client x11) + alias map (emptied v0.9.0 D-3, first live entry v0.10.1) + interactive TUI |
...
### 3. EXPLAIN_KEYS (191 keys, 50 prefixes)
...
| `test_cis_refs.py` | 196 CIS references mapping |
...
| Tests | 261 test files, ~6150 functions, **9894 collected** (v0.18.0) | `pytest --collect-only -q` |
...
| EXPLAIN_KEYS | 191 (in 50 prefixes) | `bob.explain.EXPLAIN_KEYS` |
