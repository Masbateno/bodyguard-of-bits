...
│                 9754 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
├── tests/                     ← 256 test files, ~6112 functions, 9754 collected (v0.17.1)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9754-test suite running with no mocks.
...
| Tests | 256 test files, ~6112 functions, **9754 collected** (v0.17.1) | `pytest --collect-only -q` |
