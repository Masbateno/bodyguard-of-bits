...
│                 9132 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
DATA (read-only at runtime, shipped in the package)
    ...
    bob/data/cis_refs.json        ← 192 CIS references
...
│   │   ├── cis_refs.json      ← 192 CIS references
...
├── tests/                     ← 236 test files, ~5882 functions, 9132 collected (v0.17.0)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9132-test suite running with no mocks.
...
| `test_cis_refs.py` | 192 CIS references mapping |
...
| Tests | 236 test files, ~5882 functions, **9132 collected** (v0.17.0) | `pytest --collect-only -q` |
