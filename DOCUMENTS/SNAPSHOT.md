...
│                 9781 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
├── tests/                     ← 258 test files, ~6128 functions, 9806 collected (v0.18.0)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9781-test suite running with no mocks.
...
| Tests | 258 test files, ~6128 functions, **9806 collected** (v0.18.0) | `pytest --collect-only -q` |
