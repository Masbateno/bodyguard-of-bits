...
│                 9670 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
DATA (read-only at runtime, shipped in the package)
    ...
    bob/data/cis_refs.json        ← 195 CIS references
...
EXTERNAL CONTRACTS (frozen, do not break without major bump)
    ...
    EXPLAIN_KEYS                    ← 190 keys / 50 prefixes; alias map emptied v0.9.0 D-3, first live alias v0.10.1 (ssh.x11)
...
│   │   ├── cis_refs.json      ← 195 CIS references
...
│   ├── explain.py             ← --explain TUI, EXPLAIN_KEYS (190 keys, 50 prefixes), v0.8.0 backfill of 51 WARN/ALERT gaps + v0.10.1 `ssh.x11.forwarding.client`; `EXPLAIN_KEY_ALIASES` dict emptied (v0.9.0 D-3 — drift fix at source) then first live entry v0.10.1 (ssh.x11 D-4 Rank 1 split migration path)
...
├── tests/                     ← 253 test files, ~6085 functions, 9670 collected (v0.17.1)
...
| `explain.py` | 1017 | `--explain` TUI + `EXPLAIN_KEYS` (190 keys, 50 prefixes) + `EXPLAIN_KEY_ALIASES` map (emptied v0.9.0 D-3; **first live entry v0.10.1** mapping the old `ssh.x11_forwarding` umbrella to the D-4 Rank 1 server/client split). v0.8.0 drift batch added 51 entries to cover WARN/ALERT findings previously emitted without --explain content; v0.10.1 added `ssh.x11.forwarding.client`. |
...
| 1017 | `bob/explain.py` | EXPLAIN_KEYS (190 keys / 50 prefixes after v0.8.0 backfill + v0.10.1 client x11) + alias map (emptied v0.9.0 D-3, first live entry v0.10.1) + interactive TUI |
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9670-test suite running with no mocks.
...
### 3. EXPLAIN_KEYS (190 keys, 50 prefixes)
...
| `test_cis_refs.py` | 195 CIS references mapping |
...
| Tests | 253 test files, ~6085 functions, **9670 collected** (v0.17.1) | `pytest --collect-only -q` |
...
| EXPLAIN_KEYS | 190 (in 50 prefixes) | `bob.explain.EXPLAIN_KEYS` |
