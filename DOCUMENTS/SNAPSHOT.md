...
│                 9098 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
├── tests/                     ← 235 test files, ~5868 functions, 9098 collected (v0.17.0)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9098-test suite running with no mocks.
...
| Tests | 235 test files, ~5868 functions, **9098 collected** (v0.17.0) | `pytest --collect-only -q` |
