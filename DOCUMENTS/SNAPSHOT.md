...
│                 7926 unit tests · 19 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
DATA (read-only at runtime, shipped in the package)
    ...
    bob/data/cis_refs.json        ← 189 CIS references
...
EXTERNAL CONTRACTS (frozen, do not break without major bump)
    ...
    EXPLAIN_KEYS                    ← 184 keys / 47 prefixes; alias map emptied v0.9.0 D-3, first live alias v0.10.1 (ssh.x11)
...
│   │   ├── cis_refs.json      ← 189 CIS references
...
│   ├── explain.py             ← --explain TUI, EXPLAIN_KEYS (184 keys, 47 prefixes), v0.8.0 backfill of 51 WARN/ALERT gaps + v0.10.1 `ssh.x11.forwarding.client`; `EXPLAIN_KEY_ALIASES` dict emptied (v0.9.0 D-3 — drift fix at source) then first live entry v0.10.1 (ssh.x11 D-4 Rank 1 split migration path)
...
├── tests/                     ← 191 test files, ~5365 functions, 7926 collected (v0.15.3)
...
| `explain.py` | 1017 | `--explain` TUI + `EXPLAIN_KEYS` (184 keys, 47 prefixes) + `EXPLAIN_KEY_ALIASES` map (emptied v0.9.0 D-3; **first live entry v0.10.1** mapping the old `ssh.x11_forwarding` umbrella to the D-4 Rank 1 server/client split). v0.8.0 drift batch added 51 entries to cover WARN/ALERT findings previously emitted without --explain content; v0.10.1 added `ssh.x11.forwarding.client`. |
...
| 1017 | `bob/explain.py` | EXPLAIN_KEYS (184 keys / 47 prefixes after v0.8.0 backfill + v0.10.1 client x11) + alias map (emptied v0.9.0 D-3, first live entry v0.10.1) + interactive TUI |
...
### 3. EXPLAIN_KEYS (184 keys, 47 prefixes)
...
| `test_cis_refs.py` | 189 CIS references mapping |
...
- **Snapshot + check_xxx separation**. Enables ~7926 tests with no mocks.
...
| Tests | 191 test files, ~5365 functions, **7926 collected** (v0.15.3) | `pytest --collect-only -q` |
...
| EXPLAIN_KEYS | 169 (in 47 prefixes) | `bob.explain.EXPLAIN_KEYS` |
