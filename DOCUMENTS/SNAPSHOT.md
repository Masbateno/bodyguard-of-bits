...
│                 9240 unit tests · 21 doc files · 5+ distros field-tested │
└─────────────────────────────────────────────────────────────────────────┘
...
DATA (read-only at runtime, shipped in the package)
    ...
    bob/data/cis_refs.json        ← 194 CIS references
...
│   │   ├── cis_refs.json      ← 194 CIS references
...
├── tests/                     ← 238 test files, ~5920 functions, 9240 collected (v0.17.0)
...
**Why it matters**: pulls the I/O side effects to a single function (`from_system`), making `check_xxx` deterministic for unit tests. **Do not break this contract during refactoring** — it's the foundation for the ~9240-test suite running with no mocks.
...
| `test_cis_refs.py` | 194 CIS references mapping |
...
| Tests | 238 test files, ~5920 functions, **9240 collected** (v0.17.0) | `pytest --collect-only -q` |
