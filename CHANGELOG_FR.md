...
| Version | Date | Résumé |
|---------|------|--------|
| [v0.15.1](#v0151) | non publiée | **Premier patch v0.15.x — une seconde chasse, pour savoir si la première a rendu BOB résilient (en cours, non publiée).** v0.15.0 a corrigé 26 défauts de verdict en balayant six signatures. Rejouer ces balayages ne ferait que reconfirmer ses propres correctifs : cette passe attaque donc par des angles que la précédente n'a jamais employés — au premier rang desquels **le croisement des contrôles entre eux**. BOB possède des surfaces qui se recouvrent (`firewall` / `firewall_stack` / `iptables_nftables` / `ports`, `ipv6` / `hardening`, `services` / `ports`), et deux contrôles qui se contredisent sur la même machine forment un défaut qu'aucun audit module par module ne peut voir. Porte aussi la passe documentaire reportée de v0.15.0 : cinq inexactitudes mesurées, dont une garantie de `SECURITY.md` énoncée plus largement qu'elle ne tient. **Tests** 7220 → **7261**. |
