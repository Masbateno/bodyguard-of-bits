...
| Version | Date | Résumé |
|---------|------|--------|
| [v0.16.4](#v0164) | 07-09-2026 | **« Cette garde mord » était une affirmation dans un message de commit, que rien ne revérifiait.** Neuf commits de la v0.16.3 la portaient ; il en reste **une** trace dans les fichiers de tests. La preuve s'était évaporée, et une garde dont l'ancre dérive à un refactor devient inerte en silence, l'affirmation restant dans l'historique. Les mutations vivent désormais dans `tests/mutations.py` — 26, chacune nommant le fichier, le changement exact, les tests qui doivent échouer et pourquoi le défaut compte — et `scripts/mutate.py` les rejoue en CI. **Le banc énonce ses propres contrôles à chaque exécution** : les gardes nommées doivent passer non mutées, et un changement qui ne peut rien changer (une espace dans une docstring) doit *survivre*. Les deux fois où ce banc a réellement menti, c'était de la seconde sorte — une table nette et plausible ne décrivant rien : une fois en cherchant `failed` en minuscules dans le `FAILED` de pytest, cinq gardes vivantes déclarées inertes ; une fois parce que son enveloppe shell ne transmettait pas ses arguments, aucune mutation n'ayant été appliquée. Il refuse aussi une ancre ambiguë au lieu de deviner, ce qui en a attrapé deux dès sa première exécution. **L'audit dit aussi combien de temps il a pris, le bandeau dit quel BOB a tourné, et le changelog Debian doit décrire les versions qu'il nomme.** La durée est monotone et démarre aux checks, pas à l'entrée du processus ; elle figure sur la ligne de synthèse, dans le rapport archivé et comme `duration_seconds` en JSON — `null` quand l'exécution ne s'est pas chronométrée, `null` et `0.0` étant deux affirmations différentes. La version est cadrée à droite dans le bandeau cyan des cinq assistants : seize endroits écrivaient la ligne 0 à la main, elle est donc écrite une fois dans `_chrome.draw_header` et une garde rejette tout écran qui peint la ligne 0 lui-même. Et `debian/changelog` avait été bumpé **trois versions de suite** sans être réécrit — l'entrée 0.16.3 portait le texte de la v0.16.0 et les 0.16.1/0.16.2 n'existaient pas ; chaque premier point doit désormais être unique et aucune version publiée ne peut manquer. **Et le source affirme sur lui-même des choses que rien ne vérifiait non plus.** `bob/tui/_keys.py`, `_palette.py` et `_chrome.py` promettent chacun de ne jamais importer curses ; trois modules les importent au niveau module *à cause* de cela, et la séparation Debian `bob-core` / `bob-tui` s'y appuie — porteuse en trois endroits, vraie à la mesure, et **jamais exercée**, puisque toute machine qui fait tourner la suite a curses. Un sous-processus importe désormais dix modules avec l'import bloqué, avec un contrôle négatif qui échoue si le bloqueur cesse de bloquer. Deux des trois promesses étaient de plus écrites dans les modules qui s'y *fient* plutôt que dans ceux qui pourraient les casser. **Dix-sept « single source of truth »** — toutes vraies, aucune tenue — sont épinglées : un seul `atomic_write`, un seul `_EMAIL_RE`, un seul `_SECTIONS`, quatre modules partageant un unique traducteur de repli. Et une affirmation était déjà fausse : `_build_v3` émet le schéma 3 et sa docstring disait `v2 producer` depuis la v0.12.0. Trois sondes sur la surface i18n n'ont rien donné, ce qui mérite d'être dit : 1114 clés littérales résolvent, cinq familles de clés dynamiques ne produisent rien de non traduit, aucune orpheline sur 2325. **Tests** 8572 → **8791**. |
| [v0.16.3](#v0163) | 07-09-2026 | **Une ligne d'aide, une note de profil, un manuel de référence — trois affirmations sur BOB, aucune vérifiée contre BOB.** **Huit passes, une même forme.** **Cinq assistants, cinq contrats de touches, et une ligne d'aide en anglais sur trois d'entre eux.** Chaque écran interactif inventait ses liaisons et écrivait ses indications à la main : `--explain` naviguait aux seules flèches, `--manage-logs` ajoutait PgUp/PgDn, les écrans cron ajoutaient `j`/`k`, aucun n'avait `g`/`G`, et `Esc` reculait sur certains écrans sans rien faire sur d'autres. La ligne elle-même n'était traduite que dans `--explain` — un opérateur français lisait du contenu français sous `↑↓: move   Enter: preview`, la classe fermée par la v0.15.3 pour `--help` et laissée ouverte sur tous les écrans qu'on pilote. Un socle unique désormais (`↑↓`/`jk`, `PgUp`/`PgDn`, `g`/`G`, `⏎`), chaque écran déclarant ses actions une fois, le pied de page et le répartiteur en dérivant — ils ne peuvent plus se contredire. `bob/tui/_keys.py` est aux touches ce que `_palette.py` est aux couleurs, et pour la même raison : trois copies concordant par hasard, c'est ainsi que le défaut de charte est resté invisible neuf versions. **`q` ne quitte que depuis une première page** — plus profond, `Esc` recule et une seule touche ne peut pas abandonner un cron à moitié saisi trois écrans plus haut ; six écrans imbriqués ont perdu le leur. **Les actions propres n'apparaissent que là où elles agissent** : `u tout démarquer` une fois quelque chose marqué, `s synthèse` dans l'aperçu, `m carnet` dans le gestionnaire. **Et une bascule de langue sur chaque page de garde** — ces assistants tournent sous `sudo`, qui réinitialise l'environnement, si bien qu'un opérateur au shell français pouvait atterrir dans un assistant anglais sans autre issue qu'un Ctrl-C ; `l` bascule tout l'écran à chaud, étiqueté avec la langue vers laquelle il va. La passe a aussi fait sortir des littéraux anglais portés depuis des versions — `[FULL]`, `line 1–20 / 340`, `scroll up`, `Confirm deletion below`, `Email address book` — tous traduits. Gardé dans six directions, chacune testée par mutation avec contrôle négatif ; deux de ces gardes étaient inertes à la première écriture et ont été réparées avant livraison. **Trois suites** : une ligne marquée était rouge gras dans `--manage-logs`, jaune dans `--manage-cron` et sans couleur dans les deux écrans e-mail — elle est rouge partout désormais, depuis un unique `marked_attr`. `--manage-cron` dispatchait Espace, `a` et `m` sans les déclarer, et le carnet dispatchait `a` sans le déclarer ; une garde écrite pour ce sens les a tous trouvés, après que sa première version eut comparé des guillemets doubles à la sortie d'`ast.unparse` sans rien correspondre. `Esc retour` figure sur les neuf écrans imbriqués et sur aucune des quatre pages de garde — toute l'exception : une première page n'a nulle part où revenir. **Deux autres classes fermées dans la même version.** `--explain` terminait chaque clé sans prose par profil sur « Cette découverte s'applique de la même manière à tous les profils » — faux pour **32 clés** : `ssh.password_auth` est rétrogradée en INFO par `desktop`, `workstation` et `container` et était décrite comme s'appliquant identiquement aux quatre. L'affichage testait l'existence d'une *traduction* et s'en servait pour affirmer quelque chose sur la *politique* ; 71 clés ont bien une prose par profil, les 116 autres recevaient la ligne d'uniformité par défaut et 32 d'entre elles sont surchargées par un profil : l'affirmation venait donc d'une absence d'explication — la plus ancienne classe de défauts du projet, un étage plus haut. La ligne ne survit plus que là où aucun profil ne la contredit (84 clés sur 187) ; les 103 autres rendent une section par profil. Ces sections avaient leur propre version du même trou : les 71 clés à prose n'ont aucune variante `workstation`, et la branche qui les rend sautait tout profil dont la prose manquait — si bien que `--explain clamav.db_very_outdated` affichait server, desktop et container en laissant tomber le quatrième. Chaque profil est désormais rendu compte sur chaque clé différenciée, avec sa prose là où elle est écrite et une note dérivée de son `.conf` là où il n'y en a pas. La garde qui l'a manqué s'appuyait sur une fonction que la branche prose n'appelle jamais ; elle rend les 187 pages maintenant. `workstation` rejoint `--explain` après que ses 28 surcharges propres, depuis la v0.8.1, n'y aient jamais été mentionnées. Et un audit par agent de **README_TECH** a rendu 23 affirmations ancrées, **20 vérifiées et corrigées** dans les deux locales : `sudo bob -f -y` donné en exemple sort en 3 ; `--fix` décrit comme « propose et applique » alors qu'il tourne à blanc ; le brute force auth.log documenté « >10 depuis une IP en 60 s → ALERT −2 » quand c'est ≥50 sur toute la période, toutes sources, WARN, sans déduction ; `firewall_stack` dans la table JSON trois majeures après le renommage en `firewall_drivers`, donc un `KeyError` pour le consommateur ; `network_context` proposant un `"private"` que rien n'émet ; `services[].ports` documenté en liste alors que c'est un objet ; `plugin.sandbox.error` nommée comme *la* clé du bac à sable quand elle est l'une de douze ; 5 clés JSON requises, 19 options sur 43 et 6 services sur 38 sans aucune ligne. La moitié dénombrable est désormais gardée dans les deux locales, chaque garde testée par mutation — trois étaient inertes à l'écriture, acceptant `OllamaXX` et `--targetXX`. La prose reste un travail de lecture. **Et le bas de chaque assistant est deux bandes.** Les indications étaient du texte sur la dernière ligne, et deux écrans écrivaient par-dessus : `--manage-logs` remplaçait toute la ligne de touches par sa confirmation de suppression — confirmer une action destructive sur un écran qui venait de masquer la touche d'annulation — et les assistants cron dessinaient sept saisies en `h - 1`, sur le pied de page qu'ils venaient de tracer, l'un d'eux sans aucune touche affichée. Le bas est désormais une ligne réservée aux prompts et messages au-dessus d'un **bandeau orange de touches** en miroir du bandeau cyan, blanc sur l'orange de l'outil, tirant sa couleur du même appel que la ligne de curseur. Il se replie sur une seconde ligne plutôt que de tronquer : à 80 colonnes cinq écrans débordent en français et une coupe `[:w]` taille par la droite, où se trouve l'indication de sortie. Cela ferme aussi un défaut de dimensionnement présent partout — tous utilisaient `h - 2`, une ligne de trop dès que les indications se repliaient, si bien que la seconde ligne tombait sur la dernière entrée. `--explain` reçoit le même traitement : en-têtes `[ profil ]` en orange, crochets compris, et un `HOW TO FIX` indenté. **Les assistants cron étaient encore en anglais, et un écran n'affichait aucune touche.** Dix-neuf littéraux avaient survécu à la passe v0.16.3 — toutes les saisies (`Email`, `Name`, `Time (HH:MM)`, `Expression`), les indications sous elles, tout l'écran « ajouter une adresse » du carnet et ses messages : un opérateur français pilotait un assistant français et saisissait dans des champs anglais. Tout traduit, sous un espace `cron_ui`, avec une garde qui lit le source à la recherche de toute chaîne encore dessinée sur un écran cron sans `t()` — les arguments de `t()` exclus **par règle**, `mta="sendmail"` étant un nom de programme, et le mettre en liste blanche est ainsi qu'une garde se met à s'entretenir en s'élargissant. Le séparateur est une traduction lui aussi : `Nom: nightly` n'est pas du français. **La saisie du nom de `--install-cron` n'affichait aucune touche** — le seul écran à ne jamais appeler le pied de page, si bien qu'on demandait un nom sans rien pour dire comment le valider ou repartir. `_curses_readline` prend désormais les actions de l'écran et dessine le bandeau lui-même : l'oubli devient irreprésentable. Cela en a révélé un autre : **Entrée était dispatchée par toutes les saisies et déclarée par aucune**, la seule issue annoncée d'un champ étant de l'abandonner ; `SUBMIT` (`⏎ submit` / `⏎ valider`) rejoint le contrat de touches. **Tests** 8328 → **8572**. |
| [v0.16.2](#v0162) | 04-09-2026 | **Un plafond affirme une direction, et l'aveuglement n'en a pas toujours une.** La v0.16.0 raisonnait qu'un check incapable de lire ne déduit rien alors que ces déductions sont inconnues, donc le score ne peut être que trop haut — vrai tant que le domaine de ce check est encore noté, faux quand le **domaine entier** quitte la moyenne : c'est le *dénominateur* qui change, et le résultat bouge imprévisiblement. Masquer `/etc/passwd` retire `file_perms` (10/10) et fait passer le score de 7 à **6** : `≤ 6` affirmait un plafond sous la vraie valeur pendant que le domaine affichait `no action needed` — un feu vert sur ce que BOB n'a jamais regardé. Trois états désormais : un plafond reste `≤ N/10` ; une direction imprévisible donne `~ N/10` à côté de l'intervalle où il se trouve, et cet intervalle est de l'arithmétique, pas une supposition — un domaine vaut entre 0 et 10 par construction, donc les manquants aux deux extrêmes encadrent la vérité. Il dit aussi **combien est en jeu**, ce que `~` seul ne dit pas. `score` reste un entier ; `score_low`/`score_high`/`unscored_domains` sont additifs et `score_low == score_high` se lit « entièrement vérifié » sans cas particulier. **Le portail a suivi** : `--target` lisait `score_is_upper_bound`, et rendre cette propriété honnête aurait laissé l'exécution imprévisible passer la CI au vert — il lit `score_is_uncertain`, et la garde, épinglée sur un littéral et modifiée pour la troisième version d'affilée, est devenue comportementale. **`--target` ne laissait par ailleurs aucune trace machine** alors qu'il ferme des chaînes par le code 4 ; `target` et `target_met` sont additifs en JSON et reflètent la condition de sortie au lieu de la recalculer. **Le script cron a cessé d'expédier l'arbre de travail en production** — il exportait `PYTHONPATH=<parent du paquet>` sans condition, si bien qu'un assistant lancé depuis un dépôt git envoyait indéfiniment des modifications non commitées à l'audit nocturne ; il n'est plus émis que lorsqu'un interpréteur propre ne peut pas importer `bob` seul, et échoue du bon côté. Plus quatre du balayage d'options : `--webhook` sauté en silence sous `--offline` alors que `--test-webhook` l'annonçait ; `--ignore=CLÉ` validait la forme et non l'existence, donc une faute de frappe ne masquait plus jamais rien ; le refus de `--quiet --fix --apply` donnait une raison fausse avec `--yes` ; et `--breakdown` annonçait « Plafond de score actif (plafond : 3/10) » trois lignes au-dessus de « Score final : 7/10 » sans dire que le plafond porte sur le domaine. **L'aveuglement ne doit jamais rétrécir le registre de l'aveuglement** — deux autres cas de la famille, trouvés en rendant 23 répertoires non traversables sur le banc. La détection de services sondait un chemin de binaire absolu depuis le noyau always-on, que l'isolation de `_sec` ne couvre pas, et `Path.is_file()` relève PermissionError : un `/usr/local/bin` non traversable donnait une sortie 3 sans aucun rapport. `path_is_file` est le jumeau que `path_exists` n'avait jamais eu, et sa garde a immédiatement trouvé `is_symlink()` hors du try voisin — celui-là garde une frontière de confiance, sa réponse d'échec est donc **False**. Par ailleurs une section qui *levait* émettait `<section>.unavailable`, absente de tout ensemble de visibilité : durcir `/etc/cron.d` faisait passer `unverified` de 9 clés **à 8**, l'échec le plus dur comptant comme mieux vérifié. Désormais 23 répertoires durcis donnent un audit complet qui dit ce qu'il vaut — `~ 7/10`, 18 clés non vérifiées, « entre 4 et 8 ». **Tests** 8282 → **8328**. |
| [v0.16.1](#v0161) | 04-09-2026 | **Deux surfaces d'une même affirmation, et une seule était exercée.** Chaque défaut de cette version est une paire qui se contredisait alors que chaque moitié semblait correcte isolément : l'audit qu'on lance à la main et celui que lance le cron ; la ligne de synthèse et le code de sortie ; la table du README et le binaire ; un champ écrit sur disque et le champ relu ; l'assistant texte que pilotaient les tests et l'assistant curses qu'obtient un humain. **Un audit programmé exécutait un autre audit que celui choisi.** `--install-cron` générait un script dont la ligne d'audit était figée à `bob --quiet --detailed`. Cron l'exécute en root : il lisait donc le profil enregistré de *root*, pas celui de l'opérateur — et l'environnement nu de cron, où `$LANG` est absent, si bien qu'une installation française envoyait un rapport anglais toutes les nuits. Le profil n'est pas cosmétique : il décide quels constats déduisent, donc le nombre d'avertissements, donc le code de sortie, donc si l'e-mail de notification part. L'assistant pose désormais trois questions fermées — profil, langue, sondes sortantes — sur trois écrans, dans le chemin texte comme curses, chacun ouvert sur la valeur de la session qui installe, et affiche la ligne de commande résultante avant toute écriture. Chaque dimension est émise explicitement même à sa valeur par défaut : tout l'objet est que l'audit programmé n'hérite de rien. `--verbose` a été écarté sur mesure et non sur lecture — sous `--quiet` il change le log de zéro ligne. La chaîne d'options atterrit dans un script root : elle est donc construite à partir d'ensembles fermés dans un seul module, revalidée au site d'écriture, et les seize combinaisons font l'aller-retour par le vrai parseur — une regex prouve la forme, seul le parseur prouve que ça marche. **Les crons installés ne sont pas touchés** — régénérez avec le même nom pour en migrer un. **Le rapport d'un audit français s'ouvrait et se fermait en anglais** — `[INFO] Starting audit` et `[NEXT STEPS]`, trouvés dans un vrai passage ; le premier était écrit en dur huit lignes après que le même événement eut été imprimé traduit au terminal. Une garde rejette désormais tout littéral visible par l'opérateur écrit dans le rapport texte. **L'assistant interactif plantait depuis la v0.7.0** — `'_WizardEntry' object has no attribute 'time_simple'`, levé dès qu'une planification était choisie, sur 40 versions publiées. `run_install_cron` s'aiguille sur `sys.stdout.isatty()` : chaque test et chaque vérification de cette release pilotaient donc l'assistant texte — un tuyau n'est pas un terminal. Le chemin qu'obtient un humain n'avait aucune couverture. Fermé par un balayage statique de chaque `entry.<champ>` lu par l'assistant, plus un parcours complet sur un vrai pty. **La ligne du curseur ressemblait à un second bandeau** — sélection et bandeau de titre étaient tous deux sur `COLOR_CYAN`, sur les quatre écrans curses. La sélection est désormais orange (index xterm-256 208, repli `COLOR_YELLOW` à 8 couleurs). La charte était écrite trois fois, une par module ; elle vit maintenant une seule fois dans `bob/tui/_palette.py`, avec une garde qui rejette tout `init_pair` hors de ce module et une autre qui vérifie que le fond de la sélection n'est jamais celui du bandeau. **La v0.16.0 n'avait fermé qu'une moitié de sa propre classe, et l'autre était vivante.** Elle raisonnait : un check incapable de lire ne déduit rien, donc le score ne peut être que trop haut — vrai d'un check qui s'abstient dans un domaine, faux d'un domaine entier : le score global est une moyenne sur les domaines **actifs**, et un domaine devenu illisible quitte la moyenne, changeant le dénominateur. Masquer `/etc/passwd` retire `file_perms` (10/10) et fait passer le score de 7 à **6**, déductions identiques au point près, rien dans `unverified` — lu face à une baseline voyante comme `Score dégradé de 1 point(s)`, une régression sur un hôte où rien n'a changé sauf la vue de BOB. Trouvé par un balayage local exhaustif des options (~700 invocations). Trois défauts, une racine : `user_accounts.no_passwd` / `no_shadow` étaient hors de `VISIBILITY_KEYS` parce que l'ensemble avait été énuméré **par nom** et qu'aucune ne finit par `_unreadable`, d'où une troisième garde qui balaie **par position dans le source** (corps de branche seulement — lire le `else` fait remonter le chemin lisible honnête comme une limite) ; `load_baseline` ne restaurait jamais `unverified`, donc `prev.unverified` valait toujours None et **toute la protection de visibilité de `--diff` était morte hors des tests unitaires**, qui ne passent jamais par le fichier ; et aucune branche n'existait pour un score qui *baisse* sur une exécution plus aveugle — une vraie baisse reste un avertissement, et c'est un test à part. **`--target` disait le contraire de son propre code de sortie** : la v0.16.0 avait fait échouer fermé sur un score borné en laissant la synthèse imprimer `✔ objectif atteint`, si bien qu'une même exécution disait à l'humain que c'était passé et à la CI que non — et sur tout hôte avec une section illisible, **tous** les `--target` sortaient en 4, `--target=1` compris. **Reste ouvert, délibérément** : l'en-tête affiche `≤ 6/10` là où le score voyant est 7 — quand l'aveuglement retire un domaine entier le score peut bouger dans les deux sens, donc nommer cet état est une décision de conception, reportée plutôt que devinée. **Tests** 8152 → **8282**. |
| [v0.16.0](#v0160) | 04-09-2026 | **Le score montait quand BOB voyait moins.** C'est une somme sur les checks qui ont tourné, et un check incapable de lire son entrée ne déduisait rien alors que ces déductions sont *inconnues, pas nulles* : masquer `/etc/ssh/sshd_config` en retire quatre et fait passer le score de 7 à **8**. `degraded_sections` restait vide — correctement, il est réservé aux sections dont le check a *levé* — donc une abstention propre ne laissait aucune trace dans la sortie machine, et sans privilège **huit** sections de cet hôte ne peuvent pas être entièrement lues. Une réserve à côté du score ne suffit pas, car c'est le nombre qu'on lit : il est désormais rendu comme le plafond qu'il est (`≤ 8/10`), le niveau de risque se lit comme un meilleur cas, la flèche de tendance est retenue tant qu'il est borné, et une ligne de résumé indique combien de sections n'ont pas été entièrement lues. Déduire pour illisibilité a été écarté — cela punit l'opérateur pour les privilèges de BOB. `score` reste un entier en JSON ; `score_is_upper_bound` et `unverified` sont additifs. `--diff` ne qualifie plus un constat de résolu quand sa section n'a pas été réévaluée, et une baseline antérieure au champ se lit **inconnue**, jamais « tout était visible alors ». Les 32 clés de visibilité vivent dans un ensemble explicite, gardé dans les deux sens. **`logs.brute_found` est renommée** et l'ancien nom continue de fonctionner. La v0.15.5 avait corrigé ce que l'opérateur voit et laissé la clé, parce qu'un renommage casse les baselines, `ignore.yml` et `--explain` — travail de lot planifié, et voici le lot. Elle rejoint ses deux frères en **`blocked_repeat_public`**. Chaque surface de compatibilité échoue différemment si on l'oublie, d'où un test par surface : une baseline non remappée rapporte un même problème comme résolu et nouveau, un ignore.yml non consulté laisse revenir un constat écarté, un alias manquant transforme `--explain` en exit 3. Les deux migrations de baseline se composent dans l'ordre. `auth_log.brute_force` est délibérément intacte — autre clé, dont la preuve soutient le mot — et c'est un test. **Une qualification pointant vers un constat mis en silence** est retirée : le résidu consigné par la v0.15.5, où ignorer une réserve laissait chaque constat qualifié pointer vers ce que le JSON n'émet plus. **Une vérité, six rendus, deux mis à jour.** Le rapport archivé disait `Score : 7/10` là où le terminal disait `≤ 7/10`, du même audit — et Markdown, HTML, CSV et le webhook se taisaient aussi. Les six le portent désormais, chacun sous la forme que son consommateur accepte, et **la garde est le vrai correctif** : toute sortie neuve doit s'y déclarer. Elle a immédiatement trouvé que **`--target` pouvait être satisfait par un plafond**, si bien qu'un audit lancé sans les privilèges nécessaires faisait passer un portail d'intégration au vert — il échoue fermé maintenant. Le rapport gagne aussi le **profil d'audit**, qui change les sévérités et le code de sortie. Et **`bob -q -d` écrivait un rapport de 440 lignes sans aucun bloc de synthèse** — `-q` concerne stdout, `-d` est un fichier explicitement demandé, et display.py énonçait déjà la règle que le site d'appel violait. Le corriger a introduit la forme du hotfix v0.8.3, attrapée par les tests puis par une garde statique — qui a elle-même dû être écrite deux fois, contournant une branche par `continue` alors qu'`ast.walk` livre les descendants quand même. **Une section absente du rapport avait trois sens possibles** — écartée, dégradée ou propre — et les trois se ressemblaient dans un fichier archivé. Une ligne **Périmètre** nomme désormais les deux premiers, séparément. `degraded_sections` arrivait dans la synthèse comme paramètre lu nulle part, la forme « déclaré mais jamais consommé », introduite et attrapée dans la même session. **`python3 bob` répondait par une trace nue** — exécuter le répertoire du paquet comme script exclut son parent de `sys.path`. La forme reste non supportée ; BOB nomme désormais ce qui s'est passé et les deux formes qui marchent. La chaîne est en anglais codé en dur car `bob.i18n` est justement ce qui ne peut pas être importé là, et seul un `bob` manquant est intercepté pour qu'une dépendance absente lève toujours. **`--format=html` plantait et toute la suite était verte** — la borne était appliquée en réaffectant `score` à une chaîne, au-dessus d'un `_score_color(score)` qui la compare à des seuils. Le double HTML fixe la borne à False, le chemin borné n'était donc jamais rendu ; c'est en lançant le binaire qu'il est apparu. Une vérification de présence n'est pas une vérification de rendu : les trois constructeurs qui n'en avaient qu'une sont désormais exercés dans les deux états. **Une passe exhaustive sur les options a trouvé `--install-cron` cassé par cette version** : le script email généré extrait le score du journal avec un motif que `≤` met en échec, si bien que le sujet devenait silencieusement `Score N/A` sur tout hôte borné. **Et SECURITY.md nommait une sortie inexistante** — il attribuait `html.escape` et `_safe_url` au « rapport HTML `-d` », alors que `-d` écrit du texte brut ; la protection appartient au rapport email cron. Les 45 options longues ont été exercées, et `FORCE_COLOR` ajoutée à la table des variables où elle manquait. **Tests** 8089 → **8152**. |
| [v0.15.5](#v0155) | 03-09-2026 | **Cinq angles de chasse sur une seule question : ce que dit BOB est-il vérifiable ?** Non-breaking — sur un hôte privilégié, l'audit est identique à la v0.15.4. Premier angle : « non installé » et « rien n'a pu être interrogé » étaient la même réponse. Trouvé avec un instrument neuf : un audit complet lancé dans un espace de noms où tous les outils externes sont retirés du PATH. Un audit aveugle devrait dire « je n'ai pas pu regarder » presque partout ; il a rapporté **12 déductions et zéro section dégradée**, annonçant un audit réussi. La plupart survivent honnêtement — `backup`, `log_rotation`, `mac_policy` et `ntp` détectent par l'absence de l'outil *lui-même*, si bien qu'« aucun outil de sauvegarde trouvé » est une affirmation vraie. Un seul ne l'était pas. `package_installed()` rend None aussi bien quand un paquet est absent que quand **aucun gestionnaire de paquets n'existe pour l'interroger**, et `firmware` en tirait « CPU détecté mais aucun paquet microcode installé » — un WARN avec un point déduit pour un négatif jamais établi, sur Gentoo, NixOS, Void, Slackware ou toute image minimale. `package_query_possible()` permet désormais à un contrôle de dire qu'il n'a pas pu regarder : le constat dégrade en INFO, ne coûte rien, et nomme la raison. Le jumeau de polarité est un test — un paquet réellement absent avertit toujours et déduit toujours. Un garde impose à tout appelant qui transforme une requête de paquets en affirmation de consulter le helper, avec trois exemptions documentées qui reposent sur des preuves indépendantes (un fichier de configuration, un snap, un binaire sur le PATH). **Puis l'instrument a été décliné outil par outil** — PATH reconstruit par liens symboliques, un seul binaire retiré, vingt-neuf fois. Vingt-six se taisent correctement. Retirer le seul `aa-status` faisait rapporter à BOB `mac_policy.apparmor_inactive` — WARN, un point — sur une machine dont le module AppArmor est chargé et activé : le snapshot posait `apparmor_active = False` dans la branche commentée « module chargé mais aa-status indisponible », transformant un outil manquant en un contrôle qui n'applique pas. Le noyau répond sans l'outil — `/sys/module/apparmor/parameters/enabled` rend Y et est lisible par tous, et securityfs ne monte le répertoire apparmor que tant que le LSM est vivant — si bien que le contrôle rapporte désormais « actif, profils illisibles », le constat INFO qui existait déjà pour la variante « privilège partiel » du même défaut. Deux gardes ont attrapé mes propres erreurs au passage : celle de v0.15.2 sur les `.exists()` nus a rejeté le premier jet, et les tests ont été réécrits après une version qui patchait `_run._command_exists` alors que `mac_policy` détient sa propre référence — elle annonçait la correction fonctionnelle sans jamais l'exercer. **L'instrument a une seconde variante, et elle a montré que les deux correctifs ci-dessus étaient à moitié faits.** Masquer un binaire n'est pas ainsi qu'un outil échoue en pratique — lancer BOB sans sudo l'est. L'outil est alors *présent*, donc `_command_exists` répond oui et toutes les branches « outil absent » sont sautées ; la commande ne répond simplement rien. Remplacer chaque binaire par un bouchon qui refuse a fait ressurgir `aa-status` rapportant AppArmor inactif, par la branche que le premier correctif n'atteignait jamais, et `dpkg-query` ressuscitant `firmware.microcode_missing`. Le cas des paquets est le plus tranchant : avec une base illisible, dpkg sort en 1 avec `no packages found matching <pkg>` — le **même** code de sortie et le **même** message qu'un paquet réellement absent, si bien que la distinction ne peut pas être reconstituée depuis sa sortie. Ce qui reste établissable, c'est si la couche répond : BOB demande donc à chaque gestionnaire un paquet qu'il possède nécessairement — lui-même — et n'accorde crédit à un négatif qu'après avoir produit un positif. Vingt-neuf outils × deux modes de défaillance = cinquante-huit audits ; après ces correctifs, seul `systemctl` transforme encore sa propre absence en verdict, et celui-là est diagnostiqué, non corrigé. **L'instrument a ensuite été tourné vers les fichiers plutôt que les binaires.** Des chemins existants ont été masqués un à un pour que leur lecture lève `PermissionError`. `/proc/sys/vm/swappiness` produisait `memory.swappiness_ssd_wear` — un WARN avec déduction — contre un seuil de 30, sur un hôte dont la valeur réelle est 10 : le lecteur renvoyait un `60` codé en dur quand il ne pouvait pas lire, et sa propre docstring le disait. Un helper qui répond un nombre plausible alors qu'il a échoué est indiscernable d'un helper qui a mesuré, et 60 étant le défaut du noyau, la valeur inventée était la plus susceptible d'être crue. Le lecteur répond désormais `None`, les trois comparaisons qui le consommaient sont gardées — la troisième n'a été trouvée que parce que le nouveau test a planté dessus — et le check énonce ce qu'il n'a pas pu lire plutôt que de se taire. Une garde de forme rejette tout littéral numérique renvoyé depuis un `except` dans un lecteur, resserrée après qu'une première version a signalé quatre `return 1` d'installateurs cron qui sont des codes de sortie et non des mesures : une garde qui produit du bruit finit désactivée. **Un test de frontière dont la frontière bougeait :** la suite a franchi minuit pendant ces travaux et un test d'expiration de compte a échoué sur un arbre où rien n'était cassé, parce qu'il calculait « aujourd'hui » à l'import tandis que le check lisait l'horloge à l'exécution. L'horloge est figée des deux côtés, ce qui a été vérifié en déplaçant le point fixe et en regardant le verdict de production le suivre. **Un `/etc/default/ufw` illisible était lu comme « UFW gère IPv6 ».** Même instrument, même classe, un fichier de plus : sur cet hôte ce fichier porte `IPV6=no`, et perdre la lecture inventait `True`, si bien que chaque listener IPv6 non couvert devenait `ipv6.port_no_v6_rule` — une déduction pour des règles manquantes à un pare-feu à qui l'on avait dit de ne pas gérer IPv6. La distinction est **absent/illisible** : un fichier absent est le défaut propre d'ufw et répondre True est une mesure ; un fichier illisible n'est pas une réponse. Les deux modules qui le lisent répondent désormais None, et le câblage a demandé trois passes, chacune devenue un test — `not None` vaut True, donc la branche « le noyau a IPv6, UFW non » se déclenchait sur un inconnu et coûtait deux points ; tomber au bout atteignait le `else` qui affirme l'accord des deux côtés ; et l'OK de clôture est *hors* de la chaîne, si bien que l'audit certifiait encore « tous les listeners sont couverts » face à une politique jamais lue. Un satisfecit tiré d'un fichier non lu est pire qu'un faux négatif, car personne ne va le vérifier. Masquer `/etc/sudoers` et `/var/lib/clamav/daily.cld` déplace aussi des verdicts et les deux sont des **artefacts de banc** — `mount --bind` remplace le fichier, donc les droits et la mtime lus sont ceux du masque — établi après confrontation d'un run de contrôle sans masque, dans le même espace de noms, à la référence. **La liste « diagnostiqué, pas corrigé », points A et B** — le même défaut que le reste de cette version, atteint par un autre chemin. **A :** `unit_active_state` rend un tri-état précisément parce que `is_unit_active` collapsait « inactif » et « systemd n'a jamais répondu » dans un même False ; le *repli* en dessous avait le même collapse — `_run("auditctl", "-s") or ""` — si bien que, ni systemd ni auditd ne pouvant répondre (BOB lancé sans sudo, où le binaire est présent et toutes les branches « pas d'outil » sont sautées), BOB avertissait qu'un auditd en marche était arrêté, avec déduction et ordre de démarrer un service déjà démarré. fail2ban portait une copie. Les deux répondent désormais None et rapportent `state_unknown`, INFO. **B :** `query_failed` ne consultait qu'un binaire, donc un hôte nftables sans binaire iptables voyait nft refuser et récoltait `firewall_iptables.no_backend` — trois points, la plus grosse déduction du check, plus « apt install iptables » sur une machine dont le backend tourne. Le discriminant a été établi contre nft lui-même : sans privilège il sort en **1** avec stdout vide, face à un jeu de règles réellement vide il sort en **0**, vide aussi — stdout ne les sépare pas, le code de sortie si, délibérément l'inverse du choix documenté pour systemctl. Séparer les deux a révélé un troisième état caché dans le premier : nft qui répond sans hook d'entrée est une *mesure*, donc cela garde ses trois points sous `firewall_iptables.nft_no_input_filter`, dont le correctif nomme le backend que l'hôte possède — un conseil faux par-dessus un constat vrai reste un conseil faux. **Une déduction de sauvegarde qui reposait sur un nom dans le PATH** — le miroir du reste de cette version : une observation qui existe, et un verdict qui en tire plus qu'elle ne porte. Les neuf branches s'ouvraient par `_command_exists`, donc une affirmation sur le fait que des sauvegardes sont *configurées* reposait sur la présence d'un binaire. **Reproduit sur le banc d'abord :** un PATH miroir privé des neuf binaires donnait `backup.no_backup`, −1, et ajouter une vraie sauvegarde — `/etc/cron.d/rsnapshot` plus un `nas-backup.timer` lançant duplicity vers un NAS — ne le déplaçait pas, alors que le snapshot cron du même espace de noms renvoyait ce fichier avec son chemin source. Le cas le plus serré n'exige aucun outil inconnu : cet hôte porte `/etc/timeshift/timeshift.json`, un chemin que le module détient déjà comme constante, jamais consulté parce que les artefacts se trouvaient *derrière* la porte du binaire — ce qui inverse la hiérarchie des preuves, un fichier de configuration affirmant plus sur la configuration qu'un nom sur le PATH. Les artefacts sont lus hors de la porte et les jobs planifiés balayés pour un outil nommé ; la déduction est retirée et un INFO nomme ce qui a été vu. Il **n'affirme pas** que les sauvegardes vont bien : il retire l'affirmation qu'aucune n'est configurée. La moitié dangereuse étant l'erreur inverse, le balayage est une liste de noms de binaires et non une correspondance floue : `rsync` seul en est exclu, les commentaires sont ignorés, la correspondance se fait sur frontière de mot, et un artefact illisible n'est pas une preuve. Deux messages ont cessé de surestimer leur portée. **`file_integrity` a été examiné puis laissé tel quel** — il partage la porte mais son verdict « pas d'outil » est un INFO à zéro point qui nomme déjà son périmètre ; une estimation antérieure l'incluant était fausse. **« Bruteforce attempts », depuis des paquets que le pare-feu avait déjà jetés** — l'angle de la sur-interprétation refermé. Un UFW BLOCK n'a jamais atteint de service, donc aucun identifiant n'a été présenté et une devinette répétée ne s'y lit pas ; le module frère qui lit de vrais échecs d'authentification rapporte un décompte, donc celui à la preuve la plus faible faisait l'affirmation la plus forte. **Reproduit sur le banc d'abord** avec un ufw.log fabriqué : un NAS du LAN faisant deux tentatives TCP vers 445, un objet connecté en SSDP sur 1900/udp et un balayage externe de 22/tcp donnaient **trois déductions, trois points**, les trois qualifiées de force brute — et `local_dominance`, l'exception taillée pour ces faux positifs, ne se déclenchait pas à 27 blocages sur 67. Le seuil comptait des paquets : avec `tcp_syn_retries` à 6, un connect bloqué émet six SYN, donc deux tentatives ordinaires franchissaient un seuil de dix. Le discriminant était dans chaque ligne et n'avait jamais été lu : une retransmission réutilise son port source, une nouvelle tentative non. Les sources locales et UDP sont désormais rapportées sans déduction — une adresse privée est un appareil de l'opérateur, et 1900 SSDP n'a aucun identifiant à deviner. La reclassification a failli faire taire BOB, pire que le faux positif : `display.py` ne rendait que les WARN, repéré en rejouant le banc. Une garde a dû être écrite deux fois, la première restant verte quand la branche INFO était supprimée. Trois déductions deviennent une, énoncée comme la mesure. La clé `logs.brute_found` reste délibérément inchangée — la renommer casserait les baselines et `--explain` sur un correctif. **BOB audite le fichier, l'attaquant rencontre le processus** — un troisième axe, qui ne demande plus si la preuve soutient le verdict mais de quoi le verdict *parle*. Les constats SSH sont formulés comme un comportement (« Root login via SSH **is** permitted ») et lus dans sshd_config, que sshd charge au démarrage et au rechargement ; 24 points de l'outil reposent sur ce fichier et 3 sur journald.conf. Le sens dangereux est le silencieux : un fichier tout juste durci sur un démon qui tourne encore avec les anciens réglages donne une section propre, sans aucun constat auquel accrocher une réserve — le problème de `backup` à nouveau. **La sémantique de rechargement a été établie contre systemd, sur une unité utilisateur jetable :** après `systemctl reload`, PID principal inchangé, ExecMainStartTimestamp et ActiveEnterTimestamp sont restés au démarrage initial et seul **StateChangeTimestamp** a bougé. Comparer à l'une des deux autres aurait signalé tout administrateur ayant édité puis correctement rechargé — une garde qui se déclenche sur les gens qui font bien finit désactivée. Cette propriété est désormais figée par un test qui lit les arguments passés à systemctl, la première série de mutations n'ayant rien trouvé quand on l'échangeait. BOB compare la mtime la plus récente parmi tous les fichiers ouverts par le parseur, drop-ins inclus, et le dit quand le fichier l'emporte ; cela ne coûte rien et l'inconnu reste inconnu. **samba a été examiné puis exclu sur les termes de smbd(8)** — il recharge smb.conf toutes les trois minutes, la fenêtre de divergence est donc bornée ; PAM et login.defs sont relus à chaque authentification, sysctl est lu dans /proc/sys, et huit modules interrogent déjà l'outil vivant. **« All SUID binaries are known-safe », sur une marche que find n'a pas pu finir** — l'axe de l'agrégation : un verdict qui quantifie sur un ensemble que BOB n'a pas vu en entier. `suid_audit` lisait la sortie de find et jamais son code de retour. **Établi contre le binaire que BOB lance réellement**, après qu'une première sonde a mesuré le mauvais — cet hôte a un alias `find` vers `bfs` dans le shell, alors que `subprocess.run` résout vers `/usr/bin/find`, GNU findutils 4.9.0 : sur une arborescence comportant un répertoire illisible, il sort en **1**, nomme le répertoire sur stderr, et imprime malgré tout ce qu'il a atteint — une marche partielle est donc indiscernable d'une complète sur la seule sortie standard. Le cas ordinaire, c'est BOB sans sudo : sans privilège ici, le balayage atteint 22 SUID et 13 SGID sans pouvoir entrer partout, et l'audit répondait « All SUID binaries are known-safe (22 SUID, 13 SGID) » sur exactement cela. Il nomme désormais ce qu'il a vu ; la variante vide, qui produisait « 0 SUID, 0 SGID » comme un succès, devient un balayage sauté. Le partiel coupe l'affirmation positive et jamais la négative. Deux gardes ont dû être réparées d'abord : un bouchon de `stat` à signature trop étroite faisait *erreurer* deux mutations au lieu de les faire échouer, et un décompte cherchant `FAILED` lisait cela comme une garde qui tient — deuxième fois dans cette version qu'une garde annonce une confiance non gagnée. **Une réserve restée en arrière pendant que le constat qu'elle qualifiait voyageait** — l'axe des verdicts composés. `correlation.py` fabrique des constats à partir des clés d'autres constats (six règles, trois en ALERT), et une réserve n'existait que comme entrée frère dans une liste plate : `Finding` n'avait aucun champ reliant l'une à l'autre. Démontré avant modification — sur un hôte dont sshd_config était durci mais jamais rechargé, `corr.root_no_protection` se déclenchait en ALERT sans trace de la réserve que le même audit venait d'imprimer, et **trois règles sur six consomment les deux clés que cette réserve qualifie**. L'essentiel de la couche était déjà sain et a maintenu le correctif petit : `active` est filtré sur WARN/ALERT, donc toutes les réserves de cette version qui *remplacent* un verdict n'ont besoin de rien ici, et `corr.password_auth_under_attack` a été vérifié propre (il lit de vrais échecs d'authentification). Le défaut est la moitié inachevée d'un travail livré trois commits plus tôt, pas une régression. `Finding.qualified_by` plus `CheckResult.qualify(clé, since=…)` — un index plutôt qu'une liste de clés, qui dériverait dès l'ajout d'une directive — et le qualifiant ne se qualifie jamais lui-même, ce qui récurserait dans chaque consommateur. **Les autres consommateurs avaient la même cécité**, ce qui a décidé du correctif structurel plutôt que d'un rustinage : `json_output` et `webhook` émettent `qualified_by` de façon additive, et le constat composé transporte le *message* du qualifiant, déjà une phrase écrite pour un humain. `compare` est délibérément intact — `finding_keys` est un contrat de baseline. **Une garde qui échouait sur des modifications sans défaut derrière elles** — `test_fix_coverage` adressait ses sites enregistrés par `(fichier, numéro de ligne)`, donc toute insertion au-dessus faisait échouer la suite pour rien. Sa propre note consignait deux déplacements en v0.15.0 ; cette version a déplacé l'entrée ssh deux fois de plus en une journée. Même mode de défaillance qu'une garde annonçant une confiance non gagnée, pris par l'autre bout : l'une crie au loup, l'autre dort, les deux finissent ignorées. Les sites sont désormais adressés par `(fichier, fonction englobante)` — vérifié en insérant cinq lignes dans chacun des deux fichiers enregistrés et en constatant que la garde reste verte — avec une seconde garde pour le nouveau mode de défaillance, plus silencieux : une entrée nommant une fonction disparue. **Tests** 7958 → **8134**. |
| [v0.15.4](#v0154) | 02-09-2026 | **Premiers travaux v0.15.4 — le titre des releases GitHub était tronqué en plein milieu.** ⚠️ **Numéro de patch, changements de score BREAKING** — entorse assumée au versionnage pour vider le backlog d'un coup ; voir la note du changelog complet. `publish.yml` composait le nom `vX.Y.Z — <titre>` en prenant la cellule du tableau de CHANGELOG.md et en coupant au premier ` — `. Or la cellule s'ouvre sur un gras qui contient lui-même des tirets cadratins : la coupe tombait donc à l'intérieur du gras et laissait ses `**` orphelins. Toutes les releases depuis au moins la v0.14.0 jusqu'à la v0.15.3 ont été publiées sous la forme, littéralement, `v0.15.3 — **First v0.15.3 work`. Le titre prend désormais l'intervalle entre la première paire de `**`, avec repli sur la première phrase si une ligne n'a pas de gras. La coupe de longueur a disparu avec : `cut -c` compte des **octets** dans GNU coreutils, si bien qu'un titre contenant des tirets cadratins était tronqué plus court que prévu et pouvait scinder un caractère multi-octet — le même piège octets/caractères déjà rencontré sur la largeur de l'aide en v0.15.3. Le plafond est reporté sur CHANGELOG.md, où un humain peut raccourcir, plutôt que de mutiler le titre en bout de chaîne. Le garde exécute le bloc shell du workflow au lieu de le réimplémenter, un garde qui réimplémente ce qu'il surveille cessant de le tester dès que l'un des deux dérive ; réinjecter l'ancienne coupe le fait échouer sur les douze versions documentées d'un coup. **Item 5 du backlog — toute fonction publique doit avoir un appelant.** Troisième occurrence du motif le plus rentable de la campagne : `strip_rule_index` dans `checks/_ufw.py` est née sans appelant et n'était même pas exercée par un test, et `print_port_detail` était inutilisée depuis la v0.1.0, maintenue en vie par son seul test. Retirées plutôt que mises en liste blanche — un garde qui naît avec des exemptions est affaibli dès le premier jour. Écrire une sonde qui ne ment pas a demandé trois tentatives, et les deux erreurs sont figées dans le garde : un import aliasé (`read_line as _rl`) masque le vrai nom si l'on ne compte pas les deux, et un `FunctionDef` ne produit aucun `ast.Name`, donc le seuil est `>= 1` — s'y tromper signalait toute fonction appelée une seule fois et gonflait la liste de 2 à 17. Les deux surfaces sœurs sont propres : les 11 champs de `services.json` sont tous lus, les 25 clés racine du JSON toutes documentées. **Alignement des EXEMPLES** : les deux commandes longues débordaient la gouttière de 30 colonnes et décalaient leur texte de 6 et 13 colonnes ; la colonne épouse désormais la plus longue. **Item 1 du backlog — `config_key` retiré du registre des services.** Le champ déclarait une stratégie de résolution des ports sous quatre formes ; aucune n'a jamais été câblée. `"ask"` — documentée comme « demander à l'opérateur » — n'avait pas d'implémentation, aucun service n'utilisait la forme à identifiant libre, et depuis la v0.15.2 la config d'un service est lue dès que `detection.config_files` en nomme une, quoi que dise le champ. Le retrait est préservant, et les deux faits qui le rendent tel sont désormais vérifiés par des tests plutôt qu'affirmés en prose : aucun service livré n'a jamais porté `"auto"` sans déclarer aussi un chemin, donc retirer ce terme du lecteur ne change jamais son verdict. Le champ reste *accepté* par le schéma JSON — `additionalProperties` vaut false, le supprimer rejetterait les `~/.config/bob/services.d/*.json` déjà écrits par des utilisateurs — mais il sort de `required` et il est ignoré. Une correction attrapée par les tests existants : reformuler l'unique règle qu'il imposait en « tout service exige un port » était **plus strict** que ce qu'elle remplaçait et aurait rejeté un service qui n'en déclare légitimement aucun et lit ses ports dans sa propre config. L'invariant est désormais « un port **ou** un fichier de configuration d'où le lire ». **Item 6 du backlog — le balayage des chaînes anglaises en dur, et ce qu'il innocente.** La passe de localisation F8/F8b de la v0.11.2 se croyait complète ; la v0.15.3 a prouvé qu'elle ne couvrait pas le bac à sable des plugins. Un balayage statique de la prose anglaise a rendu 350 littéraux hors docstrings — trop pour un tri, et légitimes pour la plupart : le mauvais instrument. Le différentiel bilingue qui avait trouvé `--help` a donc été appliqué à un audit complet : 886 lignes EN contre 910 FR, 144 identiques, dont 44 en prose. Le tri laisse exactement trois catégories délibérées et aucun défaut — les commandes shell des lignes de remédiation, les références CIS portant un code numéroté (décision v0.11.2 ; les 60 non codées sont traduites), et les 38 libellés de services, dont 27 portent de la prose anglaise. Les libellés restent anglais par décision : ils servent aussi de clé aux entrées `service_risk.*` et entrent dans la ligne de base, si bien que les traduire à la source renommerait 114 entrées de locale et ferait apparaître des changements fantômes dans `--diff` au changement de langue. La promesse de localisation du tutoriel ne nommait aucune de ces exceptions ; elle les nomme désormais toutes les trois. **Item 7 du backlog — tout constat négatif est désormais explicable, sans aucune exemption.** C'était l'angle mort que la garde anti-orphelines de la v0.15.3 avait dû mettre en liste blanche : les clés `explain.*` sont construites à l'exécution, donc rien ne vérifiait que le corpus correspondait à ce que BOB émet. La mesure a demandé deux corrections. Le premier passage annonçait 5 clés listées sans entrée de locale et 3 entrées hors liste ; les huit étaient des artefacts — les entrées explain s'imbriquent sur trois niveaux (`services.exposure.open_local`) et la sonde n'en aplatissait que deux, prenant un nœud intermédiaire pour une feuille. En descendant jusqu'au premier nœud porteur de chaînes, les deux ensembles coïncidaient exactement. La seconde : sur 395 clés émises, 267 sont OK/INFO et n'ont rien à expliquer ; seules les 139 clés WARN/ALERT sont tenues au contrat — et 135 étaient couvertes. Les 4 manquantes étaient toutes `plugin.sandbox.*`, et les douze entrées de cette famille **ont été écrites** plutôt qu'exemptées : l'utilisateur rencontre la clé dans le rapport que BOB vient de produire, et `--explain` lui répondait « lancez `bob --explain list` », ce qui se lit « vous vous êtes trompé de clé ». Elles portent des références **Bonne pratique**, la catégorie que le projet emploie déjà pour les 60 constats qu'aucun contrôle CIS ne couvre, si bien que les deux gardes CIS continuent de couvrir toutes les clés sans trou. EXPLAIN_KEYS 169 → **181**, préfixes 45 → 46, et la complétion bash, la liste d'exceptions de nommage et six documents ont été resynchronisés derrière. **Item 2 du backlog — un test qui lance un binaire système doit être sautable.** La règle vient d'un échec réel : un test différentiel lisait la sortie vivante de `ss`, il est passé dix-sept fois en local et a échoué sur le runner CI, qui portait une connexion IPv6 mappée absente de la machine de développement. Le balayage a trouvé trois tests de ce type — `ss`, `openssl`, `man` — et **aucun défaut** : tous trois portent déjà un skipif, et les deux différentiels gardent des jumeaux déterministes qui maintiennent la couverture quand le binaire manque. Le jumeau de l'expiration vit dans un autre fichier, ce qui a fait passer `test_v0150_ssl_expiry_boundary.py` pour une lacune à la première lecture alors qu'il n'en était pas une. `man --warnings` n'a délibérément pas de jumeau : son propre docstring en écarte l'idée, un brouillon antérieur ayant compté les `\fI` contre les `\fR` et échoué sur une page que groff rend sans un avertissement. Un garde impose désormais la moitié mécanique — le skipif — et nomme fichier, ligne et binaire quand il manque. Savoir si un jumeau se justifie reste un jugement qu'aucun test ne peut porter. **Item 3 du backlog, volet samba — deux graphies que samba accepte et que BOB ne lisait pas.** La v0.15.2 avait vérifié que `/etc/samba/smb.conf` était le même chemin sur cinq familles de distributions ; ce que le check lit *dedans* n'avait jamais été confronté à l'analyseur de samba. `testparm` résout `writeable = yes` en `read only = No`, exactement comme `writable` — et BOB lisait `writable`, `write ok` et `read only`, mais pas la graphie avec le « e ». Un partage anonyme inscriptible écrit ainsi était rapporté `samba.guest_readonly` (avertissement, 1 point) au lieu de `samba.guest_writable` (alerte, 2 points) : la gravité d'un partage anonyme en écriture, divisée par deux, sur un mot que samba tient pour identique. `directory` est de même le synonyme de `path` chez samba, et ne lire que le second laissait le constat désigner un répertoire vide. Les deux confirmés en interrogeant testparm plutôt que la documentation, et les deux mutation-testés. Les tests écrivent un vrai smb.conf et pilotent `from_system` — la leçon de v0.15.3, où un brouillon antérieur s'assurait contre sa propre copie de l'analyse et ne tuait rien quand on réinjectait le défaut. **Item 3 du backlog, seconde moitié — BOB comprend-il les formats qu'écrivent les quinze services ?** Ils ont leur propre installeur, sont absents des dépôts de distribution, et les installer modifierait la posture de la machine qui sert de référence à l'audit : ils ne l'ont donc pas été. Ce qui reste vérifiable sans eux est la question qui décide si déclarer un fichier de configuration sert à quelque chose : onze formats — INI, variables shell, quatre dialectes YAML, JSON, XML, Caddyfile, unité systemd — un extrait chacun, tiré de la syntaxe que ces projets documentent. **Dix fonctionnaient**, y compris le JSON de transmission avec son `rpc-port` à trait d'union, dont j'avais prédit l'échec. Le onzième, **ollama**, non : sa façon documentée de changer de port est `Environment="OLLAMA_HOST=0.0.0.0:11434"` dans une unité systemd — un couple hôte:port sans le mot `port` que cherche la regex générique. Une branche pour les unités le lit désormais, volontairement étroite : seule une variable portant le nom de l'unité elle-même compte. Une règle générique `*_HOST=hôte:port` prendrait `DATABASE_HOST=db:5432` pour un port d'écoute du service, alors que c'est l'adresse de ce à quoi il *se connecte* — et un faux port d'écoute est pire qu'un port manqué, l'opérateur n'ayant aucun moyen de voir qu'il est faux. Ce jumeau est un test. **Item 4 du backlog, première dent — l'isolation des conteneurs coûte désormais des points.** Le pré-requis field-test bloquant a été levé par podman, et le détecteur a été confronté à quatre conteneurs réels avant que quoi que ce soit ne soit noté : un conteneur par défaut rapporte `privileged=False`, un jeu de capacités vide et un filtre seccomp actif ; `--cap-add SYS_ADMIN` est correctement distingué de `--privileged` ; et `seccomp=0` n'apparaît que sur le drapeau explicite. Trois constats deviennent WARN avec déduction — privileged (3), CAP_SYS_ADMIN (2), seccomp désactivé (1) — selon la règle fixée par le backlog : un **choix opérateur** est pénalisé, un **défaut éditeur** ne l'est pas. Un conteneur lancé sans aucun drapeau ne perd toujours rien, et c'est la propriété qui compte. Le reste de la section demeure INFO, y compris le système de fichiers racine inscriptible et le root sans espace de noms utilisateur, parce que podman et docker les laissent ainsi par défaut. Un noyau compilé sans CONFIG_SECCOMP ne rapporte rien plutôt que zéro, et ce cas garde son constat INFO sans déduction. La promotion a tiré tout le contrat derrière elle : les trois clés réclamaient une explication, une `nature`, et une commande de correction ou un motif documenté de ne pas en avoir — l'isolation d'un conteneur se corrige en le relançant, et aucune commande sur l'hôte ne retire `--privileged` à un conteneur déjà en cours. **Deuxième dent — parité nftables, et il s'agissait d'une déduction fausse, pas d'une déduction manquante.** Le domaine pare-feu était noté depuis le chemin iptables, tandis que le chemin nft analysait les mêmes quatre signaux avec moins de portée. nft possède deux graphies pour la règle loopback — `iif` désigne l'interface par son index, `iifname` par son nom — et il **conserve celle que l'opérateur a écrite** au lieu de normaliser, ce qui a été établi en chargeant des jeux de règles dans un espace de noms réseau et en relisant ce que `nft list ruleset` imprime réellement. BOB ne reconnaissait que `iif`, or `iifname "lo" accept` est la forme que diffusent la plupart des guides. Un hôte nftables correctement durci s'entendait donc dire qu'il n'avait pas de règle loopback et **perdait un point pour une règle qu'il avait** — pire qu'un constat manqué, puisque l'opérateur est envoyé ajouter ce qui existe déjà et que la note prétend la machine plus faible qu'elle n'est. `meta iifname` et `iifname { "lo" }` n'appellent aucun motif propre : nft les normalise. Le jeu d'essai des tests est une sortie nft mot pour mot, non écrite à la main, et les jumeaux de polarité — une autre interface, l'interface de *sortie*, une règle qui vise `lo` et rejette — sont testés aussi. **Troisième dent — deux des trois signaux cloud promus, le troisième retiré avec son motif.** Le backlog déclarait une instance cloud réelle comme pré-requis BLOQUANT. Il visait le mauvais obstacle : BOB n'ouvre jamais de socket vers le point de métadonnées, par conception — il lit `ip route get 169.254.169.254`, un mode de fichier et `/sys`. Tout cela est de l'état local, si bien qu'un espace de noms réseau avec une interface factice et un répertoire DMI simulé reproduit une instance cloud **fidèlement**, les deux polarités comprises : une route on-link (le signal) et une route par la passerelle (le faux positif que l'heuristique doit rejeter) ont été chargées puis relues par la propre fonction de BOB. La section entière a ensuite tourné sous simulation et a rapporté « Amazon EC2 » avec l'IMDS accessible, exactement comme sur EC2. Le **user-data cloud-init lisible par tous** coûte désormais 2 points — le fichier transporte mots de passe, jetons et clés SSH, cloud-init l'écrit en 0600, et tout autre mode est un choix opérateur — et une **socket orpheline liée à une adresse non-loopback** en coûte 1 : systemd répond sur ce port puis fait échouer la connexion, soit de l'exposition sans fonction. La même panne sur 127.0.0.1 reste INFO. **L'IMDS on-link sans IMDSv2 est retiré du bundle**, non reporté : le docstring de ce module consigne lui-même que l'application d'IMDSv2 « ne peut être confirmée que hors de l'hôte », et aucun banc — local ou cloud — n'y change quoi que ce soit tant que BOB refuse d'interroger le point de métadonnées. Déduire sur la seule accessibilité pénaliserait toute instance cloud pour son existence. `userdata_present` seul demeure INFO, comme l'exigeait le backlog. **Tests** 7743 → **7958**. |
| [v0.15.3](#v0153) | 02-09-2026 | **Premiers travaux v0.15.3 — le backlog laissé par v0.15.2.** Samba a renommé `min protocol` en `server min protocol` et gardé la forme courte comme synonyme déprécié : une configuration écrite aujourd'hui utilise donc la longue — et BOB ne lisait que la courte. Une machine ayant explicitement activé SMB1, le vecteur EternalBlue que BOB lève lui-même en ALERTE avec deux points de déduction, était rapportée comme l'ayant désactivé : la signature de cette campagne, où plus la configuration est à jour, plus la détection la rate. Mesuré contre `testparm`, le parseur de samba, sur sept configurations. **Et le piège `exists()` de v0.15.2 n'était clôturé que dans `bob/checks/`** : un `~/.config/bob` non traversable faisait lever `UserConfig._load` avant le moindre check — code 3, aucun rapport, aucun finding. BOB tourne en root, mais root est écrasé sur un home NFS et confiné sous SELinux. Huit sites du chemin d'audit passent par `path_exists`, `bob/_atomic.py` est dispensé à dessein car `load_baseline` a besoin de distinguer FileNotFoundError et PermissionError, et les quatre modules de sous-commandes portent leur motif consigné. **Les quinze services à installeur propre** ont ensuite vu leurs chemins déclarés confrontés au réel : le champ `binary` du registre était documenté comme chemins absolus, spécifié par le schéma comme noms nus résolus via `$PATH` *ou* chemins absolus, et implémenté comme ni l'un ni l'autre — `Path(x).is_file()` résout un nom nu contre le répertoire courant, si bien que lancer BOB depuis un répertoire contenant un fichier nommé `mongod` rapportait MongoDB comme installé. Deux fichiers de configuration déclarés étaient des répertoires, laissant le port d'openvpn non lu depuis les configurations serveur un niveau plus bas. **Et `stderr` était capturé puis jeté** : `auditctl -l` y signale un sous-système d'audit désactivé tout en sortant en 0 avec un stdout vide — indiscernable d'un système joignable sans règles, et c'est ainsi que BOB le nommait. « Les règles n'ont pas pu être listées » et « l'opérateur a coupé l'audit » sont deux énoncés différents, et seul le premier est une lacune. stderr reste du texte non fiable : confronté à un marqueur connu, jamais rendu, avec un test qui échoue s'il atteint un finding. **La passe de conception sur `config_key` n'a rien trouvé à concevoir** : le champ est déclaré de quatre façons, lu une fois, et depuis v0.15.2 ne décide rien — 33 services lisent leur config parce qu'ils déclarent un chemin, 5 retombent sur le défaut faute d'en déclarer, et le nom de la stratégie ne change rien dans aucun groupe. `"ask"` n'a jamais eu d'implémentation et aucun service n'emploie la forme à clé nommée. Les trois déclarations décrivent désormais le code, retirer le champ est mis en file comme changement de contrat, et l'invariant est verrouillé par des tests dont un garde qui échoue si un second lecteur apparaît. **La même question est ensuite allée au CLI** : deux options acceptées par le parseur avaient dérivé hors de la page de manuel — `--test-webhook`, ajoutée en v0.8.2, et `--html`, dont le jumeau `--json-full` est documenté juste à côté. Toutes deux figuraient dans `--help`, mais le manuel est la référence et v0.13.4 avait déjà réparé cinq options indécouvrables. Deux autres sont absentes à dessein et portent désormais leur motif dans le garde, qui échoue aussi sur une dispense périmée. **Puis la surface CLI elle-même a été auditée** : `-r/--reconfigure` s'est révélé le seul des 43 champs que remplit `parse_args` que rien ne lit — analysé et jeté depuis la v0.1.0, annoncé dans `--help` et deux fois dans le manuel, pendant que BOB affichait « Pour la réinitialiser : bob --reconfigure » à chaque audit trouvant une config. L'assistant que promettait son libellé n'a jamais existé : le contrat restauré est donc réinitialiser-et-sortir, avec une confirmation systématique puisque le parseur réserve `-y` à `--fix --apply`. **`--help` était un anglais identique à l'octet sous `--french`**, sur le seul écran qui annonce `--french` : le paramètre `t` dormait derrière une note « réservé pour une future i18n » depuis la v0.1.0. 83 clés par locale désormais, les drapeaux restant non traduits comme syntaxe CLI. La reformulation a fermé les sept lignes dépassant 100 caractères, dont une de cinquante, si bien que le plafond est maintenant imposé. L'exemple `--install-completion` interpolait un `argv[0]` résolu, affichant depuis les sources un fichier en 644 sans shebang, et sous pipx un chemin interne au venv. Huit formes courtes et deux alias ont été ajoutés au manuel. **Un balayage des clés de locale que personne ne référence** a ensuite trouvé trois orphelines sous `plugin.sandbox`, traduites dans les deux langues et jamais appelées parce que les conditions qu'elles décrivent étaient rapportées en anglais en dur — un francophone lisait « Le plugin 'x.py' rejeté : Plugin 'x.py' missing required run_check function », à moitié anglais et le nom en double. Le worker envoie désormais une clé et des paramètres primitifs au lieu d'une phrase rendue, `SandboxRejected` porte sa propre clé, et la passe de localisation de la v0.11.2 est étendue à une surface qu'elle n'avait pas couverte. Gardes, tous mutation-testés : aucune option analysée sans lecteur, aucune clé de locale sans référence, aucune option annoncée absente du manuel, aucune ligne d'aide au-delà de 100 caractères. **Et un jour bissextil pouvait emporter toute la section des journaux UFW.** Les horodatages syslog n'ont pas d'année : l'analyseur construit la date avec l'année courante puis recule d'un an si le résultat paraît futur. La construction était gardée contre ValueError, le repli qui suit ne l'était pas — or le 29 février existe dans une année bissextile et pas dans la précédente. Une seule ligne de ce type faisait lever `_parse_log`, et la barrière de section du runner dégradait statistiques de blocages, top des IP sources et fenêtres de force brute. Une ligne du 29 février de l'année bissextile courante, lue ce jour-là ou après, s'analyse exactement comme avant ; seul le cas futur-daté, venu d'une bissextile antérieure ou d'une horloge en retard et de toute façon filtré par le seuil, est désormais écarté. L'angle qui a ouvert la piste — la locale des sous-processus — est ressorti propre : 15 des 17 appels directs à `subprocess` hors de `_run.py` forcent déjà `LC_ALL=C`, et les deux autres ne lisent que le code retour. **Et `bob --diff` n'affichait rien du tout, depuis la v0.3.0.** L'option est annoncée dans `--help`, dans le manuel et dans les EXEMPLES, et un mode inter-machines en avait fait une fonctionnalité phare de la v0.9.0 ; elle rendait zéro octet — ni sur un changement, ni sur un système inchangé, jamais. `_silent_mode` met `config.quiet = True` pour que l'audit reste muet pendant l'affichage d'une vue post-audit, et `display_delta` écrit via `output.print_*`. Le commentaire au-dessus de la branche énonce le contrat — « display post-audit views with full output (no quiet filter) » — mais seul `--breakdown` l'appliquait : `_silent_mode` a englobé `diff_mode` en v0.3.0, le commit qui introduit `--breakdown`, et la nouvelle branche a reçu son `output.init(quiet=False)` quand l'ancienne ne l'a pas eu. Cinq fichiers de tests touchent le diff, aucun ne passait par la CLI : les composants étaient couverts, le câblage entre eux ne l'était pas. Les deux vues sont désormais exclues des modes machine : `--diff` y était accidentellement inoffensif tant qu'il était muet, `--breakdown` ne l'était pas et déversait sa table dans `bob -j | jq`. **Passe documentaire avant le tag.** `man/bob.conf.5` documentait `ssh_port = 2222` et `samba_ports = 445/tcp` comme clés de configuration supportées ; aucun code ne les a jamais lues, si bien qu'un lecteur y déclarant un port SSH non standard aurait cru le faire auditer. README_TECH décrivait une invite qui « propose de sauvegarder le port » — elle n'existe pas davantage, et la même fonctionnalité imaginaire figurait à six autres endroits dans les deux langues, plus le docstring du module `bob/config.py`. Tout est rectifié vers ce que le fichier contient réellement : profil d'audit, webhook, liste blanche SUID, répertoires de journaux. Les formats de date des changelogs étaient mélangés : les champs structurels sont en ISO côté anglais et en JJ-MM-AAAA côté français, et v0.15.2 comme v0.15.3 avaient été écrites en ISO dans le tableau et les en-têtes français, brisant une colonne uniforme depuis soixante-dix releases — désormais tenu par un garde qui vérifie aussi que les deux langues datent la même version du même jour. Pages de manuel passées à la date de release, et relecture des chaînes françaises ajoutées dans cette version. **Tests** 7560 → **7743**. |
| [v0.15.2](#v0152) | 31-08-2026 | **Deuxième correctif v0.15.x — « je n'ai pas pu regarder » ne sonne plus comme « rien à signaler ».** Un test de résistance local des correctifs v0.15.0/v0.15.1 a confirmé qu'ils tiennent face à de vrais états système hostiles, et a trouvé la seconde classe de défauts que ces versions visaient encore ouverte **à sa racine**. `_run` renvoie la sortie standard d'une commande et jette son code de retour : un binaire refusé, absent ou en échec rend donc au fond exactement la même chaîne vide qu'une commande saine qui n'a rien à dire. v0.15.0 et v0.15.1 ont fermé cette ambiguïté check par check (`rules_readable`, `status_readable`, `query_failed`, `kernel_ipv6_readable`) sans jamais toucher à la primitive partagée — 26 modules appellent `_run`, 7 gardaient le résultat. **Cinq verdicts reposaient encore sur l'ambiguïté**, tous atteignables ensemble : sur une machine avec 34 sockets en écoute, retirer `ss` du `PATH` — banal sur une image minimale ou dans un conteneur, où `iproute2` est souvent absent — produisait « 0 listening port(s) detected on this system », « aucun service IPv6 en écoute », « 0 connexion TCP établie », « n'écoute pas activement » pour chaque port du registre, et un ✔ vert dans l'encadré de synthèse, sans aucune section dégradée ni le moindre avertissement. Le nouveau `run_result()` rapporte séparément la sortie et le succès ; `_run` conserve son contrat exact pour la cinquantaine d'appels où « vide » et « en échec » mènent à la même conclusion correcte. Un balayage outil par outil (25 commandes d'inspection mises en échec à tour de rôle, verdicts comparés à une référence) ne rapporte plus aucun outil défaillant produisant une nouvelle affirmation rassurante. **Trois angles de plus**, ouverts alors que ce balayage semblait propre, ont retrouvé la classe sous trois formes : la **direction alarmante** (un `systemctl` aveuglé faisait avertir BOB qu'un sshd *en marche* était « installé mais ne fonctionne pas », avec un ✔ vert contradictoire dans le même encadré), une **commande qui réussit sans rien dire d'exploitable** (`iptables`/`nft` sortant en 0 sur une sortie incompréhensible valaient encore `firewall_drivers.no_issues`), et **les fichiers plutôt que les commandes** — un `/etc/sudoers`, `/etc/passwd`, `/etc/login.defs` ou `/etc/ssh/sshd_config` illisible produisait chacun un satisfecit, le dernier rapportant `PermitRootLogin yes` comme restreint parce que toute directive non lue retombe sur la valeur par défaut compilée d'OpenSSH. **Une troisième passe a ensuite énuméré les chemins au lieu de les échantillonner** — chaque littéral `Path("/etc…")` des checks, rendu illisible à tour de rôle — et en a trouvé trois que l'échantillon manuel avait manqués, dont un mécanisme plus subtil : `configparser.read` saute un fichier qu'il ne peut pas ouvrir *sans lever*, donc le garde `conf_readable` existant de samba.py était inerte et un `smb.conf` illisible certifiait SMB1 désactivé par un parseur qui n'avait rien lu (le même piège vidait silencieusement un profil dans `profiles.py`). Également corrigés : des fichiers cron jamais ouverts valaient encore `cron.ok`, et un `journald.conf` illisible faisait lire `Storage=volatile` comme persistant. Deux angles ont été mesurés et trouvés propres (contenu lisible mais corrompu ; commandes qui pendent au lieu d'échouer), et deux affirmations antérieures sont retirées comme faux positifs d'un regroupement par préfixe. **Une quatrième passe a pris les répertoires** que les checks parcourent plutôt que les fichiers qu'ils lisent, et a trouvé le défaut le plus grave du cycle : un `/etc/ssh` en mode 0700 faisait perdre l'audit entier — aucun rapport, aucun finding, code 3 — parce que `Path.exists()` relève EACCES au lieu de renvoyer False, depuis une collecte du cœur toujours-actif délibérément non gardée. Présent à l'identique sur le tag v0.15.1. Le nouveau `path_exists()` généralise un contournement que `ddns` portait seul depuis v0.13.x, les 30 sites d'appel sont migrés, et un test interdit tout retour à l'appel nu. **Le même principe a ensuite été appliqué à l'écriture** : `bob -d` contre un répertoire de logs en lecture seule ou plein affichait `Fatal error: Read-only file system` et s'arrêtait sans aucun finding, parce que le rapport est ouvert avant le moindre check — alors que le même module affiche déjà « Report writing disabled » et termine l'audit quand une écriture échoue une étape plus loin. Un rapport impossible à ouvrir dégrade désormais pareil. Également sondés et trouvés propres : `/proc/sys` masqué (les conteneurs réels gardent les arbres sysctl, la lecture v0.15.0 « arbre absent = IPv6 coupé » tient) et les baselines `--diff` hostiles (binaire, tronquée, types faux, forme fausse — erreurs précises, aucune trace). **Les cinq fichiers d'état que BOB écrit puis relit** ont ensuite été balayés pareil — vingt combinaisons hostiles, dix-neuf dégradant proprement. La vingtième : `config.conf` contenant des octets non-UTF-8 interrompait l'audit entier sur `Fatal error: 'utf-8' codec can't decode byte 0xfa`, sans nommer le fichier, parce que `UnicodeDecodeError` est un `ValueError` et que le garde nommait `OSError` — dans un chargeur qui documente pourtant qu'il ignore les lignes malformées. **La couche scoring a ensuite été balayée comme un jeu de propriétés** plutôt que d'exemples : le breakdown rend exactement compte du score brut (cap compris), 10/10 reste réservé à un audit sans rien à corriger, `--ignore` retire un finding **et** sa déduction sans perturber d'autre clé, et les profils `info`/`skip` retirent les points quand `alert` les conserve. Rien n'était cassé ; les invariants sont désormais des tests, avec un contrôle de mordant pour qu'ils ne dégénèrent pas en assertions sur rien. **Une dernière passe a demandé quelles classes la campagne n'avait fermées que sur les feuilles visitées** — détectable mécaniquement comme une règle vivant en plusieurs copies privées — et a retrouvé l'histoire de la grammaire UFW une version plus tard : `_split_addr_port` existe dans `ports` et dans `network_context`, même nom, comportement différent, seul le premier ayant appris les crochets IPv6 en v0.15.0. Toute adresse IPv6 échouait donc au test d'adresse privée : une connexion PostgreSQL locale via `[::1]` était rapportée comme connexion établie vers une IP externe sur un port sensible, avec une déduction de deux points. `split_ss_address()` est désormais la grammaire unique, avec un garde testé par mutation contre le retour d'une copie privée. **Puis les 30 services qu'aucune machine n'a installés** : seuls 8 des 38 du registre existent sur la machine de développement, donc les lecteurs de configuration des autres n'avaient jamais tourné — et il n'est pas besoin de les installer, `_auto_detect_port` lisant des fichiers. Le lecteur prenait les premiers chiffres suivant un mot-clé : `listen 127.0.0.1:8080;` — la façon ordinaire de lier nginx à une interface — était donc audité comme **port 127**, `listen [::]:443` ne matchait rien, et le `port 0` de Redis (TCP désactivé, réglage durci) était rapporté `0/tcp`. C'est le défaut UFW de v0.15.0 dans un troisième module. Six autres formats inconnus du lecteur ont été trouvés en balayant le reste — `http.port` d'Elasticsearch, `-p` de memcached, `listener` de mosquitto, XML de Jellyfin, adresse de site Caddy, style `<NOM>_PORT` — et **20 services couvrant sept familles de format sont désormais figés par des tests**. **La moitié « présence seule » du registre a suivi** : trois des sept portent un port dans un fichier système standard tout en étant déclarés `config_key: "fixed"`, donc BOB affirmait le défaut sans regarder — PostgreSQL sur 5432 et WireGuard sur 51820 quoi qu'ait réglé l'opérateur. Les câbler a demandé des chemins à joker (PostgreSQL versionne son répertoire), le `ListenPort` CamelCase de WireGuard et le XML `<address>` de Syncthing. Le balayage des vingt déjà couverts pour des formes moins évidentes a ensuite sorti deux cas opposés : la section `[client]` de MySQL porte le port de *connexion* et répondait en premier, et une clé `port` répétée était lue à sa première occurrence alors que redis, mysql et YAML appliquent la dernière. Une seule distinction tranche les deux — `listen`/`listener` sont additifs, une clé `port` surcharge. **La plus grosse trouvaille est venue en dernier** : bien parser ne sert à rien si l'audit n'appelle jamais le parseur, et `_resolve_ports` n'agissait que sur `config_key == "auto"`. Huit services portent un chemin de configuration sous `config_key: "ask"` — stratégie que le registre documente comme interrogeant l'opérateur, qu'aucun code n'implémente — donc nginx, apache, caddy, authelia, vaultwarden, homeassistant, adguard_home et ollama étaient tous audités sur le défaut du registre, le port réellement exposé n'étant jamais examiné. Le dispatch lit désormais dès qu'un chemin est déclaré, et le lecteur répond avec tous les écouteurs plutôt qu'un seul, puisque perdre le 443 d'un nginx servant 80 et 443 échangerait un mauvais port contre un port manquant. Également corrigés : les commentaires `;` des fichiers INI lus comme des ports vivants, et les formes JSON en chaîne, XML en attribut et YAML `!!int` invisibles. **Enfin les chemins eux-mêmes** : `nginx.conf` ne porte aucun `listen` sur une installation standard — mesuré sur les deux dispositions, l'image amont range les blocs serveur dans `conf.d/*.conf` et le paquet Debian dans `sites-enabled/*`, qui sont des liens vers `sites-available`. Le registre ne déclarait que `nginx.conf`, donc le fichier décidant du port n'était jamais ouvert ; les répertoires d'inclusion sont désormais déclarés, et une politique bornée suit un lien tant que sa cible reste dans l'arbre de configuration du service, `sites-enabled/evil -> /etc/shadow` restant refusé. **Et les chemins ne nommaient qu'une distribution quand BOB en annonce cinq** : mesuré dans des conteneurs Fedora avec les vrais paquets, `/etc/apache2`, `/etc/mysql` et `/etc/memcached.conf` n'y existent pas et `/etc/vsftpd.conf` est `/etc/vsftpd/vsftpd.conf`, donc quatre services retombaient sur le défaut du registre sur toute la famille RHEL. Les deux familles sont désormais déclarées, vérifiées de bout en bout sur des installations réelles Fedora et Debian avec leurs ports modifiés. Cinq services sur les trente-trois qui déclarent un chemin — l'angle est ouvert, pas clos. **Ce travail a ensuite révélé plus grave** : BOB décidait qu'un service était installé par `dpkg-query` seul, qui n'existe ni sur la famille RHEL, ni sur Arch, ni sur openSUSE, ni sur Alpine. Tous les services du registre étaient donc lus comme NON INSTALLÉS sur quatre des cinq distributions revendiquées — sans erreur ni plantage, la section services rapportant simplement tout comme absent. Mesuré sur un conteneur Fedora avec httpd, vsftpd, memcached et mariadb-server installés et confirmés par `rpm -q` : BOB n'en voyait aucun. La requête parcourt désormais `dpkg-query`, `rpm`, `pacman` et `apk`, les équivalents de noms de paquets ont été ajoutés au registre, et les quatre checks qui gardaient chacun un appel dpkg privé partagent une réponse unique — un garde a trouvé le quatrième pendant sa rédaction. **Les vingt-trois services empaquetés ont ensuite été mesurés sur les quatre familles** (les quinze autres livrent leur propre installeur, sans variance de distribution) : **Redis, c'est Valkey désormais** — `dnf install redis` et `pacman -S redis` laissent tous deux `/etc` vide, la configuration qui existe étant `/etc/valkey/valkey.conf` ; **PostgreSQL garde sa configuration hors de `/etc`** sur les hôtes rpm, où `initdb` l'écrit dans le répertoire de données, de sorte que le port d'un service critique n'y était jamais lu ; proftpd et Apache résident chacun à un chemin différent selon la famille. Vérifié avec des ports modifiés sur des installations réelles. **openSUSE a complété les cinq** : à base rpm mais aux noms Debian, elle garde `Listen` dans `/etc/apache2/listen.conf` — un fichier qu'aucune des quatre autres ne possède — et livre redis sous forme de template que l'administrateur copie sous un nom imprévisible, auquel un glob borné répond. Apache à lui seul nomme désormais quatre dispositions, ce qui est la forme même du problème. **Puis ce que les checks lisent dans ces fichiers au-delà du port** : SSH s'est avéré correct, la priorité de ses fragments `sshd_config.d` confrontée à `sshd -T` dans les deux sens. `ssl_certs` non — un certificat que BOB ne trouve jamais est un certificat qui n'expire jamais, et il en manquait de deux façons. Le site nginx standard de Debian est `sites-enabled/default` sans extension quand le balayage cherchait `**/*.conf`, et seul `/etc/apache2` était parcouru : les vhosts Fedora et Arch sous `/etc/httpd` n'étaient pas examinés. Mesuré avec un vrai certificat à cinq jours de l'expiration : rien trouvé avant, quatre jours rapportés maintenant. **Tests** 7288 → **7560**. |
