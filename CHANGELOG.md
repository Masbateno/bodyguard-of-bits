...
|---------|------|---------|
| [v0.17.1](#v0171) | 2026-09-09 | **A service BOB believed stopped removed its own port from the audit.** Found hours after v0.17.0 shipped, on a real Fedora VM — the first machine in this project's history where a service was actually *running* while BOB looked at it. Apache was active, enabled and listening on `*:80`, and the audit warned about it nowhere. `services.json` declared its systemd unit as `apache2`; Fedora ships `httpd`, so the check said *"installed but stopped and disabled, no immediate risk"* about a running web server — the `packages` field of the same records already listed both spellings, so the cross-distro work had been done for packages and never for units. Worse, the wrong verdict **deleted the finding that would have contradicted it**: the runner credited every service's ports as "already handled" regardless of the verdict, and the ports section skips those. A port is credited now only when the service was judged active, which is not distro-specific — any service misjudged as inactive hid its own port, on any distribution. Unit names measured with `dnf repoquery -l`. Two more defects then surfaced on a real Debian 13, both in `--fix --apply`: the 30-second timeout killed `sudo` and nothing under it, so `apt-get upgrade` kept running orphaned while BOB reported *"0 of 1 fix(es) applied"* and exited, leaving packages unpacked but not configured — fixes now run in their own session, package transactions get a budget in minutes, and a command BOB stopped waiting on is reported as *outcome unknown*, never as *apply it yourself*. And the command it proposed could not have applied a kernel update at all: detection runs `apt-get -s dist-upgrade` while the fix ran plain `upgrade`, which keeps every new package back, exits 0, and got reported as `✔ Applied` in six seconds with nothing installed. `--with-new-pkgs` installs the kernel and, unlike `dist-upgrade`, removes nothing. A fifth defect muted the SSH section on four milliseconds: `systemctl` answers in whole seconds and `st_mtime` does not, so an administrator who edited `sshd_config` and reloaded sshd — correctly — was told *"modified at 15:25, after sshd last applied its configuration at 15:25"*, two identical timestamps with one declared to follow the other, and every SSH finding below demoted to a description of a file. A file counts as newer now only when it clears the interval systemd rounded away, and the timestamps print to the second. Applying any of BOB's advice twice wrote the line twice — three runs of the rp_filter fix, three identical lines in `99-hardening.conf`; every append checks before it writes now. That thread found three more: eleven filenames for the same settings between the fixes and the prose explaining them, a `sudo tee >> FILE` whose redirection is done by the unprivileged calling shell, and three samba fixes that were no-ops because appending to smb.conf puts the line in whatever section is last — `[print$]` on stock Debian 13, where samba rejects it outright. Separately, `--install-completion` printed nine messages and translated none, including the one saying a completion only loads when a shell starts; all thirteen strings are localised and that notice is now marked as the next step. Nothing had ever checked the completion's option lists: 61 long options and 21 short ones, all verified present. On Kali, BOB called a stopped VNC server *"a potential attack vector"* and then, in the same `--explain`, told the operator to `systemctl enable --now` it — advice that makes the machine strictly more exposed. Seven such packages also cost seven points, more in aggregate than a genuinely world-exposed critical service (2 pt). Both halves came from one mistake: v0.8.0 scored these as dormant *defensive* controls, which describes fail2ban, clamav and auditd and nothing in this registry — all 38 entries listen on the network, and those three carry their own findings. Following BOB's own README (*"not a threat-modeling engine — it does not enumerate attacker paths"*), the three measured facts are stated and nothing is deducted; the finding still prints, once per service, and removal is now the first remedy offered. **Scores rise on hosts carrying dormant high-risk packages.** openSUSE Leap 15.6 then showed two things BOB could measure and did not: `User : unknown` in the report header on the one machine of three whose environment carries no `USER` — the kernel identifies the process on all three, and that header is written into the report file — and `UFW : vN/A` beside two neighbours reading `not installed`, a placeholder printed as a fact and marked as a version by all three render sites. **None of these defects is a v0.17.0 regression**; all predate it by many releases, and v0.17.0 is not yanked. **Tests** 9240 → **9483**. |
| [v0.17.0](#v0170) | 2026-09-09 | **On the whole RPM family, BOB answered yes to every package it was asked about — including names that exist nowhere.** `rpm -q nosuchpackage` prints *"package nosuchpackage is not installed"* on **stdout** and exits 1, and the query table counted any output as proof of installation — a claim written for rpm and true only of pacman and apk. v0.15.2 had replaced "everything absent outside Debian" with the same fault inverted, and nothing noticed, because this failure mode is silence: a check that believes a package is present stops asking. A Fedora container's audit went from 137 findings to 60 — 76 of the 77 that went were `services.*` rows about services that were never installed, the seventy-seventh a false password-policy deduction — and the microcode verdict said OK having checked nothing. rpm is asked with `--quiet` and judged by its exit status now. **Remediation commands follow the host's package manager too**, from one table whose names were *measured in containers*, not recalled: `sudo dnf install auditd` installs nothing (Fedora calls it `audit`) and `sudo pacman -S aide` fails outright. Where BOB has no measured name it emits **no command** and says so — a confident instruction that installs nothing is worse than an admission. **And BOB kept reading Debian's files to decide.** `/etc/pam.d/common-password` is Debian's name for the PAM password stack; the password check read it, caught the error, and concluded "no PAM quality module" — a WARN and a point — on every Fedora, RHEL, openSUSE and Arch host, having read nothing. Fedora's `system-auth` carries `pam_pwquality.so` on line 13. The stack is read wherever the distribution keeps it now, and Alpine — which has no PAM at all — gets "could not establish" instead of a deduction. A third hiding place for a Debian command turned up too: **translated prose**. `prerequisites.ufw_missing` said `sudo apt install ufw` while the same finding's `cmd` said `sudo dnf install -y ufw`, BOB contradicting itself on screen. A/B in containers: Arch does not move at all (its warning is genuinely correct), Fedora loses five findings it should never have made plus the false PAM deduction and gains two it should have, Alpine's warning becomes an honest INFO. **And BOB had never asked what machine it was on.** With no UEFI present it announced "Legacy BIOS detected" — a Raspberry Pi has no BIOS either; it boots from EEPROM. It now says only that no UEFI was found, and names the board when the firmware names one. A **Raspberry Pi section** reports what a PC does not have: a FAT boot partition, which carries no ownership of its own, holding the Imager's `userconf.txt` — a **password hash** the first boot was meant to consume, still guarding the live account. Plus the first-boot `ssh` marker and the historical `pi` account when it can actually log in (`!` or `*` in the password field is a leftover, not a way in). It deliberately does **not** test for the default password: that needs `crypt`, removed from the standard library in Python 3.13 which BOB supports. No ARM hardware was available — `qemu-user-static` needs root — so the board was simulated and every branch driven through the real collection paths in both locales. **The cron wizard also promised delivery it had never established.** `--install-cron` printed *"sendmail available — notifications will be delivered"*, and the entire basis for that sentence was `shutil.which("sendmail")` — which a Postfix installed from the distribution and never given a relay satisfies while dropping every message. The case where the promise fails is exactly the case nobody sees, because the way you would find out is the mail that does not arrive. **`bob --test-email`** sends a real message to the first saved address through `send_html_email`, the same transport the generated cron script uses, and reports what sendmail actually answered — exiting non-zero when the message was refused. It stops short of the same mistake at the other end: sendmail exiting 0 means the MTA *queued* it, so the command says accepted, and says that accepted is not delivered. **A job installed with no address told nobody**: the generated script's only channel is that email, so it wrote a report to disk on a schedule and notified no one, silently, for as long as it ran. `describe_reporting` now answers *will anyone hear from this job* once, after the network choice — a webhook in root's config is the other way out, but `--offline` suppresses the POST and the wizard offers `--offline` three steps after the address. The notice also reached the screen cut in half: `msg[:w - 3]` on eighty columns, no ellipsis, no wrap, so the operator read one sentence of two. Nine mutations cover the work; two exist because a guard was inert — one of them an AST check that passed while the screen it guarded crashed on sight, found by a pty run of the real wizard. **Tests** 8951 → **9240**. |
| [v0.16.4](#v0164) | 2026-09-08 | **"This guard bites" was a claim in a commit message, and nothing re-checked it.** Nine v0.16.3 commits made that claim; one mention of it survives in the test files. The evidence evaporated, and a guard whose anchor drifts in a later refactor goes quietly inert with the claim still standing in the history. The mutations live in `tests/mutations.py` now — 26 of them, each naming the file, the exact change, the tests that must fail and why the defect matters — and `scripts/mutate.py` replays them in CI. **The bench states its own controls on every run**: the named guards must pass unmutated, and a change that cannot matter (a space inside a docstring) must *survive*. Both times this bench has actually lied it was of the second kind — a clean, plausible table describing nothing: once it grepped `failed` in lower case against pytest's `FAILED` and called five live guards inert, once its shell wrapper never forwarded its arguments so no mutation was applied at all. It also refuses an ambiguous anchor rather than guessing, which caught two on its first run. **The audit also says how long it took, the title bar says which BOB ran, and the Debian changelog must describe the versions it names.** The duration is monotonic and starts at the checks, not at process entry; it appears on the summary line, in the archived report, and as `duration_seconds` in JSON — `null` when a run did not time itself, because `null` and `0.0` are different claims. The version is right-aligned in the cyan bar of all five wizards: sixteen places wrote row 0 by hand, so it is written once in `_chrome.draw_header` and a guard rejects any screen that paints row 0 itself. And `debian/changelog` had been bumped **three releases running** without being rewritten — 0.16.3's entry carried v0.16.0's text and 0.16.1/0.16.2 had none at all; every opening bullet must now be unique and no released version may be missing. **And the source makes claims about itself that nothing checked either.** `bob/tui/_keys.py`, `_palette.py` and `_chrome.py` each promise they never import curses; three modules import them at top level *because of* that, and the Debian `bob-core` / `bob-tui` split rests on it — load-bearing in three places, true when measured, and **never exercised**, because every machine running the suite has curses. A subprocess now imports ten modules with the import blocked, with a negative control that fails if the blocker stops blocking. Two of the three promises were also written in the modules that *rely* on them rather than the ones that could break them. **Seventeen "single source of truth" claims** — all true, none held — are pinned: one `atomic_write`, one `_EMAIL_RE`, one `_SECTIONS`, four modules sharing one fallback translator. And one claim was already false: `_build_v3` emits schema 3 and its docstring said `v2 producer` from v0.12.0 onward. Three probes over the i18n surface found nothing, which is worth stating: 1114 literal keys resolve, five families of dynamic keys produce nothing untranslated, no orphans in 2325. **And `--fix --apply` executed commands declared read-only, then called it done.** `cmd_type` says whether a command remediates or only diagnoses — its docstring is explicit, and a v0.13.2 guard validates the field at write time. `run_fixes` never read it. Six findings whose command is a diagnostic were counted as automatic fixes, **executed**, and reported `✔ Applied`: four SMART alerts answered with `smartctl -a`, a full partition with `du`, no-fail2ban-jails with `fail2ban-client status`. On a dying disk, BOB ran smartctl and told the operator it was fixed — and since smartctl's exit code is a bit field, it said "applied" on healthy disks and "manual (exit 8)" on failing ones, a verdict tracking disk health rather than repair. Declared, validated, never consumed. **Their classification is unchanged and correct** — a failing disk *is* an action and `smartctl -a` *is* a diagnostic; what was wrong is that `nature` did two jobs at once. Three buckets now, because excluding diagnostics from the fix list alone would have dropped all six from the screen. One command revised, and only its command: `fail2ban.no_jails` restated its own finding. **Twelve more were counted and could never run**: their commands carry shell operators, and BOB asked whether it could execute one only *after* the operator had agreed to all of them — `ssh.password_auth` included, so BOB announced two automatic fixes, said both would be applied, and applied `0 of 2`. The question moved to classification time, covering interactive editors too (the apply path runs with stdin closed on a 30-second timeout). And the header stopped saying "0 automatic fix(es) available" above four SMART alerts. **And the graphic conventions had never been written down at all.** `DOCUMENTS/CONVENTIONS.md` (+ FR) now holds the charter — colour on both surfaces, symbols, boxes, sanitisation, width — alongside the code conventions moved out of `README_DEV`. Measuring it before writing it found five gaps, all closed: two colours declared and never consumed; `_c` meaning the colour table in six modules and the **curses module** in another; two ways to ask whether colour is on; violet absent from curses entirely, so the same commands were violet in `bob --explain` and plain inside the wizard; and an unstated box rule. **Violet marks what must be reproduced exactly** — a command *or* a config directive — because 37 lines are `sudo nano <file> → <directive>`, both at once, and sorting the rest needed a sixty-binary regex that still left 16% unclassified. Fifteen guards hold the checkable half. **And a fix that stops to ask cannot be applied by a mode that does not ask.** Driven for real in a throwaway container: `sudo apt install ufw` exits 1 under auto-fix — apt refuses without confirmation — so `--fix --apply --yes` printed `0 of 2 fix(es) applied.` Seventeen install commands carried no non-interactive flag. With `-y` the round-trip closes: `2 of 2 applied`, and both findings are gone from the next audit — a round trip this project had never run, having only ever checked that a remediation command is *coherent*. The same container disproved the tutorial's *"confirm the score went up"*: it went 7 → **6**, because installing UFW raises "installed but inactive", which costs more. **Tests** 8572 → **8951**. |
| [v0.16.3](#v0163) | 2026-09-07 | **A hint line, a profile note and a reference manual — three claims about BOB, none checked against BOB.** **Eight passes, one shape.** **Five wizards, five key contracts, and a hint line that was English on three of them.** Each interactive screen invented its own bindings and spelled its hints out beside the dispatch: `--explain` navigated with the arrows alone, `--manage-logs` added PgUp/PgDn, the cron screens added `j`/`k`, none had `g`/`G`, and `Esc` went back on some screens and did nothing on others. The line itself was translated in `--explain` only — a French operator read French content under `↑↓: move   Enter: preview`, the class v0.15.3 closed for `--help` and left open on every screen an operator drives. One floor now, everywhere (`↑↓`/`jk`, `PgUp`/`PgDn`, `g`/`G`, `⏎`), with each screen declaring its actions once and both the footer and the dispatch derived from that declaration, so they cannot say different things. `bob/tui/_keys.py` is to the keys what `_palette.py` is to the colours, and for the same reason: three copies that happened to agree is how the palette defect stayed invisible for nine releases. **`q` exits from a first page only** — deeper in, `Esc` steps back and one keystroke cannot abandon a half-entered cron job from three screens down; six nested screens lost theirs. **Screen-specific actions appear only where they act**: `u unmark all` once something is marked, `s summary` in the log preview, `m email book` in the cron manager. **And a language switch on every landing page** — these wizards run under `sudo`, which resets the environment, so an operator whose shell is French could land in an English wizard with no way out but Ctrl-C; `l` now flips the whole screen live, labelled with the language it switches *to*. The pass also surfaced English literals the wizards had carried for releases — `[FULL]`, `line 1–20 / 340`, `scroll up`, `Confirm deletion below`, `Email address book` — all translated now. Guarded in six directions, each mutation-tested with a negative control; two of those guards were inert when first written and were repaired before shipping. **Three follow-ups**: a toggled row was red bold in `--manage-logs`, yellow in `--manage-cron` and uncoloured in both e-mail screens — it reads red everywhere now, from one `marked_attr` in the palette. `--manage-cron` dispatched Space, `a` and `m` while declaring none of them, and the address book dispatched `a` undeclared; a new guard for that direction found all four, after its first version compared double-quoted literals against `ast.unparse` output and matched nothing. `Esc back` now shows on all nine nested screens and on none of the four landing pages, which is the whole exception: a first page has nowhere to go back to. **Two more classes closed in the same release.** `--explain` closed every key without per-profile prose with "This finding applies equally to all profiles" — false for **32 keys**: `ssh.password_auth` is downgraded to INFO by `desktop`, `workstation` and `container` and was described as applying equally to all four. The display tested whether a *translation* existed and used the answer to claim something about *policy*; 71 keys do have per-profile prose, the other 116 took the uniformity line by default and 32 of those are overridden by a profile, so the claim came from an absence of explanation — this project's oldest defect class, one layer up. The line now survives only where no profile contradicts it (84 keys of 187); the other 103 render a section per profile. Those sections had their own version of the same hole: all 71 prose keys lack a `workstation` variant, and the branch rendering them skipped any profile whose prose was missing — so `--explain clamav.db_very_outdated` showed server, desktop and container and dropped the fourth. Every profile is accounted for on every differentiated key now, with its prose where one is written and a note derived from its `.conf` where none is. The guard that missed it asserted on a helper the prose branch never calls; it renders all 187 pages now. `workstation` joins `--explain` after 28 overrides of its own since v0.8.1 went unmentioned. And an agent audit of **README_TECH** returned 23 anchored claims, **20 verified and fixed** in both locales: `sudo bob -f -y` given as an example of use exits 3; `--fix` described as "propose and apply" against a dry run; auth.log brute force documented as ">10 from one IP in 60 s → ALERT −2" when it is ≥50 across the whole period, all sources, WARN, no deduction; `firewall_stack` in the JSON table three majors after the rename to `firewall_drivers`, so a consumer takes a `KeyError`; `network_context` offering a `"private"` nothing emits; `services[].ports` documented as a list when it is an object; `plugin.sandbox.error` named as *the* sandbox key when it is one of twelve; 5 required JSON keys, 19 of 43 options and 6 of 38 services with no row at all. The countable half is now guarded in both locales, each guard mutation-tested — three were inert when written, accepting `OllamaXX` and `--targetXX`. Prose stays a reading job. **And the bottom of every wizard is two bands.** The key hints were text on the last row, and two screens painted over it: `--manage-logs` replaced the whole key line with its delete confirmation — confirming a destructive action on a screen that had just hidden the cancel key — and the cron wizards drew seven text prompts at `h - 1`, on top of the footer they had just drawn, one of them showing no keys at all. The bottom is now a line reserved for prompts and status over an **orange key banner** mirroring the cyan header, white on the tool's orange, taking its colour from the same call as the cursor row. It wraps to a second row rather than truncating: at 80 columns five screens overflow in French and a `[:w]` slice cuts from the right, where the exit hint sits. It also closes a sizing defect every screen had — all of them used `h - 2`, one short whenever the hints wrapped, so the second row landed on the body's last entry. `--explain` gained matching treatment: `[ profile ]` headings in orange, brackets included, and an indented `HOW TO FIX`. **The cron wizards were still in English, and one screen showed no keys at all.** Nineteen literals survived the v0.16.3 pass — every text prompt (`Email`, `Name`, `Time (HH:MM)`, `Expression`), the hints under them, the address book's whole "add an address" screen and its statuses: a French operator drove a French wizard and typed into English fields. All translated, under a `cron_ui` namespace, with a guard that reads the source for any string still drawn on a cron screen without `t()` — `t()`'s own arguments excluded **by rule**, since `mta="sendmail"` is a program name, and whitelisting it is how a guard starts being maintained by widening. The separator is a translation too: `Nom: nightly` is not French. **`--install-cron`'s name entry drew no key hints whatever** — the one screen that never called the footer helper, so an operator was asked to type a name with nothing saying how to accept it or leave. `_curses_readline` takes the screen's actions and paints the banner itself now, so forgetting it is unrepresentable. That exposed one more: **Enter was dispatched by every text input and declared by none**, so the only advertised way out of a field was to abandon it; `SUBMIT` (`⏎ submit` / `⏎ valider`) joins the key contract. **Tests** 8328 → **8572**. |
| [v0.16.2](#v0162) | 2026-09-04 | **A ceiling claims a direction, and blindness does not always have one.** v0.16.0 reasoned that a check unable to read makes deductions that are unknown, so the score can only be too high — true while that check's domain is still scored, false when the **whole domain** leaves the average: the *denominator* changes, not the numerator, and the result moves unpredictably. Masking `/etc/passwd` drops `file_perms` (10/10) and takes the score from 7 **down** to 6, so `≤ 6` claimed a ceiling below the true value while the domain read `no action needed` — a clean pass on something BOB never looked at. Three states now: a ceiling stays `≤ N/10`; an unpredictable direction renders `~ N/10` beside the span it lies in, and that span is arithmetic rather than a guess — a domain scores in [0,10] by construction, so the missing ones at both extremes bracket the truth. It also says how much is at stake, which `~` alone does not. `score` stays an integer; `score_low`/`score_high`/`unscored_domains` are additive and `score_low == score_high` reads as "fully verified" with no special case. **The gate moved with it**: `--target` read `score_is_upper_bound`, and making that honest would have let the unpredictable run start passing CI — it reads `score_is_uncertain` now, and the guard, pinned to a source literal and edited for the third release running, is behavioural. **`--target` also left no trace in any machine output** despite gating pipelines through exit 4; `target` and `target_met` are additive in JSON, mirroring the exit condition rather than recomputing it. **The cron script stopped shipping the working tree to production** — it exported `PYTHONPATH=<package parent>` unconditionally, so a wizard run from a git checkout sent uncommitted edits to the nightly audit for ever; now emitted only when a clean interpreter cannot import `bob` on its own, failing safe. Plus four from the option sweep: `--webhook` silently skipped under `--offline` while `--test-webhook` announced it; `--ignore=KEY` validated shape but not existence, so a typo suppressed nothing for ever; the `--quiet --fix --apply` refusal gave a reason that is false with `--yes`; and `--breakdown` said "Score cap active (ceiling: 3/10)" three lines above "Final score: 7/10" without saying the cap is on the domain. **Blindness must never shrink the record of blindness** — two more of the family, found by making 23 directories untraversable on the bench. Service detection probed an absolute binary path from the always-on core, which `_sec`'s fault isolation deliberately does not cover, and `Path.is_file()` re-raises PermissionError: an untraversable `/usr/local/bin` gave exit 3 with no report at all. `path_is_file` is the twin `path_exists` never got, and its guard immediately found `is_symlink()` outside the try beside it — that one gates a trust boundary, so its failure answer is **False**. Meanwhile a section that *raised* emitted `<section>.unavailable`, in no visibility set, so hardening `/etc/cron.d` took `unverified` from 9 keys **down to 8**: failing hardest counted as better verified. Now 23 untraversable directories give a complete audit that says what it is worth — `~ 7/10`, 18 unverified keys, `between 4 and 8`. **Tests** 8282 → **8328**. |
| [v0.16.1](#v0161) | 2026-09-04 | **Two surfaces of the same claim, and only one was ever exercised.** Every defect in this release is a pair that disagreed while each half looked fine alone: the audit you run by hand and the one your cron runs; the summary line and the exit code; the README table and the binary; a field written to disk and the field read back; the plain-text wizard the tests drove and the curses wizard a human gets. **A scheduled audit ran a different audit than the one you chose.** `--install-cron` generated a wrapper whose audit line was fixed at `bob --quiet --detailed`. Cron runs it as root, so it read *root's* saved profile, not the operator's — and cron's bare environment, where `$LANG` is unset, so a French installation mailed an English report every night. The profile is not cosmetic: it decides which findings deduct, therefore the warning count, therefore the exit code, therefore whether the notification email is sent at all. The wizard now asks three closed questions — profile, language, outbound probes — on three screens in both the plain and curses flows, each opening on the installing session's value, and prints the resulting command line before writing anything. Every dimension is emitted explicitly even at its default: the point is that the scheduled audit inherits nothing. `--verbose` was rejected on measurement, not reading — under `--quiet` it changes the log by zero lines. The option string lands in a root-owned script, so it is built from closed sets in one module, re-validated at the write site, and all sixteen combinations are round-tripped through the real parser: a regex proves the shape, only the parser proves it works. **Installed crons are untouched** — regenerate with the same name to migrate one. **A French audit's report opened and closed in English** — `[INFO] Starting audit` and `[NEXT STEPS]`, found in a real run; the first was written as a literal eight lines after the same event was printed translated to the terminal. A guard now rejects any operator-visible literal written to the text report. **The interactive wizard had been crashing since v0.7.0** — `'_WizardEntry' object has no attribute 'time_simple'`, raised the moment a schedule was picked, on 40 released versions. `run_install_cron` dispatches on `sys.stdout.isatty()`, so every test and every verification in this release drove the plain-text wizard instead: a pipe is not a TTY. The path a human gets had no coverage at all. Closed by a static sweep over every `entry.<field>` the wizard reads, plus an end-to-end run on a real pty. **The cursor row looked like a second banner** — the selected row and the top title bar were both on `COLOR_CYAN`, on all four curses screens. Selection is now orange (xterm-256 index 208, falling back to `COLOR_YELLOW` at 8 colours). The chart had been written out three times, once per module; it now lives once in `bob/tui/_palette.py`, with a guard rejecting any `init_pair` outside it and another asserting the selection background is never the banner background. **v0.16.0 closed one half of its own class, and the other half was live.** It reasoned that a check unable to read makes no deductions, so the score can only be too high — true of a check abstaining inside a domain, false of a whole domain: the global score is an average over **active** domains, and a domain whose input went unreadable leaves the average, changing the denominator. Masking `/etc/passwd` takes `file_perms` (10/10) out and the score from 7 down to **6**, deductions byte-identical, nothing in `unverified` — read against a sighted baseline as `Score degraded by 1 point(s)`, a regression on a host where nothing changed but BOB's eyesight. Found by an exhaustive local option sweep (~700 invocations). Three defects, one root: `user_accounts.no_passwd` / `no_shadow` sat outside `VISIBILITY_KEYS` because the set was enumerated **by name** and neither ends in `_unreadable`, so a third guard now sweeps **by position in the source** (branch body only — reading the `else` reports the honest readable path as a limit); `load_baseline` never restored `unverified`, so `prev.unverified` was always None and **the entire `--diff` visibility protection was dead outside the unit tests**, which never round-trip through the file; and there was no branch for a score that *falls* on a blinder run — a genuine drop is still a warning, and that is its own test. **`--target` said the opposite of its own exit code**: v0.16.0 made the gate fail closed on a bounded score and left the summary printing `✔ target reached`, so one run told a human it passed and a CI it failed — and on any host with one unreadable section, *every* `--target` exited 4, `--target=1` included. **Still open, deliberately**: the headline renders `≤ 6/10` where the sighted score is 7 — when blindness removes a whole domain the score can move either way, so naming that state is a design decision, deferred rather than guessed at. **Tests** 8152 → **8282**. |
| [v0.16.0](#v0160) | 2026-09-04 | **The score went up when BOB could see less.** It is a sum over the checks that ran, and a check unable to read its input made deductions that are *unknown, not zero*: masking `/etc/ssh/sshd_config` removes four and moves the score from 7 to **8**. `degraded_sections` stayed empty throughout — correctly, it is reserved for a section whose check *raised* — so a clean abstention left no trace in the machine output at all, and unprivileged on this host **eight** sections cannot be fully read. A caveat beside the score is not blunt enough, because the number is what gets read: it now renders as the ceiling it is (`≤ 8/10`), the risk level derived from it reads as a best case, the trend arrow is withheld while bounded, and a summary line names how many sections were not fully read. Deducting for unreadability was rejected — it penalises the operator for BOB's own privileges. `score` stays an integer in JSON; `score_is_upper_bound` and `unverified` are additive. `--diff` no longer calls a finding resolved when its section was not re-evaluated, and a baseline predating the field reads as **unknown**, never as "everything was visible then". The 32 visibility keys live in one explicit set, guarded in both directions. **`logs.brute_found` is renamed** and the old name keeps working. v0.15.5 fixed what the operator sees and left the key, because a rename breaks baselines, `ignore.yml` and `--explain` — a planned bundle's work, and this is the bundle. It joins its two siblings as **`blocked_repeat_public`**. Each back-compat surface fails differently when forgotten, so each has its own test: an unremapped baseline reports one issue as both resolved and new, an unconsulted ignore.yml lets a waived finding return, a missing alias turns `--explain` into exit 3. The two baseline migrations compose in order. `auth_log.brute_force` is deliberately untouched — a different key whose evidence does support the word — and that is a test. **A qualification pointing at a silenced finding** is stripped: the residue v0.15.5 recorded, where ignoring a caveat left every finding it qualified pointing at something JSON no longer emits. **One truth, six renderers, two updated.** The archived report said `Score : 7/10` where the terminal said `≤ 7/10`, about the same audit — and Markdown, HTML, CSV and the webhook were silent too. All six carry it now, each in the shape its consumer takes, and **the guard is the actual fix**: a new sink must declare itself. The guard immediately found that **`--target` could be satisfied by a ceiling**, so an audit run without the privileges it needs made a CI gate go green — a gate fails closed now. The report also gained the **audit profile**, which changes severities and the exit code. And **`bob -q -d` wrote a 440-line report with no summary block at all** — `-q` is about stdout, `-d` is a file explicitly asked for, and display.py already stated the rule that the call site broke. Fixing that introduced the v0.8.3 UnboundLocalError shape, caught by the tests and now by a static guard — which itself had to be written twice, walking past a branch with `continue` while `ast.walk` yields descendants regardless. **A section absent from the report had three possible meanings** — filtered, degraded, or clean — and all three looked identical in an archived file. A **Scope** line now names the first two, separately. `degraded_sections` had reached the summary as a parameter read nowhere, the declared-but-never-consumed shape, introduced and caught in the same session. **`python3 bob` answered with a bare traceback** — running the package directory as a script leaves its parent off `sys.path`. The form stays unsupported; BOB now names what happened and the two forms that work. The string is hardcoded English because `bob.i18n` is exactly what cannot be imported there, and only a missing `bob` is caught so a missing dependency still raises. **`--format=html` crashed and the whole suite was green** — the bound was applied by reassigning `score` to a string, above a `_score_color(score)` that compares it against thresholds. The HTML stub defaults the bound to False, so the bounded path was never rendered; it was found by running the binary. A source-presence check is not a rendering check, so the three builders that had only one are now driven in both states. **An exhaustive option pass found `--install-cron` broken by this release**: the generated email script pulls the score out of the log with a pattern that `≤` defeats entirely, so the subject silently became `Score N/A` on any bounded host. **And SECURITY.md named a sink that does not exist** — it attributed `html.escape` plus `_safe_url` to "the `-d` HTML report", but `-d` writes plain text; the control belongs to the cron email report. All 45 long options were exercised, and `FORCE_COLOR` was added to the environment table it had been missing from. **Tests** 8089 → **8152**. |
| [v0.15.5](#v0155) | 2026-09-03 | **Five hunting angles on one question: is what BOB says verifiable?** Non-breaking — on a privileged host the audit is byte-identical to v0.15.4. The first angle: "not installed" and "nothing could ask" were the same answer. Found with a new instrument: a full audit run in a namespace with every external tool stripped from PATH. A blind audit should say "I could not look" almost everywhere; it reported **12 deductions and zero degraded sections**, announcing a successful audit. Most survived honestly — `backup`, `log_rotation`, `mac_policy` and `ntp` detect by the absence of the tool *itself*, so "no backup tool found" is a true statement. One did not. `package_installed()` answers None both when a package is absent and when **no package manager exists to ask**, and `firmware` turned that into "CPU detected but no microcode package installed" — a WARN with a point deducted for a negative it never established, on Gentoo, NixOS, Void, Slackware or any minimal image. `package_query_possible()` now lets a check say it could not look: the finding degrades to INFO, costs nothing, and names the reason. The polarity twin is a test — a genuinely missing package still warns and still deducts. A guard requires every caller that turns a package query into a statement to consult the helper, with three documented exemptions that rest on independent evidence (a config file, a snap, a binary on PATH). **Then the instrument was declined tool by tool** — PATH rebuilt from symlinks, exactly one binary removed, twenty-nine times. Twenty-six stay silent correctly. Removing `aa-status` alone made BOB report `mac_policy.apparmor_inactive` — WARN, one point — on a host whose AppArmor module is loaded and enabled: the snapshot set `apparmor_active = False` in the branch commented "module loaded but aa-status not available", turning a missing tool into a control that is not enforcing. The kernel answers without the tool — `/sys/module/apparmor/parameters/enabled` reads Y and is world-readable, and securityfs only mounts the apparmor directory while the LSM is live — so the check now reports "active, profiles could not be read", the INFO finding that already existed for the partial-privilege variant of this same defect. Two guards caught mistakes of mine on the way: the v0.15.2 bare-`.exists()` guard rejected the first draft, and the tests themselves were rewritten after a first version patched `_run._command_exists` while `mac_policy` holds its own reference — it reported the fix working without ever exercising it. **The instrument has a second variant, and it caught both of the fixes above being half-done.** Masking a binary is not how a tool fails in practice — running BOB without sudo is. The tool is then *present*, so `_command_exists` says yes and every no-tool branch is skipped; the command simply answers nothing. Replacing each binary with a stub that refuses found `aa-status` reporting AppArmor inactive again, through the branch the first fix never reached, and `dpkg-query` reviving `firmware.microcode_missing`. The package case is the sharper one: with an unreadable database dpkg exits 1 with `no packages found matching <pkg>` — the **same** exit code and message as a package that genuinely is not installed, so the distinction cannot be recovered from its output. What can be established is whether the layer answers at all, so BOB now asks each manager for a package it necessarily owns — itself — and does not trust a negative until it has produced a positive. Twenty-nine tools × two failure modes = fifty-eight audits; after these fixes only `systemctl` still turns its own absence into a verdict, and that one is diagnosed, not fixed. **The instrument was then turned on files rather than binaries.** Existing paths were masked one at a time so that reading them raises `PermissionError`. `/proc/sys/vm/swappiness` produced `memory.swappiness_ssd_wear` — a WARN with a deduction — against a threshold of 30, on a host whose real value is 10: the reader returned a hardcoded `60` when it could not read, and its own docstring said so. A helper that answers a plausible number when it failed is indistinguishable from one that measured it, and 60 being the kernel default made the invented value the one most likely to be believed. The reader now answers `None`, the three comparisons that consumed it are guarded — the third was found only because the new test crashed on it — and the check states what it could not read rather than falling silent. A shape guard rejects a numeric literal returned from an `except` in any reader, narrowed after a first version flagged four `return 1` statements in cron installers that are exit codes, not measurements: a guard that reports noise gets switched off. **A boundary test whose boundary moved:** the suite crossed midnight during this work and an account-expiry test failed on a tree where nothing was wrong, because it computed "today" at import while the check read the clock at run time. The clock is pinned on both sides, verified by moving the pin and watching the production verdict follow. **An unreadable `/etc/default/ufw` was read as "UFW manages IPv6".** Same instrument, same class, one more file: on this host that file says `IPV6=no`, and losing the read invented `True` so every uncovered IPv6 listener became `ipv6.port_no_v6_rule` — a deduction for rules missing from a firewall told not to manage IPv6. The distinction is **absent versus unreadable**: an absent file is ufw's own default and answering True there is a measurement; an unreadable one is no answer. Both modules that read it now answer None, and wiring that took three passes, each now a test — `not None` is True, so the "kernel has IPv6, UFW does not" branch fired on an unknown and cost two points; falling through reached the `else` that states the two sides agree; and the closing OK sits *outside* the chain, so the audit still certified "all listeners are covered" against a policy never read. A clean bill of health drawn from an unread file is worse than a false negative, because nobody goes looking for it. Masking `/etc/sudoers` and `/var/lib/clamav/daily.cld` also moved verdicts and both are **bench artefacts** — `mount --bind` replaces the file, so the mode and mtime read are the mask's own — established after an unmasked control run in the same namespace was checked against the baseline. **The diagnosed-not-fixed list, items A and B** — the same defect as the rest of this release, reached from another direction. **A:** `unit_active_state` returns a tri-state precisely because `is_unit_active` collapsed "inactive" and "systemd never answered" into one False; the *fallback* underneath had the identical collapse — `_run("auditctl", "-s") or ""` — so with neither systemd nor auditd able to answer (running without sudo, where the binary is present and every no-tool branch is skipped) BOB warned that a running auditd was stopped, with a deduction and an order to start a service already running. fail2ban carried a copy. Both now answer None and report `state_unknown`, INFO. **B:** `query_failed` consulted one binary, so an nftables host with no iptables binary had nft refuse and got `firewall_iptables.no_backend` — three points, the largest deduction in the check, plus "apt install iptables" on a machine whose backend is running. The discriminator was established against nft itself: unprivileged it exits **1** with empty stdout, against a genuinely empty ruleset it exits **0**, also empty — stdout cannot separate them, the exit code can, deliberately the opposite of the choice documented for systemctl. Splitting the two revealed a third state hiding in the first: nft answering with no input hook is a *measurement*, so it keeps its three points under `firewall_iptables.nft_no_input_filter`, whose remedy names the backend the host has — false advice on top of a true finding is still false advice. **A backup deduction that rested on a name in PATH** — the mirror of the rest of this release: an observation that exists, and a verdict claiming more than it carries. All nine backup branches opened with `_command_exists`, so a claim about whether backups are *configured* rested on whether a binary was on PATH. **Reproduced on the bench first:** a mirrored PATH minus the nine known binaries produced `backup.no_backup`, −1, and adding a real backup — `/etc/cron.d/rsnapshot` plus a `nas-backup.timer` running duplicity to a NAS — did not move it, while the cron snapshot in the same namespace returned that very file with its source path. The tighter case needs no unknown tool: this host carries `/etc/timeshift/timeshift.json`, a path the module already holds as a constant, and it was never consulted because artefacts sat *behind* the binary gate — inverting the evidence hierarchy, since a config file states more about configuration than a name on PATH. Artefacts are now read without the gate and scheduled jobs scanned for a named backup tool; the deduction is withdrawn and an INFO names what was seen. It does **not** claim backups are fine — it withdraws the claim that none is configured. The dangerous half is the opposite error, so the scan is a list of binary names, not a fuzzy match: bare `rsync` is deliberately excluded, comments are skipped, matching is on word boundaries, and an unreadable artefact is not evidence. Two messages stopped overstating their reach. **`file_integrity` was examined and left alone** — it shares the gate but its no-tool verdict is a zero-point INFO that already names its own scope; an earlier estimate putting it in scope was wrong. **"Bruteforce attempts", from packets the firewall had already dropped** — the over-interpretation angle closed. A UFW BLOCK never reached a service, so no credential was offered and repeated credential guessing cannot be read from it; the sibling that reads real failed logins from auth.log reports a count, so the module with weaker evidence made the stronger claim. **Reproduced on the bench first** with a crafted ufw.log: a LAN NAS making two TCP attempts to 445, an IoT device doing SSDP on 1900/udp, and an external scan of 22/tcp produced **three deductions, three points**, all called brute force — and `local_dominance`, the exception hand-carved for these false positives, did not fire at 27 of 67 blocks. The threshold counted packets, and with `tcp_syn_retries` at 6 one blocked connect emits six SYNs, so two ordinary attempts crossed a threshold of ten. The discriminator was in every log line and never read: a retransmission reuses its source port, a new attempt does not. Local and UDP sources are now reported without a deduction — a private address is the operator's own device, and 1900 SSDP has no credential to guess. Reclassifying nearly made BOB fall silent, which would be worse than the false positive: `display.py` rendered WARN only, caught by re-running the bench. One guard had to be written twice, the first version staying green when the INFO branch was deleted. Three deductions became one, stated as the measurement. The key `logs.brute_found` is deliberately unchanged — renaming it would break baselines and `--explain` on a patch. **BOB audits the file, an attacker meets the process** — a third axis, asking not whether the evidence supports the verdict but what the verdict is *about*. The SSH findings are phrased as behaviour ("Root login via SSH **is** permitted") and read from sshd_config, which sshd loads at start and on reload; 24 of the tool's points ride on that file and 3 on journald.conf. The dangerous direction is the quiet one: a file just hardened on a daemon still running the old settings yields a clean section with no finding for a caveat to hang off — the `backup` problem again. **The reload semantics were established against systemd, on a disposable user unit:** after `systemctl reload` with the main PID unchanged, ExecMainStartTimestamp and ActiveEnterTimestamp both stayed at the original start and only **StateChangeTimestamp** moved. Comparing against either of the others would have flagged every administrator who edited and reloaded correctly — a guard that fires on people doing the right thing gets switched off. That property is now pinned by a test reading the arguments handed to systemctl, after the first mutation round found nothing when it was swapped. BOB compares the newest mtime across every file the parser opened, Include'd drop-ins included, and says so when the file wins; it costs nothing and unknown stays unknown. **samba was examined and excluded on smbd(8)'s own wording** — it reloads smb.conf every three minutes, so the divergence window is bounded; PAM and login.defs are read per authentication, sysctl is read from /proc/sys, and eight modules already query the live tool. **"All SUID binaries are known-safe", over a walk find could not finish** — the aggregation axis: a verdict quantifying over a set BOB has not seen in full. `suid_audit` read find's stdout and never its return code. **Established against the binary BOB actually invokes**, after a first probe measured the wrong one — this host aliases `find` to `bfs` in the shell, while `subprocess.run` resolves to `/usr/bin/find`, GNU findutils 4.9.0: over a tree with an unreadable subdirectory it exits **1**, names the directory on stderr, and still prints what it reached, so a partial walk is indistinguishable from a complete one on stdout alone. The ordinary case is running BOB without sudo — unprivileged here the scan reaches 22 SUID and 13 SGID and cannot enter every directory, and the audit answered "All SUID binaries are known-safe (22 SUID, 13 SGID)" over exactly that. It now names what it saw; the empty variant, which produced "0 SUID, 0 SGID" as a pass, is separated out as a skipped scan. Partial cuts the positive claim and never the negative one. Two guards had to be repaired first: a stat stub taking a bare `(path)` made two mutations *error* rather than fail, and a mutation count grepping for `FAILED` read that as the guard holding — the second time this release that a guard reported confidence it had not earned. **A qualification that stayed behind while the finding it qualified travelled** — the composed-verdict axis. `correlation.py` builds findings from other findings' keys (six rules, three of them ALERT), and a qualification lived only as a sibling entry in a flat list: `Finding` had no field linking one to another. Demonstrated before the change — on a host whose sshd_config was hardened but never reloaded, `corr.root_no_protection` fired as ALERT with no trace of the caveat the same audit had just printed, and **three of six rules consume the two keys that caveat qualifies**. Most of the layer was already sound and kept the fix small: `active` is filtered to WARN/ALERT, so every caveat in this release that *replaces* a verdict needs nothing here, and `corr.password_auth_under_attack` was checked clean (it reads real failed logins). The defect is the unfinished half of work landed three commits earlier, not a regression. `Finding.qualified_by` plus `CheckResult.qualify(key, since=…)` — an index rather than a key list, which would drift the first time a directive is added — and the qualifier never qualifies itself, which would recurse through every consumer. **The other key consumers had the same blindness**, which decided the structural fix over patching correlation alone: `json_output` and `webhook` now emit `qualified_by` additively, and the composed finding carries the qualifier's *message*, already a sentence written for a human. `compare` is deliberately untouched — `finding_keys` is a baseline contract. **A guard that failed on edits with no defect behind them** — `test_fix_coverage` addressed its registered dispatch sites by `(file, line number)`, so every insertion above one failed the suite for nothing. Its own NOTE recorded v0.15.0 moving them twice; this release moved the ssh entry twice more in a single day. Same failure mode as a guard reporting unearned confidence, from the other side: one cries wolf, the other sleeps, both end up ignored. Sites are keyed by `(file, enclosing function)` now — verified by inserting five lines into both registered files and watching it stay green — with a second guard for the new, quieter failure mode, a registry entry naming a function that no longer exists. **Tests** 7958 → **8134**. |
| [v0.15.4](#v0154) | 2026-09-02 | **First v0.15.4 work — the GitHub release title was truncated mid-phrase on every release.** ⚠️ **Patch number, BREAKING scoring changes** — deliberate versioning exception to empty the backlog in one pass; see the note in the full changelog. `publish.yml` built the name as `vX.Y.Z — <headline>` by taking the CHANGELOG.md table cell and cutting at the first ` — `. The cell opens with a bold lead that contains em dashes of its own, so the cut landed inside the bold span and left its opening `**` unbalanced: every release from at least v0.14.0 to v0.15.3 shipped as, literally, `v0.15.3 — **First v0.15.3 work`. The title now takes the span between the first pair of `**`, with a first-sentence fallback for a row without a bold lead. The length cut went with it: `cut -c` counts **bytes** in GNU coreutils, so a lead containing em dashes was truncated shorter than intended and could split a multibyte character outright — the same byte-versus-character trap the v0.15.3 help-width work had already met. The cap now lives on CHANGELOG.md, where a human can shorten a lead, rather than mutilating the title at the sink. The guard runs the workflow's own shell block instead of reimplementing it, since a guard that reimplements what it watches stops testing it the moment either side drifts; reinjecting the old cut fails it on all twelve documented versions at once. **Backlog item 5 — every public function must have a caller.** Third instance of the campaign's highest-yield pattern: `strip_rule_index` in `checks/_ufw.py` was born without one and never even exercised by a test, and `print_port_detail` had been unused since v0.1.0, kept alive only by its own test. Both removed rather than allowlisted — a guard that ships with exemptions is weakened on day one. Writing a sweep that does not lie took three attempts, and both mistakes are pinned in the guard: an aliased import (`read_line as _rl`) hides the real name unless both are counted, and a `FunctionDef` produces no `ast.Name`, so the threshold is `>= 1` — getting that wrong flagged every singly-called function and inflated the list from 2 to 17. The two sibling surfaces came back clean: all 11 `services.json` fields are read, and all 25 JSON root keys are documented. **EXAMPLES alignment**: the two long commands overflowed the 30-column gutter and pushed their text 6 and 13 columns right of the rest; the column now fits the longest command. **Backlog item 1 — `config_key` retired from the service registry.** The field declared a port-resolution strategy in four forms; none was ever wired. `"ask"` — documented as prompting the operator — had no implementation, no service used the free-form identifier form, and since v0.15.2 a service's config is read whenever `detection.config_files` names one, whatever the field said. Removal is behaviour-preserving and the two facts that make it so are now asserted, not asserted in prose: no bundled service ever carried `"auto"` without also declaring a path, so dropping that term from the reader never flips it. The field stays *accepted* by the JSON schema — `additionalProperties` is false, so deleting it would reject the `~/.config/bob/services.d/*.json` files users have already written — but it is out of `required` and ignored. One correction caught by the existing tests: restating the lone rule the field enforced as "every service needs a port" was **stricter** than what it replaced and would have rejected a service that legitimately declares none and parses its ports from its own config. The invariant is now "a port **or** a config file to read one from". **Backlog item 6 — the hardcoded-English sweep, and what it cleared.** The v0.11.2 F8/F8b localisation pass believed itself complete; v0.15.3 proved it had not covered the plugin sandbox. A static sweep for English prose reached 350 literals outside docstrings — too many to triage and mostly legitimate, so the instrument was wrong. The bilingual differential used on `--help` was applied to a full audit instead: 886 lines EN against 910 FR, 144 identical, 44 of them prose. Triage leaves exactly three deliberate categories and no defect — shell commands in remediation lines, CIS references carrying a numbered code (the v0.11.2 decision; the 60 uncoded ones are translated), and the 38 service labels, 27 of which carry English prose. The labels stay English by decision: they also key the `service_risk.*` entries and enter the audit baseline, so translating them at the source would rename 114 locale entries and make `--diff` report phantom changes on a locale switch. The tutorial's localisation promise named none of these exceptions and now names all three. **Backlog item 7 — every negative finding is now explainable, with no exemptions.** The blind spot the v0.15.3 orphan-locale guard had to allowlist: `explain.*` keys are built at runtime, so nothing verified the corpus matched what BOB emits. Measuring it took two corrections. The first pass reported 5 listed keys with no locale entry and 3 locale entries outside the list; all eight were artefacts — explain entries nest three levels (`services.exposure.open_local`) and the sweep flattened two, mistaking a middle node for a leaf. Descending to the first node carrying string values, the two sets matched exactly. The second: of 395 emitted keys, 267 are OK/INFO with nothing to explain, so only the 139 WARN/ALERT keys are held to the contract — and 135 were covered. The 4 gaps were all `plugin.sandbox.*`, and the twelve entries of that family **were written** rather than exempted: a user meets the key in the report BOB just produced, and `--explain` answered "run 'bob --explain list'", which reads as "you mistyped". They carry **Best practice** references, the category the project already uses for the 60 findings no CIS control covers, so both CIS guards keep covering every key without a hole. EXPLAIN_KEYS 169 → **181**, prefixes 45 → 46, and the bash completion, the naming-convention exception list and six documents were resynchronised behind them. **Backlog item 2 — a test that runs a system binary must be skippable.** The rule comes from a real failure: a differential test read live `ss` output, passed seventeen times locally, and failed on the CI runner, which happened to hold an IPv6-mapped connection the dev host did not. The sweep found three such tests — `ss`, `openssl`, `man` — and **no defect**: all three already carry a skipif, and the two differential ones keep deterministic twins so the logic stays covered when the binary is absent. The expiry twin lives in another file, which is why a first reading of `test_v0150_ssl_expiry_boundary.py` looked like a gap and was not one. `man --warnings` deliberately has no twin: its own docstring rejects the idea, an earlier draft having counted `\fI` against `\fR` and failed on a page groff renders cleanly. A guard now enforces the mechanical half — the skipif — and names the file, line and binary when one is missing. Whether a twin is warranted stays a judgement no test can make. **Backlog item 3, samba half — two spellings samba accepts and BOB did not read.** v0.15.2 verified `/etc/samba/smb.conf` was the same path on five distribution families; what the check reads *inside* it had never been confronted with samba's own parser. `testparm` resolves `writeable = yes` to `read only = No`, exactly like `writable` — and BOB read `writable`, `write ok` and `read only` but not the spelling with the "e". A world-writable anonymous share written that way was reported as `samba.guest_readonly` (WARN, 1 point) instead of `samba.guest_writable` (ALERT, 2 points): the severity of an anonymous writable share, halved, on a word samba treats as identical. `directory` is likewise samba's synonym for `path`, and reading only the latter left the finding naming an empty directory. Both confirmed by asking testparm rather than the documentation, and both mutation-tested. The tests write a real smb.conf and drive `from_system` — the v0.15.3 lesson, where an earlier draft asserted against its own copy of the parsing and killed nothing when the defect was reinjected. **Backlog item 3, second half — does BOB understand the formats the fifteen services write?** They have their own installers, are absent from the distribution repositories, and installing them would change the posture of the host the audit uses as its reference, so they were not installed. What is checkable without them is the question that decides whether declaring a config file buys anything: eleven formats — INI, shell env, four YAML dialects, JSON, XML, Caddyfile, systemd unit — one snippet each, taken from the syntax the projects document. **Ten worked**, including transmission's JSON with its hyphenated `rpc-port`, which had been predicted to fail. The eleventh, **ollama**, did not: its documented way to move its port is `Environment="OLLAMA_HOST=0.0.0.0:11434"` in a systemd unit — a host:port pair with no `port` keyword for the generic regex. A unit-file branch now reads it, deliberately narrow: only a variable named after the unit itself. A generic `*_HOST=host:port` rule would read `DATABASE_HOST=db:5432` as a port the service listens on, when it is the address of something it connects *to* — and a false listening port is worse than a missed one, because the operator cannot tell it is wrong. That twin is a test. **Backlog item 4, first tooth — container isolation now costs points.** The blocking field-test prerequisite was lifted by podman, and the detector was confronted with four real containers before anything was scored: a default one reports `privileged=False`, an empty capability set and an active seccomp filter; `--cap-add SYS_ADMIN` is correctly told apart from `--privileged`; and `seccomp=0` appears only on the explicit flag. Three findings became WARN with a deduction — privileged (3), CAP_SYS_ADMIN (2), seccomp switched off (1) — on the rule the backlog set: an **operator choice** is penalised, an **editor default** is not. A container started with no flags at all still scores zero, which is the property that matters. The rest of the section stays INFO, including the writable root filesystem and root-without-userns, because podman and docker leave those that way out of the box. A kernel built without CONFIG_SECCOMP reports nothing rather than zero, and that case keeps its own INFO finding with no deduction. Promotion pulled the whole contract behind it — the three keys needed an explain entry, a `nature`, and a `cmd` or a documented reason for having none: a container's isolation is fixed by re-launching it, and no command run on the host can undo `--privileged` on a container already running. **Second tooth — nftables parity, and it was a false deduction, not a missing one.** The firewall domain was scored from the iptables path while the nft path parsed the same four signals with less reach. nft has two spellings for the loopback rule — `iif` matches the interface by index, `iifname` by name — and it **preserves whichever the operator wrote** instead of normalising to one, which was established by loading rulesets into a network namespace and reading back what `nft list ruleset` actually prints. BOB matched `iif` only, and `iifname "lo" accept` is the form most guides ship. A correctly hardened nftables host was therefore told it had no loopback rule and **lost a point for a rule it had** — worse than a missed finding, because the operator is sent to add something already there and the score claims the machine is weaker than it is. `meta iifname` and `iifname { "lo" }` need no pattern of their own: nft normalises both. The fixture in the tests is verbatim nft output, not hand-written, and the polarity twins — another interface, the *output* interface, a rule that matches `lo` and drops — are tested too. **Third tooth — two of the three cloud signals promoted, and the third retired with its reason.** The backlog called a real cloud instance a BLOCKING prerequisite. It was aiming at the wrong obstacle: BOB never opens a socket to the metadata endpoint, by design — it reads `ip route get 169.254.169.254`, a file mode and `/sys`. All of that is host-local state, so a network namespace with a dummy interface and a simulated DMI directory reproduces a cloud instance **faithfully**, both polarities included: an on-link route (the signal) and a route via the gateway (the false positive the heuristic must reject) were loaded and read back through BOB's own function. The full section was then run under simulation and reported "Amazon EC2" with IMDS reachable, exactly as it would on EC2. **World-readable cloud-init user-data** now costs 2 points — the file carries passwords, tokens and SSH keys, cloud-init writes it 0600, and any other mode is an operator choice — and an **orphan socket unit bound to a non-loopback address** costs 1: systemd answers on that port and then fails the connection, which is exposure without a function. The same breakage on 127.0.0.1 stays INFO. **IMDS-on-link without enforced IMDSv2 is dropped from the bundle**, not deferred: this module's own docstring records that IMDSv2 enforcement "can only be confirmed off the host", and no bench — local or cloud — changes that while BOB refuses to query the endpoint. Deducting on reachability alone would penalise every cloud instance for existing. `userdata_present` alone remains INFO, as the backlog required. **Tests** 7743 → **7958**. |
| [v0.15.3](#v0153) | 2026-09-02 | **First v0.15.3 work — the backlog v0.15.2 left.** Samba renamed `min protocol` to `server min protocol` and kept the short form as a deprecated synonym, so a configuration written today uses the long one — and BOB read only the short one. A host that had explicitly enabled SMB1, the EternalBlue vector BOB itself raises as an ALERT with a two-point deduction, was reported as having it disabled: the signature of this campaign once more, where the more current the configuration the likelier the miss. Measured against `testparm`, samba's own parser, across seven configurations. **And the `exists()` trap of v0.15.2 was only fenced inside `bob/checks/`**: a `~/.config/bob` that cannot be traversed made `UserConfig._load` raise before a single check ran — exit 3, no report, no findings. BOB runs as root, but root is squashed on an NFS home and confined under SELinux. Eight audit-path sites now go through `path_exists`, `bob/_atomic.py` is deliberately exempt because `load_baseline` needs FileNotFoundError and PermissionError kept apart, and the four subcommand-only modules carry their reason on record. **The fifteen own-installer services** then had their declared paths confronted with reality: the registry's `binary` field was documented as absolute paths, specified by the schema as bare names resolved via `$PATH` *or* absolute paths, and implemented as neither — `Path(x).is_file()` resolves a bare name against the working directory, so running BOB from a directory holding a file called `mongod` reported MongoDB as installed. Two declared config files were directories, leaving openvpn's port unread from the server configs one level down. **And `stderr` was being captured and thrown away**: `auditctl -l` reports a switched-off audit subsystem there while exiting 0 with an empty stdout — indistinguishable from a reachable system holding no rules, which is what BOB called it. "The rules could not be listed" and "the operator turned auditing off" are different statements, and only the first is a gap in the audit. stderr stays untrusted text: matched against a known marker, never rendered, with a test that fails if it reaches a finding. **The design pass on `config_key` found nothing to design**: the field is declared four ways, read once, and since v0.15.2 decides nothing — 33 services read their config because they declare a path, 5 fall back because they do not, and the strategy name changes nothing in either group. `"ask"` never had an implementation and no service uses the named-key form. The three declarations now describe the code, retiring the field is queued as a contract change, and the invariant is pinned by tests including a guard that fails if a second reader appears. **The same question then went to the CLI**: two options the parser accepts had drifted out of the manual page — `--test-webhook`, added in v0.8.2, and `--html`, whose sibling `--json-full` is documented beside it. Both were in `--help`, but the manual is the reference and v0.13.4 had already repaired five undiscoverable options once. Two more are absent on purpose and now carry their reason in the guard, which fails on a stale exemption too. **Then the CLI surface itself was audited**, and `-r/--reconfigure` turned out to be the only one of the 43 fields `parse_args` populates that nothing reads — parsed and dropped since v0.1.0, advertised in `--help` and twice in the manual, while BOB printed "To reset it: bob --reconfigure" on every run that found a saved config. The wizard its wording promised never existed, so the restored contract is reset-and-exit, with an unconditional confirmation because the parser scopes `-y` to `--fix --apply`. **`--help` was byte-identical English under `--french`**, on the one screen that advertises `--french`: the `t` parameter had sat behind a "reserved for future i18n" note since v0.1.0. 83 keys per locale now, flags left untranslated as CLI syntax. The rewording closed all seven lines that ran past 100 characters, one by fifty, so the ceiling is now enforced. The `--install-completion` example interpolated a resolved `argv[0]`, printing a mode-644 file with no shebang from a checkout and a venv-internal path under pipx. Eight short forms and two aliases were added to the manual. **A sweep of locale keys nobody references** then found three orphans under `plugin.sandbox`, translated in both locales and never called because the conditions they describe were reported with hardcoded English instead — a French user read "Le plugin 'x.py' rejeté : Plugin 'x.py' missing required run_check function", half English with the name twice. The worker now ships a key and primitive params instead of a rendered sentence, `SandboxRejected` carries its own key, and the v0.11.2 localisation pass is extended to a surface it had not covered. Guards, all mutation-tested: no parsed option without a reader, no locale key without a reference, no advertised option absent from the manual, no help line past 100 characters. **And a leap day could take out the whole UFW log section.** Syslog timestamps carry no year, so the parser builds the date with the current year and rolls back one year when the result looks future-dated. The construction was guarded against ValueError; the rollback that followed was not, and Feb 29 exists in a leap year but not in the one before it — one such line raised out of `_parse_log` and the runner's section barrier degraded blocked-attempt statistics, top source IPs and bruteforce windows, from a single line. A Feb 29 line of the current leap year read on or after that day still parses exactly as before; only the future-dated case, which comes from an earlier leap year or a clock behind and would be filtered by the cutoff anyway, is now skipped. The angle that opened this — subprocess locale — came back clean: 15 of the 17 direct `subprocess` calls outside `_run.py` already force `LC_ALL=C`, and the two that do not read only the return code. **And `bob --diff` printed nothing at all, since v0.3.0.** The flag is advertised in `--help`, in the man page and in EXAMPLES, and shipped a cross-machine mode as a headline v0.9.0 feature; it emitted zero bytes — not on a change, not on an unchanged system, never. `_silent_mode` sets `config.quiet = True` so the audit stays mute while a post-audit view is shown, and `display_delta` writes through `output.print_*`. The comment above the branch states the contract — "display post-audit views with full output (no quiet filter)" — but only `--breakdown` implemented it: `_silent_mode` grew to cover `diff_mode` in v0.3.0, the commit that introduced `--breakdown`, and the new branch got its `output.init(quiet=False)` while the pre-existing one did not. Five test files touch the diff and none drove it through the CLI, so the components were covered and the wiring between them was not. Both views are now also excluded from the machine modes: `--diff` was accidentally safe there while it was mute, `--breakdown` was not and emitted its table straight into `bob -j | jq`. **Documentation pass before the tag.** `man/bob.conf.5` documented `ssh_port = 2222` and `samba_ports = 445/tcp` as supported configuration keys; no code has ever read them, so a reader who set a non-standard SSH port there would have believed BOB was auditing it. README_TECH described a prompt that "offers to save the port" — it does not exist either, and the same invented feature appeared in six more places across both languages plus the `bob/config.py` module docstring. All corrected to what the file actually holds: audit profile, webhook, SUID whitelist, log directories. Changelog date formats were mixed: the structural fields are ISO on the English side and DD-MM-YYYY on the French one, and v0.15.2 and v0.15.3 had been written ISO in the French table and headings, breaking a column uniform for seventy releases — now pinned by a guard that also checks both languages date the same version on the same day. Man pages bumped to the release date, and a French proofreading pass on the strings added this release. **Tests** 7560 → **7743**. |
| [v0.15.2](#v0152) | 2026-08-31 | **Second v0.15.x patch — "I could not look" stops sounding like "nothing to see".** A local stress test of the v0.15.0/v0.15.1 fixes confirmed they hold under real adversarial system states, and found the second defect class those releases attacked still open **at its root**. `_run` returns a command's stdout and drops its exit status, so a refused, absent or failing binary hands the caller the same empty string as a healthy one with nothing to report. v0.15.0 and v0.15.1 closed that ambiguity check by check (`rules_readable`, `status_readable`, `query_failed`, `kernel_ipv6_readable`) without ever touching the shared primitive — 26 modules call `_run`, 7 guarded the result. **Five verdicts were still built on the ambiguity**, all reachable together: on a host with 34 listening sockets, taking `ss` off `PATH` — routine on minimal images and in containers, where `iproute2` is often absent — produced "0 listening port(s) detected on this system", "no IPv6 listeners", "0 established TCP connection(s)", "not actively listening" for every registry port, and a green ✔ in the summary box, with no degraded section and no warning anywhere. New `run_result()` reports stdout and success separately; `_run` keeps its exact contract for the ~50 call sites where empty and failed lead to the same correct conclusion. A tool-by-tool sweep (25 inspection commands failed in turn, verdicts diffed against a baseline) now reports no failing tool that produces a new reassuring claim. **Three further angles**, opened after that sweep looked clean, found the class alive in three more shapes: the **alarm direction** (a blinded `systemctl` made BOB warn that a *running* sshd was "installed but not running", with a contradictory green tick in the same summary box), a **command that succeeds but says nothing usable** (`iptables`/`nft` exiting 0 on unparseable output still earned the host `firewall_drivers.no_issues`), and **files rather than commands** — an unreadable `/etc/sudoers`, `/etc/passwd`, `/etc/login.defs` or `/etc/ssh/sshd_config` each produced an affirmative all-clear, the last reporting `PermitRootLogin yes` as restricted because every unread directive falls back to OpenSSH's compiled-in default. **A third pass then enumerated the paths instead of sampling them** — every `Path("/etc…")` literal in the checks, made unreadable in turn — and found three the hand-picked sample had missed, one of them a subtler mechanism: `configparser.read` skips a file it cannot open *without raising*, so samba.py's existing `conf_readable` guard was inert and an unreadable `smb.conf` certified SMB1 disabled by a parser that had read nothing (the same trap in `profiles.py` quietly emptied a profile). Also fixed: cron files that never opened still earned `cron.ok`, and an unreadable `journald.conf` made `Storage=volatile` read as persistent. Two angles were measured and found clean (readable-but-corrupted content; commands that hang rather than fail), and two earlier claims are withdrawn as false positives of prefix-grouping. **A fourth pass took the directories** the checks list rather than the files they read, and found the most severe defect of the cycle: `/etc/ssh` at mode 0700 lost the entire audit — no report, no findings, exit 3 — because `Path.exists()` re-raises EACCES instead of returning False, from an always-on core collection that is deliberately unguarded. Present identically on the v0.15.1 tag. New `path_exists()` generalises a workaround `ddns` had carried alone since v0.13.x, all 30 call sites are migrated, and a test forbids a return to the bare call. **The same principle then applied to writing**: `bob -d` against a read-only or full log directory printed `Fatal error: Read-only file system` and stopped with no findings at all, because the report is opened before any check runs — while the same module already prints "Report writing disabled" and finishes the audit when a write fails one step later. An unopenable report now degrades the same way. Also probed and found clean: masked `/proc/sys` (real containers keep the sysctl trees, so the v0.15.0 "absent tree means IPv6 off" reading stands) and hostile `--diff` baselines (binary, truncated, wrong types, wrong shape — precise errors, no tracebacks). **The five state files BOB writes and reads back** were then swept the same way — twenty hostile combinations, nineteen degrading cleanly. The twentieth: `config.conf` holding non-UTF-8 bytes aborted the whole audit with `Fatal error: 'utf-8' codec can't decode byte 0xfa`, without naming the file, because `UnicodeDecodeError` is a `ValueError` and the guard named `OSError` — in a loader that already documents ignoring malformed lines. **The scoring layer was then swept as properties** rather than examples: the breakdown accounts for the raw score exactly (cap included), 10/10 stays reserved for an audit with nothing to fix, `--ignore` removes a finding *and* its deduction without disturbing any other key, and profile `info`/`skip` drop the points while `alert` keeps them. Nothing was broken; the invariants are now tests, with a bite check so they cannot decay into assertions about nothing. **A last pass asked which defect classes the campaign had closed only on the leaves it visited** — mechanically detectable as one rule living in several private copies — and found the UFW grammar story repeating one release later: `_split_addr_port` exists in `ports` and in `network_context`, same name, different behaviour, only the first taught about IPv6 brackets in v0.15.0. Every IPv6 peer therefore failed the private-address test, so a local PostgreSQL connection over `[::1]` was reported as an established connection to an external IP on a sensitive port, with a two-point deduction. `split_ss_address()` is now the single grammar, with a mutation-tested guard against a private copy returning. **Then the 30 services no machine has installed**: only 8 of the registry's 38 exist on the development host, so the config readers for the rest had never run — and they need no installing, since `_auto_detect_port` reads files. The reader took the first digits after a directive keyword, so `listen 127.0.0.1:8080;` — the ordinary way to bind nginx to one interface — was audited as **port 127**, `listen [::]:443` matched nothing at all, and Redis's `port 0` (TCP disabled, a hardened setting) was reported as `0/tcp`. That is the v0.15.0 UFW defect in a third module. Six further config formats the reader did not know were found by sweeping the rest — Elasticsearch's `http.port`, memcached's `-p`, mosquitto's `listener`, Jellyfin's XML, Caddy's site address, the `<NAME>_PORT` env style — and **20 services across seven format families are now pinned by tests**. **The presence-only half of the registry followed**: three of the seven ship a port in a standard system file while declaring `config_key: "fixed"`, so BOB asserted the default without looking — PostgreSQL on 5432 and WireGuard on 51820 whatever the operator had set. Wiring them needed wildcard paths (PostgreSQL versions its directory), WireGuard's CamelCase `ListenPort`, and Syncthing's `<address>` XML. Sweeping the covered twenty for less obvious shapes then found two opposites: MySQL's `[client]` section carries the port clients connect *to* and was answering first, and a repeated `port` key was read as its first occurrence when redis, mysql and YAML all apply the last. One distinction settles both — `listen`/`listener` are additive, a `port` key overrides. **The largest find came last**: parsing correctly is worthless if the audit never calls the parser, and `_resolve_ports` acted on `config_key == "auto"` alone. Eight services carry a config path under `config_key: "ask"` — a strategy the registry documents as prompting the operator, which no code implements — so nginx, apache, caddy, authelia, vaultwarden, homeassistant, adguard_home and ollama were every one audited on the registry default, with the port actually exposed never examined. The dispatch now reads whenever a path is declared, and the reader answers with every listener rather than one, since dropping 443 from an nginx serving 80 and 443 would trade a wrong port for a missing one. Also fixed: `;` comments in INI files were read as live ports, and JSON string, XML attribute and YAML `!!int` forms were invisible. **Finally the paths themselves**: `nginx.conf` carries no `listen` on any stock install — measured on both layouts, the upstream image keeps server blocks in `conf.d/*.conf` and the Debian package in `sites-enabled/*`, which are symlinks into `sites-available`. The registry declared `nginx.conf` alone, so the file deciding the port was never opened; the include directories are now declared, and a scoped policy follows a symlink while its target stays inside the service's own config tree, leaving `sites-enabled/evil -> /etc/shadow` refused. **And the paths named one distribution while BOB claims five**: measured in Fedora containers with the real packages, `/etc/apache2`, `/etc/mysql` and `/etc/memcached.conf` do not exist there and `/etc/vsftpd.conf` is `/etc/vsftpd/vsftpd.conf`, so four services fell through to the registry default across the whole RHEL family. Both families are declared now, verified end to end on real Fedora and Debian installs with their ports edited. Five services of the thirty-three that declare a path — the angle is opened, not closed. **That work then exposed something larger**: BOB decided a service was installed through `dpkg-query` alone, which does not exist on the RHEL family, Arch, openSUSE or Alpine. Every service in the registry therefore read as NOT INSTALLED on four of the five distributions BOB claims — no error, no crash, the services section simply reported everything absent. Measured on a Fedora container with httpd, vsftpd, memcached and mariadb-server installed and confirmed by `rpm -q`: BOB saw none of them. The query now walks `dpkg-query`, `rpm`, `pacman` and `apk`, package-name equivalents were added to the registry, and the four checks that each kept a private dpkg call now share one — a guard found the fourth while it was being written. **The twenty-three distribution-packaged services were then measured on all four families** (the other fifteen ship their own installer, so they have no distribution variance): **Redis is Valkey now** — `dnf install redis` and `pacman -S redis` both leave `/etc` empty and the config that exists is `/etc/valkey/valkey.conf`; **PostgreSQL keeps its config outside `/etc`** on rpm hosts, where `initdb` writes it into the data directory, so the port of a critical-risk service was never read there; proftpd and Apache each sit at a different path on each family. Verified with edited ports on real installs. **openSUSE completed the five**: rpm-based but Debian-named, it keeps `Listen` in `/etc/apache2/listen.conf` — a file none of the other four has — and ships redis as a template the administrator copies to an unpredictable name, answered by a scoped glob. Apache alone now names four layouts, which is the shape of the whole problem. **Then what the checks read in those files beyond the port**: SSH proved correct, its `sshd_config.d` drop-in precedence checked against `sshd -T` in both directions. `ssl_certs` did not — a certificate BOB never finds is one that never expires, and it missed them twice over. Debian's stock nginx site is `sites-enabled/default` with no extension while the scan globbed `**/*.conf`, and only `/etc/apache2` was scanned so Fedora and Arch vhosts under `/etc/httpd` went unexamined. Measured with a real certificate five days from expiry: nothing found before, four days reported now. **Tests** 7288 → **7560**. |
| [v0.15.1](#v0151) | 2026-08-31 | **First v0.15.x patch — a second hunt, to find out whether the first one made BOB resilient.** v0.15.0 fixed 26 verdict defects by sweeping six signatures. Repeating those sweeps would mostly re-confirm its own fixes, so this pass attacks from angles the previous one never used — chief among them **cross-checking the checks against each other**: BOB has overlapping surfaces (`firewall` / `firewall_stack` / `iptables_nftables` / `ports`, `ipv6` / `hardening`, `services` / `ports`) and two checks that disagree about the same machine is a defect no single-module audit can see. Also carries the documentation pass v0.15.0 deferred: five measured inaccuracies, including a `SECURITY.md` guarantee stated more broadly than it holds. **Tests** 7220 → **7288**. |
| [v0.15.0](#v0150) | 2026-08-31 | **First v0.15.x release — verdict accuracy.** The v0.15.0 objective is the correctness of what BOB *says*, before any new deduction is added: a wrong verdict discredits the tool more than a missing one. Each fix in this cycle is differential-tested against the tool's own parser — `sshd -T` / `ssh -G` for SSH, `cvtsudoers -f json` for sudoers — so the expectations record what the system actually does rather than what the configuration line looks like. **One defect class dominates**: a check's patterns were written against the tidy form of a line and stopped matching the moment an operator added a trailing comment — so the more disciplined the administrator, the likelier the miss. Found so far in `sshd_config`/`ssh_config` (server and client), in `ufw status` (a commented `allow from anywhere` wildcard was **completely undetected**, ALERT and 2-point deduction both gone), in `sudoers` (`NOPASSWD: ALL # temporary` silently downgraded from WARN −2 to a scoreless INFO), and — the inverse and more dangerous direction — in `apt.conf` (a commented-out `Unattended-Upgrade "1"` was reported as **enabled**, a reassurance for a control apt does not read at all). **Also**: the audit's main loop had never been executed by a test — three files inspect `run_checks` with `ast`, none ran it — and the first live run found five findings constructed **without a key**, unreachable by `--explain` and anonymous in the JSON. **And**: the always-on core still aborted the whole audit on any failure — seven of eight injected faults lost it, one after 29 962 bytes had already reached the operator. Ten of the twelve core collections are leaves and now degrade in place; the two that are not stay unguarded on purpose, because an empty port table would render as *"nothing is listening"*. **Password policy**: login.defs and pwquality.conf are last-one-wins, so appending the hardened value to the end of the file — what every guide writes — left BOB reporting the value the operator had just overridden; `pwquality.conf.d/` was never opened; and a wrapped PAM line lost its `minlen=`. **Kernel hardening**: an unreadable sysctl fell back to a *hardened* default, so a kernel built without Yama got `[OK] ptrace restricted` for a protection that did not exist — "not read" is now a distinct answer from any value. A sweep found the same pattern ten knobs wide in the SYSTEM HARDENING section, where an unreadable `/proc/sys` produced ten passes for ten parameters BOB never read; the JSON `hardening` block now reports an unread parameter as `null`. The same sweep found that with `UseDNS` on, sshd logs a hostname rather than an IP, which `auth_log` filed as *private* — so a host accepting SSH logins from the internet reported none. **Disk**: the partition filter matched on the device path starting with `/dev/`, which no ZFS dataset does — a Proxmox or Ubuntu-on-ZFS root at 93% produced no finding at all — and a mount point containing a space was truncated to its first word. **systemd**: `systemctl list-units` prefixes a not-ok unit with a status glyph that shifts every column, so `services_state` read the glyph as the unit name and `loaded` as its state — a failed security service was invisible to the check whose whole purpose is to report one. **Accounts**: an account expiring *today* is already locked out per `chage(1)`, and BOB's strict `<` called it fine; an expiry field of `0` is ambiguous per `shadow(5)` and is now reported as such rather than skipped. **TLS**: `days_left` is a floored timedelta, so `days <= 0` covered the whole final day of a *valid* certificate — one with 23 hours left was reported EXPIRED with a 2-point deduction, against `openssl x509 -checkend 0` calling it valid. **Cron and timers**: the "download piped into a shell" rule existed twice with different blind spots — `curl … | sudo bash`, the published and root-running form, matched neither, while `curl … | ssh host` matched one of them because "ssh" ends in "sh". **UFW coverage**: rules written as an application profile (`ufw allow OpenSSH`, the documented way), as a range (`6000:6007/tcp`) or as a list (`80,443/tcp`) were read as covering one port or none, so a correctly configured host was told its ports had no firewall rule. **Containers**: an unreadable seccomp status produced the same silence as an active filter, and `/proc/self/status` omits that line exactly on a kernel built without `CONFIG_SECCOMP` — the case where it matters most. **IPv6**: `/proc/sys/net/ipv6` is missing entirely when the kernel boots with `ipv6.disable=1`, and the reader answered "assume enabled if unreadable", so a host with no IPv6 stack was reported `[OK] IPv6 configuration is consistent`. **umask**: every file the reader scans is last-one-wins and it took the first match, so a stricter value appended to `/etc/login.defs` or `/etc/profile` was invisible — and a value *weakened* by an appended line was reported as the strict one it had replaced. **Network context**: `ss` omits the State column when given a state filter, so the connection parser read the peer as the local address and the process column as the peer — every established connection was dropped (32 reported by ss, 0 by the snapshot), which silenced the sensitive-remote-port finding entirely and zeroed the connection counts in the JSON and the display. **auditd and fail2ban**: `auditctl -l` and `fail2ban-client status` both return empty output when the query is refused, which was read as "nothing configured" — a WARN and a deduction (two, for auditd) for a configuration never read. **AppArmor and the firewall ruleset**: same shape — `aa-status` prints the module line then fails on the profile set, so a host with 120 enforced profiles was told it had none; and `iptables -S` leaves stdout empty when refused, which read as "no firewall backend at all" and cost 3 points. **Service exposure**: `services` searched the whole ufw line for the port, so one rule `80/tcp ALLOW IN 192.168.1.22` made ports 1, 22, 168 and 192 all read as "restricted to the local network" — a host running SSH with no rule at all was told SSH was firewalled. The ufw grammar now lives in one module shared with `ports`. **Docker**: `[]`, `null` and `"text"` are valid JSON without a `.get()`, and only `JSONDecodeError` was caught, so a broken `daemon.json` — the state Docker refuses to start on — raised out of the snapshot and cost the whole Docker section. **Markdown report**: finding messages carry system-derived names, and only `|` was escaped — so `<img src=x onerror=…>`, `[click](javascript:…)` and `![x](https://…)` all reached the report intact, the last making the auditor's machine fetch a remote image on open. The HTML writer and the `-d` report already escaped this. **Slack webhook**: the same content went out unescaped to Slack, where `<!channel>` notifies a whole workspace and `<http://elsewhere|looks legitimate>` renders as a link — a name on the audited host controlling what the security channel displays. **Tests** 6719 → **7220**. |
